...
vi.mock('@/lib/storage/signedUrl', () => ({
  signedUrlForKey: async (key: string) =>
    `https://signed.local/${encodeURIComponent(key)}?sig=fake&exp=999999`,
}));
...
vi.mock('@/lib/mercadopago/client', () => ({
  ...
  MP_CONFIG: {
    webhookSecret: 'stub',
    successUrl: 'http://localhost:3000/f',
    failureUrl: 'http://localhost:3000/f',
    pendingUrl: 'http://localhost:3000/f',
    sandbox: true,
    publicKey: 'pk_test',
  },
}));
...
describe('V1 Flow Integración (estable)', () => {
  ...
  it('1) Anchor-detect: POST /api/admin/anchor-detect { eventId } → detected>=2', async () => {
    ...
    const req = new Request('http://localhost/api/admin/anchor-detect', {
      method: 'POST',
      body: JSON.stringify({ eventId: ctx.eventId, onlyMissing: false }),
    }) as any;
  ...
  it('2) Group: POST /api/admin/group { eventId } → assigned>0, unassigned>=1', async () => {
    ...
    const req = new Request('http://localhost/api/admin/group', {
      method: 'POST',
      body: JSON.stringify({ eventId: ctx.eventId, dryRun: false }),
    }) as any;
    const res = await (POST as any)(req);
    const data = await res.json();
  ...
  it('3) Publish: POST /api/admin/publish { codeId: SV-001 } → token y URL', async () => {
    ...
    const req = new Request('http://localhost/api/admin/publish', {
      method: 'POST',
      body: JSON.stringify({ codeId: ctx.codes['SV-001'].id }),
    }) as any;
    const res = await (POST as any)(req);
    const data = await res.json();
    expect(res.status).toBe(200);
  ...
  it('4) Gallery: GET /api/family/gallery-simple/[token]?page=1&limit=60 → 200, photos.length>=3, URLs firmadas y sin storage_path/code_id', async () => {
    ...
    const { GET } = await import(
      '@/app/api/family/gallery-simple/[token]/route'
    );
    const req = new Request(
      `http://localhost/api/family/gallery-simple/${token}?page=1&limit=60`,
      { method: 'GET' }
    ) as any;
  ...
  it('5) Selection: POST /api/public/selection { token, selectedPhotoIds, package } → { ok:true, orderId }', async () => {
    ...
    const req = new Request('http://localhost/api/public/selection', {
      method: 'POST',
      body: JSON.stringify({
        token,
  ...
  it('6) Export: GET /api/admin/orders/export?eventId=... → CSV con fila SV-001', async () => {
    ...
    const req = new Request(
      `http://localhost/api/admin/orders/export?eventId=${ctx.eventId}`,
      { method: 'GET' }
    ) as any;
  ...
  it('7) Unpublish: POST /api/admin/publish/unpublish { codeId } → luego GET galería 403', async () => {
    ...
    const req1 = new Request('http://localhost/api/admin/publish/unpublish', {
      method: 'POST',
    ...
    const req2 = new Request(
      `http://localhost/api/family/gallery-simple/${token}?page=1&limit=10`,
      { method: 'GET' }
    ) as any;
