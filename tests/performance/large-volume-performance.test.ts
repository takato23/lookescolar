...
describe('Large Volume Performance Tests', () => {
  ...
  test('should handle bulk operations on 1000 photos', async () => {
    ...
    for (const batch of batches) {
      const response = mockApiResponses['/api/admin/photos/bulk-approve'](
        batch,
        true
      );
  ...
  test('should validate folder hierarchy performance with deep nesting', async () => {
    ...
    for (let level = 1; level <= 10; level++) {
      ...
      for (let cls = 1; cls <= 5; cls++) {
        const classFolder = generateMockFolder(
          level * 10 + cls,
          eventId,
          levelFolder.id
        );
        ...
        for (let sub = 1; sub <= 20; sub++) {
          const subFolder = generateMockFolder(
            level * 100 + cls * 10 + sub,
            eventId,
            classFolder.id
          );
  ...
  test('should simulate realistic school photo upload scenario', async () => {
    ...
    for (let batch = 0; batch < batches; batch++) {
      ...
      const response =
        mockApiResponses['/api/admin/photos/simple-upload'](files);
      ...
      (_, i) => async () => {
        ...
        switch (operationType) {
          case 0: // Photo approval
            return mockApiResponses['/api/admin/photos/bulk-approve'](
              [`photo-${i}`],
              true
            );
          case 1: // Photo move
            return mockApiResponses['/api/admin/photos/bulk-move'](
              [`photo-${i}`],
              `folder-${i % 10}`
            );
          ...
          case 3: // Photo upload
            return mockApiResponses['/api/admin/photos/simple-upload']([
              { name: `photo-${i}.jpg` },
            ]);
        }
      }
  ...
  test('should validate system limits and boundaries', () => {
    ...
    const maxPhotosInFreeTier = Math.floor(
      (limits.freeTierLimitGB * 1024 * 1024 * 1024) /
        (limits.targetPhotoSizeKB * 1024)
    );
...
describe('Edge Cases and Error Scenarios', () => {
  ...
  test('should handle malformed data', () => {
    ...
    expect(() => {
      const validPhotos = malformedPhotos.filter(
        (photo) =>
          photo &&
