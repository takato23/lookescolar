...
describe('Perf Smoke (mocks)', () => {
  ...
  it('A) firmar 100 keys < 500ms (CI: < 1500ms)', async () => {
    ...
    const keys = Array.from(
      { length: 100 },
      (_, i) => `events/evt/preview_${i}.webp`
    );
    ...
    const laxThreshold = process.env.SLOW_MACHINE
      ? Number(process.env.SLOW_MACHINE)
      : threshold;
  ...
  it('B) groupBetweenAnchors dryRun con 1k fotos < 1500ms (CI: < 3000ms)', async () => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      photos.push({
        ...
        is_anchor: isAnchor,
        anchor_raw: isAnchor
          ? `SV-${String(1 + (i % 6)).padStart(3, '0')}`
          : null,
        exif_taken_at: new Date(base.getTime() + i * 1000).toISOString(),
        created_at: new Date(base.getTime() + i * 1000).toISOString(),
        original_filename: `f_${i}.jpg`,
      });
    }
    const codes = [
      'SV-001',
      ...
      'SV-005',
      'SV-006',
    ].map((cv, idx) => ({ id: `c_${idx}`, code_value: cv }));
    ...
    vi.doMock('@/lib/supabase/server', () => ({
      createServerSupabaseServiceClient: async () => ({
        from: (table: string) => {
          if (table === 'photos') {
            return {
              select: () => ({ eq: () => ({ data: photos, error: null }) }),
            } as any;
          }
          if (table === 'codes') {
            return {
              select: () => ({ eq: () => ({ data: codes, error: null }) }),
            } as any;
          }
    ...
    const { groupBetweenAnchors } = await import(
      '@/lib/photos/groupBetweenAnchors'
    );
