/**
 * Lazy-loaded Photo Gallery with Dynamic Imports
 * Optimizes loading performance by splitting heavy components
 */
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {Array.from({ length: 20 }).map((_, i) => (
          ...
          >
            <div className="h-full w-full rounded-lg bg-gradient-to-r from-gray-200 via-gray-300 to-gray-200"></div>
          </div>
        ))}
      </div>
    </div>
...
export function LazyPhotoGallery({
  ...
  // Preload signed URLs for visible photos
  useEffect(() => {
    let isMounted = true;
