...
import { useVirtualScrolling, useIntersectionObserver } from '@/lib/utils/performance';
...
export function VirtualEventGrid({
  ...
  const handleEventSelect = useCallback((event: any, selected: boolean) => {
  ...
  const handleEventView = useCallback((event: any) => {
    onEventView?.(event);
  }, [onEventView]);

  // Render virtual event row
  const renderEventRow = useCallback((rowData: { item: any[]; index: number; offsetTop: number }) => {
    ...
    return (
      ...
      >
        ...
        {eventRow.length < responsiveColumns && 
          Array.from({ length: responsiveColumns - eventRow.length }).map((_, emptyIndex) => (
    ...
    <div ref={containerRef} className={cn("w-full", className)}>
      ...
      >
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="fixed bottom-4 right-4 neural-glass-card p-2 text-xs font-mono bg-black/80 text-white">
...
export function VirtualGridStats({ events, visibleRange }: { 
  events: any[]; 
  ...
  return (
    <div className="neural-glass-card p-3 text-xs space-y-1">
      ...
      <div>Rango: {visibleRange.startIndex}-{visibleRange.endIndex}</div>
      {memoryUsage && (
        <div>Memoria: {memoryUsage.used}MB / {memoryUsage.total}MB</div>
      )}
    </div>
  );
}