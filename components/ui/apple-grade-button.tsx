...
import { buttonVariants as animationVariants, springConfig, subtleSpring } from '@/lib/design-system/animations';

interface AppleGradeButtonProps extends Omit<HTMLMotionProps<'button'>, 'size'>, ButtonVariants {
  children: ReactNode;
  loading?: boolean;
  loadingText?: string;
  leftIcon?: ReactNode;
...
export const AppleGradeButton = forwardRef<HTMLButtonElement, AppleGradeButtonProps>(
  ({
    children,
    className,
    variant = 'primary',
    ...
    const getAgeVariant = () => {
      if (variant === 'primary') {
        switch (ageGroup) {
          ...
          case 'secondary': return 'secondary_school';
    ...
    return (
      <motion.button
        ...
        className={cn(
          ...
          gradient && 'bg-gradient-to-r from-current to-current bg-size-200 hover:bg-pos-100',
          className
        )}
        style={glowStyles}
...
export const FAB = forwardRef<HTMLButtonElement, Omit<AppleGradeButtonProps, 'variant' | 'size'>>(
  ({ className, ...props }, ref) => (
    <AppleGradeButton
      ref={ref}
      variant="primary"
...
// Icon button
export const IconButton = forwardRef<HTMLButtonElement, Omit<AppleGradeButtonProps, 'children'> & {
  icon: ReactNode;
  label: string;
}>(
...
// Social login button
export const SocialButton = forwardRef<HTMLButtonElement, AppleGradeButtonProps & {
