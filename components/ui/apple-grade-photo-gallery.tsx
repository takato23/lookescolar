          ...
          <div className="absolute bottom-4 left-4 rounded-lg bg-black/50 p-3 text-white backdrop-blur-sm">
            ...
            {photo.price && (
              <p className="text-sm opacity-90">
                ${photo.price}{' '}
                {photo.packageType === 'package' ? '(Paquete)' : '(Individual)'}
              </p>
            )}
          </div>
        </motion.div>
...
export const AppleGradePhotoGallery: React.FC<AppleGradePhotoGalleryProps> = ({
  ...
  const virtualizer = useVirtualizer({
    count: shouldVirtualize
      ? Math.ceil(photos.length / getItemsPerRow())
      : photos.length,
  ...
  function getItemsPerRow() {
    ...
    const containerWidth =
      scrollElementRef.current?.offsetWidth || window.innerWidth;
    const minItemWidth =
      ageGroup === 'kindergarten' ? 150 : ageGroup === 'elementary' ? 200 : 250;
    return Math.floor(
      containerWidth / (minItemWidth + parseInt(gridConfig.gap))
    );
  }
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      {selectionMode && selectedPhotos.length > 0 && (
        <motion.div
          className="sticky top-0 z-40 border-b bg-white/80 p-4 backdrop-blur-sm"
          style={{
