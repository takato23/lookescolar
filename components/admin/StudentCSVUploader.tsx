...
export function StudentCSVUploader({ 
  ...
  const [uploadMethod, setUploadMethod] = useState<'csv' | 'text' | 'manual'>('csv');
  ...
  // Parse CSV content
  const parseCSVContent = useCallback((content: string): Array<{ name: string; email?: string; grade_section?: string }> => {
    const lines = content.trim().split('\n');
    if (lines.length === 0) return [];
    
    const firstLine = lines[0];
    const hasHeaders = firstLine.toLowerCase().includes('nombre') || 
    ...
    return dataLines
      ...
      .map(line => {
        ...
        if (hasHeaders) {
          ...
          const gradeIndex = headers.findIndex(h => h.includes('grado') || h.includes('grade') || h.includes('curso'));
          ...
          return {
            name: columns[0] || '',
            email: columns[1] && columns[1].includes('@') ? columns[1] : undefined,
            grade_section: columns[2] || (columns[1] && !columns[1].includes('@') ? columns[1] : undefined),
          };
        }
      })
  ...
  const parseTextInput = useCallback((text: string): Array<{ name: string; grade_section?: string }> => {
    return text
      .split('\n')
      .map((line) => line.trim())
      .filter((line) => line.length > 0)
  ...
  const submitStudents = useCallback(async (students: Array<{ name: string; email?: string; grade_section?: string }>) => {
    ...
    try {
      ...
      toast.success(`✅ Se agregaron ${created.length} estudiantes al evento`, {
