...
const getPageInfo = (pathname: string): { title: string; subtitle?: string } => {
  ...
  if (pathname.startsWith('/admin/publish')) return { title: 'Publicar', subtitle: 'Compartir con familias' };
  if (pathname.startsWith('/admin/settings')) return { title: 'Ajustes', subtitle: 'Configuracion del sistema' };
  if (pathname.startsWith('/admin/store-settings')) return { title: 'Tienda', subtitle: 'Configuracion de tienda' };
  return { title: 'Admin' };
};

export default function CleanHeader({ user, onMobileMenuToggle }: CleanHeaderProps) {
  ...
  return (
    <header className="header-modern liquid-glass-navbar">
      ...
      <div className="header-modern-container relative">
        ...
        {/* Center Section: Quick Nav Pills - REMOVED: Using sidebar for desktop navigation */}
        {/* Navigation is handled by CleanSidebar on desktop and LiquidGlassNav on mobile */}
