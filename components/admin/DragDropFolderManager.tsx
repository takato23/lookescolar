...
export function DragDropFolderManager({
  ...
  const executeOperation = useCallback(async (operation: DragDropOperation) => {
    ...
    try {
      ...
      if (result && typeof result === 'object') {
        if (result.successful !== undefined && result.total_processed !== undefined) {
  ...
  const handleBulkAction = useCallback((action: 'publish' | 'unpublish') => {
  ...
  const renderOperation = useCallback((operation: DragDropOperation) => {
    const getStatusIcon = () => {
    ...
    return (
      <div
        key={operation.id}
        className={`p-3 rounded-lg border ${getStatusColor()} transition-all`}
      >
        ...
        {operation.results && (
          <div className="mt-2 text-xs text-gray-600">
            {operation.type === 'bulk_publish' && operation.results.successful && (
              <span>✅ {operation.results.successful}/{operation.results.total_processed} publicadas</span>
            )}
            {operation.type === 'bulk_unpublish' && operation.results.successful && (
              <span>✅ {operation.results.successful}/{operation.results.total_processed} despublicadas</span>
            )}
          </div>
        )}
      </div>
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card 
          ...
          onClick={() => selectedFolders.length > 0 && handleBulkAction('publish')}
        >
          <div className="text-center">
            <div className="mx-auto w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mb-3">
              <Eye className="h-6 w-6 text-green-600" />
            </div>
            <h3 className="font-medium text-green-800 mb-1">Zona de Publicación</h3>
        ...
        <Card 
          ...
          onClick={() => selectedFolders.length > 0 && handleBulkAction('unpublish')}
        >
          <div className="text-center">
            <div className="mx-auto w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center mb-3">
              <EyeOff className="h-6 w-6 text-orange-600" />
            </div>
            <h3 className="font-medium text-orange-800 mb-1">Zona de Despublicación</h3>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-blue-700 mb-3">
                Puedes usar las zonas de arriba para publicar/despublicar en lote, o arrastra las carpetas para organizarlas.
              </p>
      ...
      {isDragging && (
        <div className="fixed inset-0 bg-black/10 pointer-events-none z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-lg p-4 flex items-center gap-3">
            <Move className="h-5 w-5 text-blue-600" />
            <span className="font-medium">
              Moviendo {draggedItems.length} elemento{draggedItems.length === 1 ? '' : 's'}
            </span>
          </div>
        </div>
      )}
