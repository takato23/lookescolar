  ...
  useEffect(() => {
    const loadPhotos = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/events/${folder.parent_id}/folders/${folder.id}/photos`);
        ...
        if (response.ok) {
          ...
          const assetsResp = await fetch(`/api/admin/assets?folder_id=${folder.id}&limit=100`);
          if (!assetsResp.ok) {
  ...
  return (
    <Dialog open={true} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-6xl max-h-[90vh] p-0 overflow-hidden bg-white">
        {/* Header */}
        <DialogHeader className="p-4 sm:p-6 border-b border-slate-200">
          ...
          {loading ? (
            ...
            <div className="h-96 flex items-center justify-center">
              <div className="text-center space-y-3">
                ...
                <p className="text-slate-600 font-medium">Carpeta vacía</p>
                <p className="text-slate-500 text-sm">Esta carpeta no contiene fotos</p>
              </div>
            </div>
          ) : (
            <div className="h-full flex flex-col lg:flex-row">
              {/* Main Photo Display */}
              <div className="flex-1 relative bg-slate-50 flex items-center justify-center min-h-[300px] lg:min-h-0">
                  ...
                  <>
                    ...
                    <div className="relative max-w-full max-h-full">
                      <Image
                        ...
                        onError={() => handleImageError(photos[selectedPhoto].id)}
                    ...
                    {photos.length > 1 && (
                      <>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 hover:bg-white shadow-lg"
                          onClick={prevPhoto}
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 hover:bg-white shadow-lg"
                          onClick={nextPhoto}
                        >
              ...
              <div className="lg:w-72 border-t lg:border-t-0 lg:border-l border-slate-200 bg-white">
                <div className="p-4">
                  <h3 className="font-semibold text-slate-900 mb-3">
                    Fotos en la carpeta
                  </h3>
                  <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-3 gap-2 max-h-96 lg:max-h-[500px] overflow-y-auto">
