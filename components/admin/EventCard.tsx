...
export function EventCard({
  ...
  const revenueProgress = revenueTarget > 0 ? Math.min((event.stats?.revenue || 0) / revenueTarget * 100, 100) : 0;
  ...
  useEffect(() => {
    const timer = setTimeout(() => setShowMicroChart(true), 300 + animationDelay);
  ...
  const MicroChart = ({ progress, color, className }: { progress: number; color: string; className?: string }) => {
    const segments = 12;
    const bars = Array.from({ length: segments }, (_, i) => {
      const height = Math.sin((i / segments) * Math.PI) * 100 + Math.random() * 20;
    ...
    return (
      <div className={cn("flex items-end gap-0.5 h-8", className)}>
        {bars.map((bar, i) => (
          <div
            key={i}
            className={`w-1 ${color} transition-all duration-300 rounded-sm`}
            style={{
  ...
  const ProgressRing = ({ progress, size = 24, strokeWidth = 2, color }: { 
    progress: number; 
    size?: number; 
    strokeWidth?: number; 
    color: string;
  ...
  const cardContent = (
    <div
      ...
      className={cn(
        ...
        'hover:scale-[1.02] hover:shadow-2xl hover:-translate-y-3',
    ...
    >
      ...
      <div className={`absolute inset-0 ${colorScheme.gradient} opacity-40 transition-opacity duration-500`} />
      ...
      <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${colorScheme.primary} transition-all duration-500`}>
      ...
      {isDraggable && (
        <div className="absolute top-3 left-3 z-20">
          <DragHandle className="neural-glass-card bg-white/80 hover:bg-white rounded-lg" />
        </div>
      )}
      ...
      {onSelect && (
        <div className={cn(
          ...
          <button
            ...
            className={cn(
              "neural-glass-card w-6 h-6 rounded-lg flex items-center justify-center transition-all duration-200",
              isSelected 
      ...
      {priority !== 'medium' && (
        <div className="absolute top-3 right-3 z-10">
          <div className={cn("neural-glass-card px-2 py-1 text-xs font-medium rounded-lg", priorityColors[priority])}>
      ...
      <div className={cn(
          ...
          <div className="min-w-0 flex-1 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {completionRate > 90 && (
                <Badge
                  variant="outline"
                  className="neural-glass-card text-xs border-emerald-200 text-emerald-600 bg-emerald-50/50"
                >
                  ...
                  {event.metadata?.tags && event.metadata.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {event.metadata.tags.slice(0, 3).map((tag, index) => (
                        <span 
                          key={index} 
                          className="text-xs px-2 py-1 rounded-full bg-gray-100/80 text-gray-600 border"
                        >
                          {tag}
                        </span>
                      ))}
                      {event.metadata.tags.length > 3 && (
                        <span className="text-xs px-2 py-1 rounded-full bg-gray-100/80 text-gray-500">
                          +{event.metadata.tags.length - 3}
                        </span>
                      )}
                    </div>
        ...
        {showProgress && event.stats && (
          <div className="space-y-3 mb-4 border-t pt-4 border-white/20">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Etiquetado</span>
                    <span className="text-xs font-bold">{photoTaggingProgress}%</span>
                  </div>
            ...
            {revenueTarget > 0 && (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Meta de ingresos</span>
                    <span className="text-xs font-bold">{Math.round(revenueProgress)}%</span>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-1">
                    <div 
            ...
            {completionRate > 0 && (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Completado</span>
        ...
        {/* Photo Preview Grid */}
        {showPreview && event.stats?.photoPreview && event.stats.photoPreview.length > 0 && (
          <div className="space-y-2 mb-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Vista previa</span>
              <span className="text-xs text-gray-500">{event.stats.totalPhotos} fotos</span>
            </div>
            <div className="grid grid-cols-4 gap-1.5 rounded-lg overflow-hidden">
              {event.stats.photoPreview.slice(0, 4).map((photo, index) => (
                <div 
                  key={photo.id} 
                  className="aspect-square bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden group cursor-pointer"
                ...
                >
                  <img 
                    ...
                    loading="lazy"
                  />
                  {index === 3 && event.stats && event.stats.totalPhotos > 4 && (
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                      <span className="text-white text-xs font-bold">+{(event.stats?.totalPhotos || 0) - 3}</span>
                    </div>
                  )}
                </div>
              ))}
        ...
        {showAnalytics && event.stats && showMicroChart && (
          <div className="space-y-3 mb-4 border-t pt-4 border-white/20">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Tendencia</span>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="text-center">
                ...
                <div className="text-sm font-bold text-green-600">${(event.stats.revenue || 0).toLocaleString()}</div>
              </div>
              ...
              <div className="text-center">
                ...
                <div className="text-sm font-bold text-purple-600">{Math.round(engagementRate)}%</div>
              </div>
            </div>
            
            {/* Recent Activity */}
            {event.stats.recentActivity && (
              <div className="bg-gradient-to-r from-blue-50/50 to-purple-50/50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-2">
                  <Zap className="h-3 w-3 text-blue-500" />
                  <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Actividad reciente</span>
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  {event.stats.recentActivity.photosThisWeek && (
                    <div className="flex items-center gap-1">
                      <Camera className="h-3 w-3 text-blue-500" />
                      <span className="text-gray-600 dark:text-gray-400">{event.stats.recentActivity.photosThisWeek} esta semana</span>
                    </div>
                  )}
                  {event.stats.recentActivity.ordersThisWeek && (
                    <div className="flex items-center gap-1">
                      <ShoppingCart className="h-3 w-3 text-green-500" />
                      <span className="text-gray-600 dark:text-gray-400">{event.stats.recentActivity.ordersThisWeek} pedidos</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="mt-auto flex flex-col gap-3 border-t pt-3 border-white/20">
          ...
          <div className="flex gap-2">
            <Link href={`/admin/events/${event.id}`} className="flex-1">
              <Button className="neural-glass-card w-full border-white/20 hover:bg-white/20" size={compact ? "sm" : "md"}>
