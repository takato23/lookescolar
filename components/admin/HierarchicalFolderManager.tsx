...
export function HierarchicalFolderManager({
  ...
  const hierarchicalFolders = useMemo(() => {
    ...
    const folderMap = new Map(folders.map(folder => [folder.id, { ...folder, children: [] as FolderItem[] }]));
    ...
    if (statusFilter !== 'all') {
      const filterByStatus = (items: any[]): any[] => {
        return items.map(item => ({
          ...
          const matchesStatus = statusFilter === 'published' ? item.is_published : !item.is_published;
          const hasMatchingChildren = item.children && item.children.length > 0;
    ...
    const sortRecursive = (items: any[]): any[] => {
      return items.map(item => ({
        ...
        switch (sortBy) {
          ...
          case 'date':
            if (!a.published_at && !b.published_at) return a.name.localeCompare(b.name);
  ...
  const renderFolderItem = useCallback((folder: any, depth: number = 0) => {
        ...
        >
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              {folder.event_name && (
                <p className="text-sm text-gray-500 truncate">{folder.event_name}</p>
              )}
            </div>
  ...
  }, [selectedFolders, expandedFolders, onSelectionChange, onPublish, onUnpublish, onRotateToken, toggleExpanded, copyToClipboard]);
  ...
  const stats = useMemo(() => {
    ...
    return { total, published, unpublished: total - published, totalPhotos, selectedCount };
        ...
        <div className="flex justify-between items-center">
          ...
          {stats.selectedCount > 0 && (
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" onClick={() => onBulkPublish(selectedFolders)} className="bg-green-600 hover:bg-green-700">
                <Eye className="h-4 w-4 mr-1" />
                Publicar
              </Button>
              <Button size="sm" variant="outline" onClick={() => onBulkUnpublish(selectedFolders)}>
