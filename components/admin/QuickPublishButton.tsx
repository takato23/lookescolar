...
export default function QuickPublishButton({ eventId, photoIds, onComplete }: QuickPublishButtonProps) {
  ...
  const runQuickPublish = async () => {
    ...
    try {
      ...
      const watermarkBody = photoIds && photoIds.length > 0 
        ? { photoIds }
        : { eventId };
      ...
      toast.success(`Watermark aplicado a ${watermarkResult.processed || 'las'} fotos`);
      ...
      const codes = Array.isArray(listResult) ? listResult : listResult.rows || [];
      
      // Filter published codes with tokens
      const publishedCodes = codes.filter((c: any) => c.is_published && c.token);
      ...
      toast.error(error instanceof Error ? error.message : 'Error en el proceso de publicación');
    } finally {
  ...
  const getStepMessage = () => {
    switch(currentStep) {
      case 'upload': return 'Preparando...';
      case 'watermark': return 'Aplicando watermark...';
      case 'detect': return 'Detectando códigos QR...';
      case 'group': return 'Agrupando fotos...';
      case 'publish': return 'Publicando...';
      case 'links': return 'Generando enlaces...';
      ...
      <Dialog open={showResults} onOpenChange={setShowResults}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Se han generado {publishResults.length} enlaces para compartir con las familias
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              {publishResults.map((result, index) => (
                <div key={result.codeId} className="border rounded-lg p-3 space-y-2">
            ...
            {publishResults.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                No se encontraron códigos publicados. Verifica que las fotos tengan códigos QR asignados.
              </div>
            )}
          </div>
        </DialogContent>
