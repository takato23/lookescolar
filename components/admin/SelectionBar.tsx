...
export function SelectionBar({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-gray-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/75 lg:left-80">
      <div className="mx-auto max-w-[1440px] px-3 sm:px-4 h-14 flex items-center justify-between gap-3">
        <span className="text-sm text-gray-700">
          <strong>{count}</strong> seleccionada{count === 1 ? '' : 's'}
        </span>
        <div className="flex items-center gap-2">
          <Button variant="secondary" onClick={onMove}>Mover</Button>
          <Button variant="secondary" onClick={onAssign}>Asignar</Button>
          <Button variant="secondary" onClick={onPublish}>Publicar</Button>
