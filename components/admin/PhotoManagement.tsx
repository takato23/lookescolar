    ...
    filtered.sort((a, b) => {
      ...
      switch (state.sortBy) {
        case 'name':
          ...
          break;
        case 'date':
          const dateA = a.metadata?.dateCreated
            ? new Date(a.metadata.dateCreated)
            : new Date(0);
          const dateB = b.metadata?.dateCreated
            ? new Date(b.metadata.dateCreated)
            : new Date(0);
    ...
    deleteFolder: useCallback(async (folderId: string) => {
      try {
        ...
        setState((prev) => ({
          ...
          currentFolder:
            prev.currentFolder === folderId ? null : prev.currentFolder,
          isLoading: false,
        }));
    ...
    <div className={cn('flex h-full bg-muted', className)}>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <UploadQueue
          ...
          isUploading={state.uploadQueue.some(
