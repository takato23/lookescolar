...
import { DEFAULT_PRODUCTS, getProductsByType, getHighlightedProducts } from '@/lib/config/default-products';
import { TrustBadges } from '@/components/ui/trust-badges';
import { StoreFAQ } from '@/components/ui/faq';
import { FolderSelector } from '@/components/store/FolderSelector';
...
import type { FolderHierarchyProps, FolderNavigationHandlers } from '@/lib/types/folder-hierarchy-types';
...
export function PixiesetTemplate({
  ...
  const isPackageCompatible = (pkgId: string) => {
  ...
  const missingForFreeShipping = getShippingCost() > 0 ? 15000 - getSubtotal() : 0;
  ...
  const canCheckout = cart.length > 0 && (selectedPackage ? isPackageReady : allowExtrasOnly);
              ...
              <div className="flex gap-3 pb-2">
                ...
                {Object.entries(settings.products)
                  ...
                  .map(([id, product]) => {
                    ...
                    return (
                      <Card key={id} className="flex-shrink-0 w-32 p-3 hover:shadow-lg transition-shadow">
