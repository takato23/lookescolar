...
async function createExecSqlFunction() {
  ...
  console.log(
    '⚠️  Note: You may need to create the exec_sql function manually in Supabase SQL Editor:'
  );
...
async function main() {
  ...
  for (const migration of migrations) {
    ...
    try {
      ...
      console.error(
        `❌ Failed to read migration file ${migration}: ${err.message}`
      );
      allSuccess = false;
    }
  }
  ...
  if (allSuccess) {
    ...
    console.log(
      '⚠️  Some migrations had issues. Please review the output above.'
    );
    console.log(
