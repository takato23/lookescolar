...
async function executeAgent(agentKey) {
  ...
  try {
    execSync(`git checkout -b ${agent.branch} 2>/dev/null || git checkout ${agent.branch}`, { stdio: 'pipe' });
  ...
  log(`Or run: ./scripts/agents/workflow-publish-optimization.sh ${agentKey}`, 'yellow');
...
async function runWorkflow(phase = 'all') {
  ...
  if (phase === 'all') {
    ...
    log(`Unknown phase: ${phase}. Available: ${Object.keys(AGENTS).join(', ')}`, 'red');
    return false;
  }
