  ...
  try {
    ...
    for (const table of tables) {
      ...
      const backupFile = path.join(
        backupDir,
        `${table}_backup_${Date.now()}.json`
      );
      await fs.writeFile(backupFile, JSON.stringify(data, null, 2));
      await log(
        `Backed up ${data?.length || 0} records from ${table} to ${backupFile}`
      );
    }
...
async function migrateFolders(): Promise<Map<string, string>> {
  ...
  while (processedIds.size < oldFolders.length) {
    for (const oldFolder of oldFolders) {
      ...
      if (
        oldFolder.parent_folder_id &&
        !processedIds.has(oldFolder.parent_folder_id)
      ) {
        continue;
      }
      ...
      const newFolder: NewFolder = {
        ...
        name: oldFolder.name,
        parent_id: oldFolder.parent_folder_id
          ? folderMapping.get(oldFolder.parent_folder_id) ||
            oldFolder.parent_folder_id
          : null,
  ...
  await log(
    `${DRY_RUN ? 'Would migrate' : 'Migrated'} ${newFolders.length} folders`,
    'success'
  );
...
async function migratePhotos(
  ...
  for (const oldPhoto of oldPhotos) {
    ...
    const checksum = await generateChecksum(
      oldPhoto.storage_path + oldPhoto.file_size
    );
    ...
    const newAsset: NewAsset = {
      ...
      dimensions:
        oldPhoto.width && oldPhoto.height
  ...
  await log(
    `${DRY_RUN ? 'Would migrate' : 'Migrated'} ${newAssets.length} photos as assets`,
    'success'
  );
...
async function migrateGalleryShares(folderMapping: Map<string, string>) {
  ...
  await log(
    `${DRY_RUN ? 'Would migrate' : 'Migrated'} ${newAlbums.length} gallery shares to albums and tokens`,
    'success'
  );
}
...
async function validateMigration() {
  ...
  for (const validation of validations) {
    ...
    if (error) {
      await log(
        `Validation failed for ${validation.table}: ${error.message}`,
        'error'
      );
    } else {
...
async function main() {
  ...
  try {
    ...
    if (DRY_RUN) {
      ...
      await log(
        'Migration successful! The unified photo system is now ready.',
        'success'
      );
