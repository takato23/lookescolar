...
async function main() {
  ...
  try {
    ...
    if (viewError) {
      console.error(
        '❌ Error accediendo a folders_with_sharing:',
        viewError.message
      );
      return;
    }
    ...
    console.log(
      `✅ Vista accesible, ${viewData?.length || 0} folders encontrados`
    );
    ...
    try {
      const response = await fetch(
        'http://localhost:3000/api/admin/folders/published?include_unpublished=true',
        {
          headers: {
      ...
      if (!response.ok) {
        console.warn(
          `⚠️  API response: ${response.status} ${response.statusText}`
        );
        console.log(
        ...
        console.log(
          `✅ API funciona, ${data.folders?.length || 0} folders retornados`
        );
      }
    } catch (error) {
      console.warn(
        '⚠️  No se puede conectar al API local (servidor no corriendo)'
      );
    }
    ...
    if (codesError) {
      console.warn(
        '⚠️  No se puede acceder a tabla codes:',
        codesError.message
      );
    } else {
      const publishedCodes =
        codes?.filter((c) => c.is_published && c.token) || [];
      console.log(
        `📈 Codes obsoletos: ${codes?.length || 0} total, ${publishedCodes.length} publicados`
      );
    ...
    if (functionsError) {
      ...
      if (missingFunctions.length > 0) {
        ...
        console.log(
          '✅ Todas las funciones de folder sharing están disponibles'
        );
      }
    }
    ...
    console.log('\n💡 PRÓXIMOS PASOS:');
    console.log(
      '1. Si hay codes obsoletos, ejecutar: npm run migrate:codes -- --confirm'
    );
