...
async function verifyHierarchicalSystem() {
  ...
  try {
    ...
    const { error } = await supabase.rpc('api.get_token_context', {
      p_token: 'test',
    });

    if (error && error.message.includes('function api.get_token_context')) {
      console.log(
        '❌ API functions: not found - need to run canonical_api_functions.sql'
      );
    } else {
      console.log(
        '✅ API functions: schema and functions appear to be available'
      );
    }
  } catch (err: any) {
  ...
  if (tableCount >= 6) {
    // At least most core tables exist
