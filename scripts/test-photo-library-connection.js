...
async function testEventPhotoLibraryConnection() {
  ...
  try {
    ...
    const foldersResponse = await fetch(
      `${BASE_URL}/api/admin/events/${TEST_DATA.eventId}/folders`
    );
    ...
    const photosResponse = await fetch(
      `${BASE_URL}/api/admin/events/${TEST_DATA.eventId}/photos`
    );
    ...
    console.log(
      `   ✅ Found ${photosData.photos?.length || 0} photos in event`
    );

    // Step 3: Check if subjects exist for assignment
    console.log('\n2️⃣ Checking subjects for assignment...');
    const subjectsResponse = await fetch(
      `${BASE_URL}/api/admin/events/${TEST_DATA.eventId}/subjects`
    );
    ...
    if (subjectsData.subjects?.length === 0) {
      console.log(
        '   ⚠️  Warning: No subjects found. You need to create students/subjects for this event first.'
      );
    }
    ...
    if (subjectsData.subjects?.length > 0 && photosData.photos?.length > 0) {
      ...
      console.log(
        `   📝 Would assign ${photosData.photos.length} photos to ${assignmentPayload.subjectIds.length} subjects`
      );
      console.log(
        `   📝 Assignment API endpoint: POST ${BASE_URL}/api/admin/events/${TEST_DATA.eventId}/assign-folder-photos`
      );
    ...
    const familyResponse = await fetch(
      `${BASE_URL}/api/family/gallery-simple/${TEST_DATA.familyToken}?page=1&limit=10`
    );
    ...
    if (!familyResponse.ok) {
      ...
      console.log(
        `   💡 This is expected if photos haven't been assigned to subjects yet`
      );
    } else {
      console.log(`   ✅ Family gallery accessible`);
      console.log(
        `   📊 Found ${familyData.photos?.length || 0} photos for family`
      );

      if (familyData.photos?.length === 0) {
        console.log(
          `   💡 No photos found - this confirms photos need to be assigned via the Event Photo Library`
        );
      }
    }
    ...
    console.log(
      '1. Go to Event Photo Library: ' +
        `${BASE_URL}/admin/events/${TEST_DATA.eventId}/library`
    );
    ...
    console.log(
      '6. Test family gallery again: ' +
        `${BASE_URL}/f/${TEST_DATA.familyToken}/simple-page`
    );
    ...
    console.log(
      '- Verify database connection and Event Photo Library feature flags are enabled'
    );
  }
}
