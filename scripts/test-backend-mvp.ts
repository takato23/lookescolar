...
const generateToken = customAlphabet('ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789', 25)
...
async function log(step: string, success: boolean, data?: any, error?: string, duration?: number) {
  const status = success ? '✅' : '❌'
  console.log(`${status} ${step}${duration ? ` (${duration}ms)` : ''}`)
  if (error) console.error(`   Error: ${error}`)
  if (data && typeof data === 'object') console.log(`   Data: ${JSON.stringify(data, null, 2)}`)
  
  results.push({ step, success, data, error, duration })
}

async function testAPI(method: string, endpoint: string, body?: any, headers?: Record<string, string>): Promise<any> {
...
async function runBackendTest() {
  console.log('🚀 Iniciando prueba End-to-End del Backend MVP de Look Escolar\n')
  ...
  try {
    ...
    try {
      ...
      const { data, duration } = await testAPI('POST', '/api/admin/events', eventData, {
        'Authorization': `Bearer ${process.env.TEST_ADMIN_TOKEN || 'test'}`
      })

      eventId = data.event.id
      await log('Crear evento vía API', true, { id: eventId, name: eventData.name }, undefined, duration)
    } catch (error: any) {
    ...
    try {
      ...
      await log('Crear sujeto con token', true, { id: subjectId, token: `${token.substring(0, 8)}***` })
    } catch (error: any) {
    ...
    try {
      const { data, duration } = await testAPI('GET', `/api/gallery/${eventId}`)
      
      await log('Galería pública funcional', true, {
    ...
    try {
      ...
      const { data, duration } = await testAPI('POST', '/api/storage/signed-url', 
        { path: testPath },
        { 'x-family-token': token }
      )
      
      if (data.url && data.url.includes('supabase')) {
        await log('URLs firmadas funcionales', true, { expires_in: data.expiresIn }, undefined, duration)
      } else {
        await log('URLs firmadas funcionales', false, undefined, 'URL inválida generada')
      }
    } catch (error: any) {
    ...
    try {
      ...
      try {
        ...
        if (error.message.includes('Invalid signature')) {
          await log('Webhook MP validación de firma', true, { message: 'Rechaza firmas inválidas correctamente' })
        } else {
          await log('Webhook MP validación de firma', false, undefined, error.message)
        }
      }
    } catch (error: any) {
      await log('Webhook MP validación de firma', false, undefined, error.message)
