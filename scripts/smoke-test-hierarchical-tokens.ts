...
async function createTestData() {
  ...
  const courses = [
    { id: '00000000-0000-0000-0000-000000000011', name: 'Curso A', event_id: event.id },
    { id: '00000000-0000-0000-0000-000000000012', name: 'Curso B', event_id: event.id }
  ];
  ...
  const folders = [
    { id: '00000000-0000-0000-0000-000000000021', name: 'Carpeta Curso A', event_id: event.id, subject_id: courses[0].id },
    { id: '00000000-0000-0000-0000-000000000022', name: 'Carpeta Curso B', event_id: event.id, subject_id: courses[1].id }
  ];
  ...
  const assets = [
    { id: '00000000-0000-0000-0000-000000000031', filename: 'test1.jpg', folder_id: folders[0].id, event_id: event.id },
    { id: '00000000-0000-0000-0000-000000000032', filename: 'test2.jpg', folder_id: folders[0].id, event_id: event.id },
    { id: '00000000-0000-0000-0000-000000000033', filename: 'test3.jpg', folder_id: folders[1].id, event_id: event.id }
  ];
...
async function testToken(token: string, type: 'Event' | 'Course' | 'Family'): Promise<TokenTestResult> {
  ...
  try {
    // 1. Validar token
    const { data: context } = await supabase.rpc('public.get_token_context', { p_token: token });
    ...
    // 2. Obtener carpetas
    const { data: folders } = await supabase.rpc('public.folders_for_token', { p_token: token });
    result.foldersCount = folders?.length || 0;

    // 3. Obtener assets
    const { data: assets } = await supabase.rpc('public.assets_for_token', { p_token: token });
    result.assetsCount = assets?.length || 0;

    // 4. Test de aislamiento (verificar que Course token no ve Curso B)
    if (type === 'Course') {
      const courseAFolders = folders?.filter((f: any) => f.folder_name.includes('Curso A')) || [];
      const courseBFolders = folders?.filter((f: any) => f.folder_name.includes('Curso B')) || [];
      result.isolationTest = courseAFolders.length > 0 && courseBFolders.length === 0;
    } else {
    ...
    try {
      ...
      result.securityHeaders = robotsHeader?.includes('noindex') && 
    ...
    console.log(`✅ Token ${type} - Validación: ${result.validation}, Carpetas: ${result.foldersCount}, Assets: ${result.assetsCount}, Descarga: ${result.canDownload}, Aislamiento: ${result.isolationTest}, Headers: ${result.securityHeaders}`);
...
async function testDownloadPermissions(tokens: any) {
  ...
  try {
    ...
    const noDownloadResponse = await fetch(`${SITE_URL}/api/s/${tokens.event}/download/00000000-0000-0000-0000-000000000031`, {
      headers: { 'User-Agent': 'SmokeTest/1.0' }
    });
    
    const noDownloadBlocked = noDownloadResponse.status === 403;
    console.log(`✅ Token sin descarga bloqueado: ${noDownloadBlocked ? 'SÍ' : 'NO'} (${noDownloadResponse.status})`);

    // Test con token que permite descarga (Course)
    const downloadResponse = await fetch(`${SITE_URL}/api/s/${tokens.course}/download/00000000-0000-0000-0000-000000000031`, {
      headers: { 'User-Agent': 'SmokeTest/1.0' }
    });
    
    const downloadAllowed = downloadResponse.status === 200 || downloadResponse.status === 404; // 404 = archivo no encontrado es OK
    console.log(`✅ Token con descarga permitido: ${downloadAllowed ? 'SÍ' : 'NO'} (${downloadResponse.status})`);
...
async function cleanupTestData() {
  ...
  await supabase.from('access_tokens').delete().eq('event_id', '00000000-0000-0000-0000-000000000001');
  await supabase.from('assets').delete().eq('event_id', '00000000-0000-0000-0000-000000000001');
  await supabase.from('folders').delete().eq('event_id', '00000000-0000-0000-0000-000000000001');
  await supabase.from('subjects').delete().eq('event_id', '00000000-0000-0000-0000-000000000001');
  await supabase.from('events').delete().eq('id', '00000000-0000-0000-0000-000000000001');
...
async function main() {
  ...
  try {
    ...
    results.forEach(result => {
      ...
      console.log(`  ✓ Validación: ${result.validation ? '✅ PASS' : '❌ FAIL'}`);
      console.log(`  ✓ Carpetas encontradas: ${result.foldersCount} ${result.foldersCount > 0 ? '✅ PASS' : '❌ FAIL'}`);
      console.log(`  ✓ Assets encontrados: ${result.assetsCount} ${result.assetsCount > 0 ? '✅ PASS' : '❌ FAIL'}`);
      console.log(`  ✓ Permisos descarga: ${result.canDownload} ${result.type === 'Course' ? (result.canDownload ? '✅ PASS' : '❌ FAIL') : (result.canDownload ? '❌ FAIL' : '✅ PASS')}`);
      console.log(`  ✓ Aislamiento: ${result.isolationTest ? '✅ PASS' : '❌ FAIL'}`);
      console.log(`  ✓ Headers seguridad: ${result.securityHeaders ? '✅ PASS' : '❌ FAIL'}`);

      if (result.validation) passedTests++;
      if (result.foldersCount > 0) passedTests++;
      if (result.assetsCount > 0) passedTests++;
      if (result.type === 'Course' ? result.canDownload : !result.canDownload) passedTests++;
      if (result.isolationTest) passedTests++;
      if (result.securityHeaders) passedTests++;
    });

    console.log(`\n📥 Tests de descarga:`);
    console.log(`  ✓ Sin permisos bloqueado: ${downloadTests.noDownloadBlocked ? '✅ PASS' : '❌ FAIL'}`);
    console.log(`  ✓ Con permisos permitido: ${downloadTests.downloadAllowed ? '✅ PASS' : '❌ FAIL'}`);
    ...
    if (passedTests >= (totalTests + 2) * 0.8) { // 80% de éxito mínimo
