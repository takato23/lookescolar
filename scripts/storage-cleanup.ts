...
async function showCleanupSummary(photos: any[], options: CleanupOptions) {
  ...
  console.log(
    `Fecha límite: ${new Date(Date.now() - options.daysOld * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}`
  );
  ...
  for (const [eventKey, eventPhotos] of Object.entries(byEvent)) {
    ...
    if (options.verbose) {
      ...
      const newestDate =
        eventPhotos[eventPhotos.length - 1]?.created_at?.split('T')[0];
  ...
  if (options.verbose) {
    ...
    photos.slice(0, 10).forEach((photo, i) => {
      ...
      const path = photo.storage_path.substring(
  ...
  for (const eventId of eventIds) {
    try {
      ...
      if (metrics.length > 0) {
        ...
        console.log(
          `  📊 ${eventName}: ${totalMB} MB servidos (${eventPhotos.length} fotos a eliminar)`
        );
      }
    } catch (error) {
