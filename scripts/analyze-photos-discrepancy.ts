...
async function analyzePhotosDiscrepancy() {
  ...
  try {
    ...
    const { data: oldPhotos, error: oldError, count: oldCount } = await supabase
      .from('photos')
      .select('*', { count: 'exact' })
      .eq('event_id', eventId);
    ...
    const newSystemCount = folders?.reduce((total, folder) => {
    ...
    if (oldSystemCount !== newSystemCount) {
      ...
      if (oldSystemCount > newSystemCount) {
        console.log(`   ➡️  El sistema antiguo tiene MÁS fotos (${oldSystemCount - newSystemCount} más)`);
      } else {
        console.log(`   ➡️  El sistema nuevo tiene MÁS assets (${newSystemCount - oldSystemCount} más)`);
      }
    } else {
