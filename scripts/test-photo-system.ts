...
async function testPhotoSystem() {
  ...
  try {
    ...
    if (uploadResponse.status === 401) {
      ...
      console.log(
        `   ⚠️ Upload API - Unexpected status: ${uploadResponse.status}`
      );
    }
  } catch (error) {
  ...
  console.log('\n📊 Summary:');
  console.log(
    `   Components: ${allComponentsExist ? '✅' : '❌'} All critical components present`
  );
  console.log(
    `   Services: ${allServicesExist ? '✅' : '❌'} All services implemented`
  );
  console.log(
    `   Tests: ${testCount > 0 ? '✅' : 'ℹ️'} ${testCount} test files found`
  );
