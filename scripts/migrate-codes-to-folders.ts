...
async function main() {
  ...
  try {
    ...
    if (photoError) {
      console.warn(
        '⚠️  No se puede acceder a photos.code_id:',
        photoError.message
      );
    }
    ...
    console.log(
      `   Con fotos: ${codesWithPhotos.filter((c) => c.photos_count > 0).length}`
    );
    console.log(
      `   Publicados: ${codesWithPhotos.filter((c) => c.is_published && c.token).length}`
    );
    console.log(
      `   Con tokens válidos: ${codesWithPhotos.filter((c) => c.token && c.photos_count > 0).length}`
    );
    ...
    console.log(
      `   Con fotos: ${folders?.filter((f) => (f.photo_count || 0) > 0).length || 0}`
    );

    // PASO 3: Identificar migración necesaria
    console.log('\n🔄 PASO 3: Identificando datos a migrar...');

    const validCodes = codesWithPhotos.filter(
      (c) => c.token && c.photos_count > 0 && c.is_published
    );
    ...
    for (const code of validCodes) {
      console.log(
        `   • ${code.code_value}: ${code.photos_count} fotos, token: ${code.token?.substring(0, 8)}...`
      );
    }
    ...
    if (!shouldContinue) {
      console.log(
        '💡 Ejecuta con --confirm para continuar: npm run migrate-codes -- --confirm'
      );
      return;
    }
    ...
    if (!hasSharing) {
      ...
      console.log(
        '📄 Archivo: supabase/migrations/20250826_folder_sharing_system.sql'
      );
      return;
    }
    ...
    for (const code of validCodes) {
      try {
        ...
        if (createError) {
          console.error(
            `❌ Error creando folder para ${code.code_value}:`,
            createError.message
          );
          continue;
        }
        ...
        console.error(
          `❌ Error inesperado migrando ${code.code_value}:`,
          error
        );
      }
    }
    ...
    for (const { code, folder } of migratedCodes) {
      try {
        ...
        if (updateError) {
          console.error(
            `❌ Error actualizando fotos para ${code.code_value}:`,
            updateError.message
          );
        } else {
          console.log(
            `✅ Fotos actualizadas: ${code.code_value} (${code.photos_count} fotos)`
          );
        }
      } catch (error) {
        console.error(
          `❌ Error inesperado actualizando fotos para ${code.code_value}:`,
          error
        );
      }
    }
    ...
    for (const { code } of migratedCodes) {
      try {
        ...
        if (updateError) {
          console.error(
            `❌ Error marcando code como deprecated:`,
            updateError.message
          );
        }
      } catch (error) {
    ...
    for (const { code, folder } of migratedCodes.slice(0, 3)) {
      console.log(
        `   • /f/${folder.share_token} (era código: ${code.code_value})`
      );
    }
