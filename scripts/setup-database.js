...
async function setupDatabase() {
  ...
  try {
    ...
    const migrationPath = join(
      __dirname,
      '../supabase/migrations/20240106_001_create_base_schema.sql'
    );
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      if (error) {
        ...
        if (
          ...
          console.log(
            `   ⚠️  Advertencia (esperada): ${error.message.substring(0, 80)}...`
          );
        } else {
    ...
    if (!adminUser?.users || adminUser.users.length === 0) {
      console.log(
        '📝 No hay usuarios. Necesitas crear un usuario admin desde el dashboard de Supabase Auth.'
      );
      console.log(
        '   1. Ve a https://supabase.com/dashboard/project/exaighpowgvbdappydyx/auth/users'
      );
      console.log('   2. Haz click en "Add user"');
      console.log(
        '   3. Usa email: admin@lookescolar.com y una contraseña segura'
      );
    } else {
      console.log(
        `✅ Encontrados ${adminUser.users.length} usuario(s) en Auth`
      );
    }
