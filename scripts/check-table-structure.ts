  ...
  try {
    ...
    try {
      ...
      if (photosError) {
        ...
        console.log(`   ✅ Photos table exists with ${photos?.length || 0} photos for this event`);
        photos?.forEach(p => console.log(`      - ${p.filename} (${p.created_at})`));
      }
    } catch (e) {
    ...
    try {
      ...
      if (assetsError) {
        ...
        console.log(`   ✅ Assets table exists with ${assets?.length || 0} total assets`);
    ...
    try {
      ...
      if (foldersError) {
        ...
        console.log(`   ✅ Folders table exists with ${folders?.length || 0} folders for this event`);
        folders?.forEach(f => console.log(`      - ${f.name} (id: ${f.id}, parent: ${f.parent_id})`));
        ...
        if (folders && folders.length > 0) {
          ...
          for (const folder of folders) {
            ...
            if (!folderAssetsError && folderAssets) {
              console.log(`      Folder "${folder.name}": ${folderAssets.length} assets`);
              folderAssets.forEach(a => console.log(`        - ${a.filename} (${a.status})`));
            }
          }
        }
      }
    ...
    try {
      ...
      if (subjectsError) {
        ...
        console.log(`   ✅ Subjects table exists with ${subjects?.length || 0} subjects for this event`);
