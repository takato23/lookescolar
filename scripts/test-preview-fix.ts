...
try {
  ...
  console.log('❌ Preview proxy route NOT found at app/admin/previews/[filename]/route.ts');
  process.exit(1);
}

// Check PhotoAdmin.tsx changes
try {
  const result = execSync('grep -n "getPreviewUrl" components/admin/PhotoAdmin.tsx', { encoding: 'utf8' });
  if (result.trim()) {
    console.log('✅ PhotoAdmin.tsx has been updated with getPreviewUrl function');
...
console.log('4. Check browser network tab for successful /admin/previews/* requests');