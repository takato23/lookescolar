  ...
  try {
    ...
    if (data.event?.stats?.totalPhotos !== undefined) {
      ...
      count = data.folders.reduce(
        (sum: number, folder: any) => sum + (folder.photo_count || 0),
        0
      );
    }
...
async function verifyDashboardConsistency() {
  ...
  if (uniqueCounts.length === 1) {
    ...
    console.log(
      `   Todos los endpoints que funcionan muestran: ${uniqueCounts[0]} fotos`
    );
  } else if (uniqueCounts.length > 1) {
    console.log(`⚠️  INCONSISTENCIA DETECTADA!`);
    console.log(
      `   Diferentes números de fotos encontrados: ${uniqueCounts.join(', ')}`
    );
  } else {
  ...
  for (const [systemName, systemResults] of Object.entries(systemGroups) as [
    string,
    any[],
  ][]) {
    ...
    if (validCounts.length > 0) {
      ...
      if (uniqueSystemCounts.length === 1) {
        ...
        console.log(
          `   ⚠️  Inconsistente: ${uniqueSystemCounts.join(', ')} fotos`
        );
      }
    } else {
      console.log(`   ❌ No hay datos válidos`);
...
async function verifyDatabaseDirectly() {
  ...
  try {
    ...
    if (oldCount === newCount) {
      ...
      console.log(
        `⚠️  Diferencia de ${Math.abs((oldCount || 0) - newCount)} fotos`
      );
    }
  } catch (error: any) {
