...
class LoadTester {
  ...
  private async executeScenario(scenarioName: string, userId: number): Promise<void> {
  ...
  private async testPhotoUpload(userId: number): Promise<void> {
    ...
    const response = await fetch(`${this.config.baseUrl}/api/admin/photos/simple-upload`, {
      method: 'POST',
      body: formData,
  ...
  private async testFolderCreation(userId: number): Promise<void> {
    ...
    for (let i = 0; i < foldersCount; i++) {
      ...
      const response = await fetch(`${this.config.baseUrl}/api/admin/events/${eventId}/folders`, {
        method: 'POST',
        headers: {
  ...
  private async testBulkOperations(userId: number): Promise<void> {
    ...
    const photoIds = Array.from({ length: photosCount }, (_, i) => 
      `load-test-photo-${userId}-${i}`
    );
    
    // Test bulk approval
    const approvalResponse = await fetch(`${this.config.baseUrl}/api/admin/photos/bulk-approve`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...this.getAuthHeaders()
    ...
    if (approvalResponse.ok) {
      ...
      const moveResponse = await fetch(`${this.config.baseUrl}/api/admin/photos/bulk-move`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...this.getAuthHeaders()
  ...
  private async testBrowsing(userId: number): Promise<void> {
    ...
    for (let i = 0; i < pagesCount; i++) {
      ...
      if (!response.ok) {
        throw new Error(`Browsing request failed: ${response.status} for ${endpoint}`);
      }
  ...
  private recordMetric(scenarioName: string, responseTime: number, success: boolean): void {
  ...
  private updateCalculatedMetrics(scenarioName: string): void {
    ...
    // Calculate average
    metric.averageResponseTime = times.reduce((sum, t) => sum + t, 0) / times.length;
  ...
  private generateReport(): void {
    ...
    const totalDurationSeconds = (performance.now() - this.testStartTime) / 1000;
    ...
    for (const [scenarioName, metric] of this.metrics) {
      ...
      console.log(`  Successful: ${metric.successfulRequests} (${((metric.successfulRequests / metric.totalRequests) * 100).toFixed(1)}%)`);
      console.log(`  Failed: ${metric.failedRequests} (${metric.errorRate.toFixed(1)}%)`);
      console.log(`  Avg Response Time: ${metric.averageResponseTime.toFixed(2)}ms`);
      console.log(`  Min/Max Response Time: ${metric.minResponseTime.toFixed(2)}ms / ${metric.maxResponseTime.toFixed(2)}ms`);
      console.log(`  P95 Response Time: ${metric.p95ResponseTime.toFixed(2)}ms`);
      console.log(`  Throughput: ${metric.throughputPerSecond.toFixed(2)} req/sec`);
      console.log('');
    ...
    console.log(`  Error Rate: ${((totalFailed / totalRequests) * 100).toFixed(2)}%`);
    console.log(`  Overall Throughput: ${(totalRequests / totalDurationSeconds).toFixed(2)} req/sec`);
    console.log('');
  ...
  private checkPerformanceThresholds(): void {
    ...
    for (const [scenarioName, metric] of this.metrics) {
      const scenarioPassed = 
        ...
        (metric.successfulRequests / metric.totalRequests * 100) >= thresholds.minSuccessRate &&
      ...
      if (!scenarioPassed) {
        ...
        if (metric.averageResponseTime > thresholds.maxAverageResponseTime) {
          console.log(`    ⚠️  Average response time too high: ${metric.averageResponseTime.toFixed(2)}ms > ${thresholds.maxAverageResponseTime}ms`);
        }
        if (metric.p95ResponseTime > thresholds.maxP95ResponseTime) {
          console.log(`    ⚠️  P95 response time too high: ${metric.p95ResponseTime.toFixed(2)}ms > ${thresholds.maxP95ResponseTime}ms`);
        }
        if (metric.errorRate > thresholds.maxErrorRate) {
          console.log(`    ⚠️  Error rate too high: ${metric.errorRate.toFixed(2)}% > ${thresholds.maxErrorRate}%`);
        }
      }
    }
