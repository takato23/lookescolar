...
describe('Mercado Pago Integration', () => {
  ...
  describe('Payment Preference Creation', () => {
    ...
    it('should throw error when MP_ACCESS_TOKEN is missing', async () => {
      ...
      await expect(createPaymentPreference(params))
        .rejects
        .toThrow('MP_ACCESS_TOKEN no configurado');
    });
  });
  ...
  describe('Webhook Signature Verification', () => {
    it('should verify valid webhook signature', () => {
      ...
      const expectedSignature = 'v1=' + require('crypto')
  ...
  describe('MP Status Mapping', () => {
    ...
    it('should handle all possible MP statuses', () => {
      const mpStatuses = [
        'approved', 'pending', 'in_process', 'in_mediation',
        'rejected', 'cancelled', 'refunded', 'charged_back'
      ];

      mpStatuses.forEach(status => {
        expect(MP_STATUS_MAPPING[status as keyof typeof MP_STATUS_MAPPING]).toBeDefined();
      });
    });
  });
  ...
  describe('Single Pending Order Validation', () => {
    it('should allow order creation when no pending orders exist', async () => {
      ...
      vi.mocked(require('@/lib/supabase/server').createServiceClient)
    ...
    it('should throw error on database error', async () => {
      ...
      await expect(validateSinglePendingOrder('test-subject-id'))
        .rejects
        .toThrow('Error validando pedidos existentes');
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle MP SDK errors gracefully', async () => {
      ...
      vi.mocked(require('mercadopago').Preference)
        .mockImplementation(() => ({
      ...
      await expect(createPaymentPreference(params))
        .rejects
        .toThrow('Error al crear preferencia de pago');
    });
