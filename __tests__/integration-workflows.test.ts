...
import { setupTestData, cleanupTestData, createTestClient, setupMocks, assertSuccessfulCheckoutResponse } from './test-utils';
...
describe('Integration Workflows Test Suite', () => {
  ...
  afterAll(async () => {
    ...
    if (workflowContext.createdResources.orderIds.length > 0) {
      await supabase.from('orders').delete().in('id', workflowContext.createdResources.orderIds);
    }
  ...
  describe('Complete Admin Workflow', () => {
    test('Admin workflow: Create event → Upload photos → Tag photos → Manage orders', async () => {
      ...
      try {
        ...
        const createEventResponse = await fetch(`${API_BASE_URL}/api/admin/events`, {
          method: 'POST',
          headers: { 
        ...
        if (createEventResponse.ok) {
          ...
          console.log(`   ✓ Event created: ${eventData.event?.name || 'Success'}`);
          
          if (eventData.event?.id) {
            workflowContext.createdResources.eventIds.push(eventData.event.id);
          }
        }

        // 2. Create Subject for the event
        console.log('   Step 2: Creating subject...');
        const createSubjectResponse = await fetch(`${API_BASE_URL}/api/admin/subjects`, {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${workflowContext.adminToken}`
        ...
        if (createSubjectResponse.ok) {
          ...
          console.log(`   ✓ Subject created with token: ${subjectData.token ? 'Yes' : 'No'}`);
        }

        // 3. Upload Photos
        console.log('   Step 3: Uploading photos...');
        const mockImageBlob1 = new Blob(['workflow test image 1'], { type: 'image/jpeg' });
        const mockImageBlob2 = new Blob(['workflow test image 2'], { type: 'image/jpeg' });
        ...
        const uploadResponse = await fetch(`${API_BASE_URL}/api/admin/photos/upload`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${workflowContext.adminToken}` },
          body: formData
        });
        ...
        if (uploadedPhotoIds.length > 0) {
          ...
          if (taggingResponse.ok) {
            ...
            console.log(`   ✓ Tagged ${taggingData.tagged_count || 0} photos`);
          }
        }

        // 5. Verify Family Can See Photos
        console.log('   Step 5: Verifying family access...');
        const familyGalleryResponse = await fetch(`${API_BASE_URL}/api/family/gallery/${workflowContext.subjectToken}`);
        ...
        if (familyGalleryResponse.ok) {
          ...
          console.log(`   ✓ Family can see photos: ${hasNewPhotos ? 'Yes' : 'No'}`);
        ...
        const checkoutResponse = await fetch(`${API_BASE_URL}/api/family/checkout`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (checkoutResponse.ok) {
          ...
          console.log(`   ✓ Order created: ${createdOrderId.substring(0, 8)}...`);
        }
        ...
        if (createdOrderId) {
          ...
          // View order details
          const orderDetailResponse = await fetch(`${API_BASE_URL}/api/admin/orders/${createdOrderId}`, {
            method: 'GET',
            headers: { 'Authorization': `Bearer ${workflowContext.adminToken}` }
          });
          ...
          if (orderDetailResponse.ok) {
            ...
            console.log(`   ✓ Order details retrieved: Status ${orderData.status || 'unknown'}`);
    ...
    test('Admin QR workflow: Generate QR PDF → Print → Scan → Tag', async () => {
      if (!workflowContext.adminToken) {
      ...
      try {
        ...
        const qrPdfResponse = await fetch(`${API_BASE_URL}/api/admin/events/${workflowContext.eventId}/qr-pdf`, {
          method: 'GET',
          headers: { 'Authorization': `Bearer ${workflowContext.adminToken}` }
        });
        ...
        const mockImageBlob = new Blob(['QR workflow test image'], { type: 'image/jpeg' });
        const formData = new FormData();
        formData.append('eventId', workflowContext.eventId);
        ...
        if (uploadResponse.ok) {
          ...
          const familyAccessResponse = await fetch(`${API_BASE_URL}/api/family/gallery/${workflowContext.subjectToken}`);
          
          if (familyAccessResponse.ok) {
            const galleryData = await familyAccessResponse.json();
            const hasQrPhoto = galleryData.photos?.some((p: any) => p.id === qrPhotoId);
            console.log(`   ✓ QR-tagged photo accessible to family: ${hasQrPhoto ? 'Yes' : 'No'}`);
  ...
  describe('Complete Family Workflow', () => {
    test('Family workflow: Access gallery → Add to cart → Checkout → Payment', async () => {
      let familyOrderId: string;

      try {
        // 1. Access Family Gallery
        console.log('   Step 1: Accessing family gallery...');
        const galleryResponse = await fetch(`${API_BASE_URL}/api/family/gallery/${workflowContext.subjectToken}`);
        
        expect(galleryResponse.ok).toBe(true);
        const galleryData = await galleryResponse.json();
        
        console.log(`   ✓ Gallery accessed: ${galleryData.photos?.length || 0} photos found`);
        ...
        const cartItems = galleryData.photos.slice(0, 2).map((photo: any) => ({
          photo_id: photo.id,
          quantity: 1,
          price_type: 'base'
        }));

        const addToCartResponse = await fetch(`${API_BASE_URL}/api/family/cart`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            token: workflowContext.subjectToken,
        ...
        console.log(`   ✓ Family checkout completed: Order ${familyOrderId.substring(0, 8)}...`);
        ...
        const orderStatusResponse = await fetch(`${API_BASE_URL}/api/family/order/status`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            token: workflowContext.subjectToken,
        ...
        const webhookResponse = await fetch(`${API_BASE_URL}/api/payments/webhook`, {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
        ...
        const finalStatusResponse = await fetch(`${API_BASE_URL}/api/family/order/status`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            token: workflowContext.subjectToken,
        ...
        if (finalStatusResponse.ok) {
          ...
          console.log(`   ✓ Final order status: ${finalStatusData.status || 'unknown'}`);
        }
    ...
    test('Family error handling workflow: Expired token → Invalid photos → Payment failures', async () => {
      try {
        // 1. Test Expired Token Handling
        console.log('   Step 1: Testing expired token handling...');
        const expiredTokenResponse = await fetch(`${API_BASE_URL}/api/family/gallery/${workflowContext.testData.expiredToken}`);
        
        expect([401, 403]).toContain(expiredTokenResponse.status);
        console.log(`   ✓ Expired token properly rejected: ${expiredTokenResponse.status}`);

        // 2. Test Invalid Photo Access
        console.log('   Step 2: Testing invalid photo access...');
        const invalidPhotoResponse = await fetch(`${API_BASE_URL}/api/storage/signed-url`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        console.log(`   ✓ Invalid photo access properly rejected: ${invalidPhotoResponse.status}`);

        // 3. Test Invalid Checkout Data
        console.log('   Step 3: Testing invalid checkout data...');
        const invalidCheckoutResponse = await fetch(`${API_BASE_URL}/api/family/checkout`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            token: workflowContext.subjectToken,
        ...
        console.log(`   ✓ Invalid checkout data properly rejected: ${invalidCheckoutResponse.status}`);
        ...
        const failureWebhookResponse = await fetch(`${API_BASE_URL}/api/payments/webhook`, {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            'x-signature': 'v1=mock-valid-signature'
        ...
        console.log(`   ✓ Payment failure webhook handled: ${failureWebhookResponse.status}`);
  ...
  describe('Complete Public Workflow', () => {
    test('Public workflow: Browse public gallery → Select photos → Public checkout', async () => {
      let publicOrderId: string;

      try {
        // 1. Access Public Gallery
        console.log('   Step 1: Accessing public gallery...');
        const publicGalleryResponse = await fetch(`${API_BASE_URL}/api/gallery/${workflowContext.eventId}`);
        
        if (publicGalleryResponse.ok) {
          const publicGalleryData = await publicGalleryResponse.json();
          console.log(`   ✓ Public gallery accessed: ${publicGalleryData.photos?.length || 0} photos found`);
          ...
          if (publicGalleryData.photos.length === 0) {
            console.warn('   No public photos available for public workflow test');
            return;
          }
          ...
          const publicCheckoutResponse = await fetch(`${API_BASE_URL}/api/gallery/checkout`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              event_id: workflowContext.eventId,
          ...
          if (publicCheckoutResponse.ok) {
            ...
            console.log(`   ✓ Public checkout completed: Order ${publicOrderId.substring(0, 8)}...`);
            ...
            console.warn(`   Public checkout failed: ${publicCheckoutResponse.status}`, errorData);
          }

          // 4. Verify Public Order in Admin Panel
          if (publicOrderId && workflowContext.adminToken) {
            console.log('   Step 4: Verifying public order in admin panel...');
            const adminOrderResponse = await fetch(`${API_BASE_URL}/api/admin/orders/${publicOrderId}`, {
              method: 'GET',
              headers: { 'Authorization': `Bearer ${workflowContext.adminToken}` }
            });
            ...
            if (adminOrderResponse.ok) {
              ...
              console.log(`   ✓ Public order visible in admin: ${orderData.is_public_order ? 'Yes' : 'No'}`);
          ...
          console.warn(`   Public gallery access failed: ${publicGalleryResponse.status}`);
        }
  ...
  describe('Cross-Workflow Integration', () => {
    test('Admin photo approval affects family gallery visibility', async () => {
      if (!workflowContext.adminToken) {
      ...
      try {
        ...
        const mockImageBlob = new Blob(['cross-workflow test image'], { type: 'image/jpeg' });
        const formData = new FormData();
        formData.append('eventId', workflowContext.eventId);
        ...
        if (uploadResponse.ok) {
          ...
          workflowContext.createdResources.photoIds.push(crossWorkflowPhotoId);
          console.log(`   ✓ Photo uploaded: ${crossWorkflowPhotoId.substring(0, 8)}...`);
          ...
          if (taggingResponse.ok) {
            ...
            if (familyGalleryResponse.ok) {
              const galleryData = await familyGalleryResponse.json();
              const hasUnapprovedPhoto = galleryData.photos?.some((p: any) => p.id === crossWorkflowPhotoId);
              console.log(`   ✓ Family sees unapproved photo: ${hasUnapprovedPhoto ? 'Yes (ERROR!)' : 'No (Correct)'}`);
    ...
    test('Order status changes reflect across admin and family views', async () => {
      if (!workflowContext.adminToken) {
      ...
      try {
        ...
        const orderResponse = await fetch(`${API_BASE_URL}/api/family/checkout`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            token: workflowContext.subjectToken,
        ...
        if (orderResponse.ok) {
          ...
          const familyStatusResponse = await fetch(`${API_BASE_URL}/api/family/order/status`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              token: workflowContext.subjectToken,
          ...
          if (familyStatusResponse.ok) {
            ...
            console.log(`   ✓ Family view status: ${familyStatusData.status}`);
            expect(familyStatusData.status).toBe('pending');
          }

          // 3. Check same order from admin perspective
          const adminOrderResponse = await fetch(`${API_BASE_URL}/api/admin/orders/${orderId}`, {
            method: 'GET',
            headers: { 'Authorization': `Bearer ${workflowContext.adminToken}` }
          });
  ...
  describe('Performance Integration', () => {
    test('Complete workflow should complete within reasonable time limits', async () => {
      ...
      try {
        ...
        const quickCheckoutResponse = await fetch(`${API_BASE_URL}/api/family/checkout`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            token: workflowContext.subjectToken,
        ...
        if (quickCheckoutResponse.ok) {
          ...
          workflowContext.createdResources.orderIds.push(checkoutData.orderId);
        }
        ...
        const totalTime = Date.now() - startTime;
        console.error(`   ❌ Performance workflow failed after ${totalTime}ms:`, error);
        throw error;
      }
    }, TEST_TIMEOUT);
  });
