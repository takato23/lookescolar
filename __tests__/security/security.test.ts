...
import { SecurityValidator, SecuritySanitizer, SECURITY_CONSTANTS } from '@/lib/security/validation';
  ...
  describe('Input Validation and Sanitization', () => {
    describe('File Type Validation', () => {
      ...
      it('debería validar MIME type vs extensión de archivo', () => {
        ...
        validCombinations.forEach(({ filename, mimeType }) => {
          expect(SecurityValidator.validateFileMimeType(filename, mimeType)).toBe(true);
        });

        invalidCombinations.forEach(({ filename, mimeType }) => {
          expect(SecurityValidator.validateFileMimeType(filename, mimeType)).toBe(false);
        });
      });
      ...
      it('debería rechazar archivos demasiado grandes', () => {
        ...
        expect(SecurityValidator.isValidFileSize(100 * 1024 * 1024)).toBe(false); // 100MB
      });
      ...
      it('debería validar contenido real del archivo (magic bytes)', async () => {
        ...
        expect(await SecurityValidator.validateImageMagicBytes(validJpeg)).toBe(true);
        expect(await SecurityValidator.validateImageMagicBytes(validPng)).toBe(true);
        expect(await SecurityValidator.validateImageMagicBytes(fakeImage)).toBe(false);
      });
    });
    ...
    describe('Filename Sanitization', () => {
      ...
      it('debería generar nombres únicos para archivos conflictivos', () => {
        ...
        const unique1 = SecuritySanitizer.generateUniqueFilename(conflictingName);
        const unique2 = SecuritySanitizer.generateUniqueFilename(conflictingName);
    ...
    describe('Path Traversal Prevention', () => {
      ...
      it('debería normalizar y validar paths de storage', () => {
        const testPaths = [
          { input: 'events/test/../photos/img.jpg', expected: 'events/photos/img.jpg' },
          { input: 'events//test///photos/img.jpg', expected: 'events/test/photos/img.jpg' },
          { input: './events/test/photos/img.jpg', expected: 'events/test/photos/img.jpg' }
        ];
  ...
  describe('Access Control and Authorization', () => {
    describe('IP-based Access Control', () => {
      ...
      it('debería implementar rate limiting por IP', () => {
        ...
        for (let i = 0; i < maxRequests; i++) {
          expect(SecurityValidator.checkRateLimit(testIP, maxRequests, windowMs)).toBe(true);
        }

        // El siguiente request debería ser bloqueado
        expect(SecurityValidator.checkRateLimit(testIP, maxRequests, windowMs)).toBe(false);
      });
    });
    ...
    describe('Token-based Access Control', () => {
      it('debería validar tokens de familia con permisos limitados', () => {
        ...
        const adminPermissions = ['view:all_photos', 'create:event', 'manage:users'];

        expect(SecurityValidator.hasPermission(familyToken, 'view:own_photos')).toBe(true);
        expect(SecurityValidator.hasPermission(familyToken, 'view:all_photos')).toBe(false);
        expect(SecurityValidator.hasPermission(adminToken, 'view:all_photos')).toBe(true);
      });
      ...
      it('debería validar expiración de tokens', () => {
        ...
        expect(SecurityValidator.isTokenExpired(expiredToken.expiresAt)).toBe(true);
        expect(SecurityValidator.isTokenExpired(validToken.expiresAt)).toBe(false);
      });
      ...
      it('debería implementar refresh de tokens automático', () => {
        ...
        const shouldRefresh = tokenService.shouldRefreshToken(nearExpiryToken.expiresAt);
        ...
        expect(refreshedToken.expiresAt.getTime()).toBeGreaterThan(nearExpiryToken.expiresAt.getTime());
      });
    });
  });
  ...
  describe('Cryptographic Security', () => {
    describe('HMAC Signature Validation', () => {
      ...
      it('debería validar firmas de webhook de MercadoPago', () => {
        ...
        expect(SecurityValidator.validateWebhookSignature(
          webhookPayload, 
          validSignature, 
      ...
      it('debería rechazar firmas con timing attack protection', () => {
        ...
        const constantTimeCompare = vi.fn()
        ...
        expect(SecurityValidator.validateWebhookSignature(payload, validSig, secret)).toBe(true);
        expect(SecurityValidator.validateWebhookSignature(payload, invalidSig1, secret)).toBe(false);
        expect(SecurityValidator.validateWebhookSignature(payload, invalidSig2, secret)).toBe(false);
      });
    ...
    describe('Password and Session Security', () => {
      ...
      it('debería generar session IDs seguros', () => {
        (crypto.randomBytes as any).mockReturnValue(Buffer.from('secure-session-id-bytes'));
      ...
      it('debería validar integridad de session', () => {
        ...
        const validSession = SecurityValidator.signSession(sessionData, process.env.SESSION_SECRET!);
        expect(validSession).toHaveProperty('data');
        expect(validSession).toHaveProperty('signature');

        expect(SecurityValidator.verifySession(validSession, process.env.SESSION_SECRET!)).toBe(true);
        ...
        expect(SecurityValidator.verifySession(tamperredSession, process.env.SESSION_SECRET!)).toBe(false);
      });
    });
  });
  ...
  describe('Content Security and Validation', () => {
    describe('Image Content Validation', () => {
      it('debería detectar imágenes maliciosas', async () => {
        ...
        const oversizedImage = await sharp({
          create: { width: 10000, height: 10000, channels: 3, background: { r: 255, g: 255, b: 255 } }
        }).jpeg().toBuffer();
        ...
        expect(await SecurityValidator.validateImageContent(normalImage)).toBe(true);
        expect(await SecurityValidator.validateImageContent(oversizedImage)).toBe(false); // Muy grande
      });
      ...
      it('debería validar dimensiones de imagen', () => {
        ...
        validDimensions.forEach(({ width, height }) => {
          expect(SecurityValidator.isValidImageDimensions(width, height)).toBe(true);
        });

        invalidDimensions.forEach(({ width, height }) => {
          expect(SecurityValidator.isValidImageDimensions(width, height)).toBe(false);
        });
      });
      ...
      it('debería detectar y rechazar polyglot files', async () => {
        ...
        expect(await SecurityValidator.validateImageContent(polyglotFile)).toBe(false);
      });
    });
    ...
    describe('Metadata Security', () => {
      it('debería remover metadata peligroso de imágenes', async () => {
        ...
        const cleanedImage = await SecuritySanitizer.removeImageMetadata(imageWithMetadata);
        ...
        expect(await SecurityValidator.validateImageContent(cleanedImage)).toBe(true);
      });
      ...
      it('debería preservar metadata esencial para la aplicación', async () => {
        ...
        const processedImage = await SecuritySanitizer.processImageSecurely(imageBuffer);
  ...
  describe('Request Security and Validation', () => {
    describe('Request Rate Limiting', () => {
      it('debería implementar rate limiting por endpoint', () => {
        ...
        endpoints.forEach(({ path, limit, window }) => {
          ...
          for (let i = 0; i < limit; i++) {
            expect(SecurityValidator.checkEndpointRateLimit(ip, path, limit, window)).toBe(true);
          }
          
          // El siguiente debería fallar
          expect(SecurityValidator.checkEndpointRateLimit(ip, path, limit, window)).toBe(false);
        });
      });
      ...
      it('debería aplicar rate limiting diferencial por rol', () => {
        ...
        expect(SecurityValidator.checkRoleBasedRateLimit(ip, endpoint, 'family')).toBe(true);
        
        // Admin tiene límite más alto
        expect(SecurityValidator.checkRoleBasedRateLimit(ip, endpoint, 'admin')).toBe(true);
      });
      ...
      it('debería implementar backoff exponencial para requests abusivos', () => {
        ...
        const backoffTime = SecurityValidator.getBackoffTime(abusiveIP, endpoint);
    ...
    describe('Request Validation', () => {
      it('debería validar Content-Type headers', () => {
        ...
        validContentTypes.forEach(contentType => {
          expect(SecurityValidator.isAllowedContentType(contentType)).toBe(true);
        });

        invalidContentTypes.forEach(contentType => {
          expect(SecurityValidator.isAllowedContentType(contentType)).toBe(false);
        });
      });
      ...
      it('debería validar request size limits', () => {
        ...
        expect(SecurityValidator.isValidRequestSize(maxRequestSize + 1)).toBe(false);
      });
      ...
      it('debería validar request headers para anomalías', () => {
        ...
        expect(SecurityValidator.hasAnomalousHeaders(suspiciousHeaders)).toBe(true);
        expect(SecurityValidator.hasAnomalousHeaders(normalHeaders)).toBe(false);
      });
    });
  });
  ...
  describe('Logging and Monitoring Security', () => {
    describe('Security Event Logging', () => {
      it('debería logear eventos de seguridad sin exponer datos sensibles', () => {
        ...
        const secureLog = SecurityValidator.createSecureLogEntry('auth_attempt', sensitiveData);
      ...
      it('debería detectar patrones de ataque en logs', () => {
        const logEntries = [
          { ip: '192.168.1.100', endpoint: '/api/admin/login', status: 401, timestamp: Date.now() },
          { ip: '192.168.1.100', endpoint: '/api/admin/login', status: 401, timestamp: Date.now() + 1000 },
          { ip: '192.168.1.100', endpoint: '/api/admin/login', status: 401, timestamp: Date.now() + 2000 },
          { ip: '192.168.1.100', endpoint: '/api/admin/login', status: 401, timestamp: Date.now() + 3000 },
          { ip: '192.168.1.100', endpoint: '/api/admin/login', status: 401, timestamp: Date.now() + 4000 }
        ];
    ...
    describe('Anomaly Detection', () => {
      it('debería detectar anomalías en patrones de uso', () => {
        const normalActivity = [
          { timestamp: Date.now() - 60000, action: 'view_gallery', user: 'user1' },
          { timestamp: Date.now() - 50000, action: 'view_photo', user: 'user1' },
          { timestamp: Date.now() - 40000, action: 'add_to_cart', user: 'user1' }
        ];

        const anomalousActivity = [
          { timestamp: Date.now(), action: 'admin_login', user: 'user1' }, // Escalación de privilegios
          { timestamp: Date.now() + 1000, action: 'delete_all_photos', user: 'user1' }, // Acción destructiva
          { timestamp: Date.now() + 2000, action: 'export_user_data', user: 'user1' } // Exfiltración de datos
        ];

        expect(SecurityValidator.isAnomalousActivity(normalActivity)).toBe(false);
        expect(SecurityValidator.isAnomalousActivity(anomalousActivity)).toBe(true);
      });
      ...
      it('debería detectar intentos de evasión de security', () => {
        ...
        evasionAttempts.forEach(({ technique, detected }) => {
          expect(SecurityValidator.detectEvasionTechnique(technique)).toBe(detected);
        });
      });
    });
  });
  ...
  describe('Configuration and Environment Security', () => {
    describe('Environment Variable Security', () => {
      ...
      it('debería detectar secrets expuestos en código', () => {
        ...
        const exposedSecrets = SecurityValidator.findExposedSecrets(codeSnippets.join('\n'));
        
        expect(exposedSecrets.length).toBeGreaterThan(0);
        expect(exposedSecrets.some(s => s.includes('hardcoded-secret'))).toBe(true);
      });
    });
    ...
    describe('Security Headers', () => {
      it('debería configurar CSP headers correctamente', () => {
        ...
        expect(cspHeader).toContain("script-src 'self' 'unsafe-eval' 'unsafe-inline' https://sdk.mercadopago.com");
        expect(cspHeader).toContain("img-src 'self' blob: data: https://*.supabase.co");
