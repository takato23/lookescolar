...
const TEST_CONFIG = {
  ...
  courses: [
    // Jardín
    { name: 'Sala Verde', grade: 'Sala', section: 'Verde', level: 'Jardín', student_count: 15 },
    { name: 'Sala Amarilla', grade: 'Sala', section: 'Amarilla', level: 'Jardín', student_count: 18 },
    // Primaria
    { name: '1°A', grade: '1°', section: 'A', level: 'Primaria', student_count: 25 },
    { name: '1°B', grade: '1°', section: 'B', level: 'Primaria', student_count: 22 },
    { name: '6°A', grade: '6°', section: 'A', level: 'Primaria', student_count: 28 },
    // Secundaria
    { name: '1°A Sec', grade: '1°', section: 'A', level: 'Secundaria', student_count: 30 },
    { name: '5°B Sec', grade: '5°', section: 'B', level: 'Secundaria', student_count: 24 }
  ]
};
...
let testLevelIds: Record<string, string> = {};
...
describe('Comprehensive Workflow System Tests', () => {
  ...
  describe('Phase 1: Admin Authentication & Setup', () => {
    ...
    it('1.3 Create levels (optional hierarchy) should work', async () => {
      ...
      for (const level of TEST_CONFIG.levels) {
        const response = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/levels`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${adminSession.access_token}`
  ...
  describe('Phase 2: Course & Student Management', () => {
    ...
    it('2.1 Create courses within event and levels should work', async () => {
      ...
      for (const course of TEST_CONFIG.courses) {
        ...
        const response = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/courses`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${adminSession.access_token}`
      ...
      console.log('✅ Courses created across all levels:', Object.keys(testCourseIds));
    });
    ...
    it('2.2 Batch import students with token generation should work', async () => {
      ...
      for (const course of TEST_CONFIG.courses) {
        ...
        for (let i = 1; i <= course.student_count; i++) {
          ...
          const qrCode = course.level === 'Secundaria' ? `STU-${studentNumber}-QR` : null;
        ...
        const response = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/courses/batch?operation=students`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${adminSession.access_token}`
        ...
        console.log(`✅ Created ${students.length} students for ${course.name}`);
      }
      ...
      for (const level of hierarchyCheck.levels) {
        ...
        for (const course of level.courses) {
          ...
          for (const student of course.students) {
            ...
            expect(new Date(student.tokens[0].expires_at) > new Date()).toBe(true);
          }
        }
      }
  ...
  describe('Phase 3: Bulk Photo Upload & Processing', () => {
    ...
    it('3.1 Bulk upload photos to event level should work', async () => {
      ...
      for (let i = 1; i <= photoCount; i++) {
        ...
        formData.append('photo_type', i <= 20 ? 'individual' : i <= 35 ? 'group' : 'activity');
      ...
      for (let i = 0; i < photoUploads.length; i += batchSize) {
        ...
        console.log(`✅ Uploaded batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(photoUploads.length/batchSize)}`);
      }
    ...
    it('3.2 QR code detection for secondary school photos should work', async () => {
      ...
      const response = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/photos/qr-detect`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${adminSession.access_token}`
  ...
  describe('Phase 4: Photo Classification Workflow', () => {
    ...
    it('4.1 Classify photos to courses (group photos) should work', async () => {
      ...
      for (const courseName of Object.keys(testCourseIds)) {
        ...
        const response = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/photos/classify?action=to-course`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${adminSession.access_token}`
        ...
        console.log(`✅ Classified ${coursePhotos.length} group photos to ${courseName}`);
      }
    });
    ...
    it('4.2 Classify photos to students (individual photos) should work', async () => {
      ...
      for (let i = 0; i < individualPhotos.length; i++) {
        const response = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/photos/classify?action=to-student`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${adminSession.access_token}`
    ...
    it('4.3 Verify classification statistics are accurate', async () => {
      ...
      console.log(`   Photos: ${stats.photos.total}, Courses: ${stats.courses.total}, Students: ${stats.students.total}`);
    });
  });
  ...
  describe('Phase 5: Family Access & Token Distribution', () => {
    ...
    it('5.2 Family gallery access with token should work for all levels', async () => {
      ...
      for (const student of sampleStudents) {
        ...
        const response = await fetch(`${TEST_CONFIG.baseUrl}/api/family/gallery/${token}`);
        expect(response.status).toBe(200);
        ...
        console.log(`✅ ${levelName} student "${student.name}" can access gallery (${result.photos.length} photos)`);
      }
    });
    ...
    it('5.3 Course-level access should include group photos', async () => {
      ...
      if (courseWithPhotos && courseWithPhotos.photo_courses.length > 0) {
        ...
        const response = await fetch(`${TEST_CONFIG.baseUrl}/api/family/course/${courseId}`);
        ...
        console.log(`✅ Course gallery includes ${groupPhotos.length} group photos`);
      }
    });
  });
  ...
  describe('Phase 6: Purchase Flow & Product System', () => {
    ...
    it('6.1 Wizard-based product selection should work', async () => {
      ...
      if (photos.length > 0) {
        ...
        const checkoutResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/family/checkout`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        console.log(`✅ Wizard checkout (Option 1) created for student: ${testStudent.name}`);
      }
    });

    it('6.2 Physical product options should be available', async () => {
      // Test that physical products are properly configured
      const productResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/products/physical`);
      ...
      if (productResponse.status === 200) {
        ...
        if (products.length > 0) {
          ...
          console.log(`✅ Physical products available: ${products.length} options`);
        }
      } else {
        console.log('ℹ️ Physical products endpoint not available (expected in Phase 3)');
      }
    });
  ...
  describe('Phase 7: Performance & Scale Testing', () => {
    ...
    it('7.1 Event with 500+ students should load efficiently', async () => {
      ...
      const response = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/events/${testEventId}/statistics`, {
        headers: {
          'Authorization': `Bearer ${adminSession.access_token}`
        }
      });
      ...
      console.log(`✅ Large event statistics loaded in ${loadTime}ms (${stats.students.total} students)`);
    });

    it('7.2 Gallery pagination should handle large photo sets efficiently', async () => {
      // Test pagination with large photo sets
      const response = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/photos?event_id=${testEventId}&page=1&limit=20`, {
        headers: {
          'Authorization': `Bearer ${adminSession.access_token}`
        }
      });
      ...
      console.log(`✅ Photo pagination working (${result.pagination.total} total photos)`);
    });
    ...
    it('7.3 Course navigation with 500+ students should be efficient', async () => {
      ...
      const courseResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/courses?event_id=${testEventId}`, {
        headers: {
          'Authorization': `Bearer ${adminSession.access_token}`
        }
      });
      ...
      const totalStudents = courses.reduce((sum: number, course: any) => sum + course.student_count, 0);
      expect(totalStudents).toBeGreaterThan(100);
      
      console.log(`✅ Course navigation efficient: ${courses.length} courses, ${totalStudents} total students`);
    });
  });
  ...
  describe('Phase 8: Integration & Data Integrity', () => {
    ...
    it('8.1 Complete workflow data integrity should be maintained', async () => {
      ...
      const unprocessedPhotos = integrityCheck.photos.filter(p => p.processing_status !== 'completed');
    ...
    it('8.2 Order processing integrity should be maintained', async () => {
      if (testOrderIds.length > 0) {
        ...
        console.log(`✅ Order processing integrity verified for ${orders.length} orders`);
      }
    });

    it('8.3 Security and access control should be properly enforced', async () => {
      // Test invalid token access
      const invalidTokenResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/family/gallery/invalid-token-comprehensive`);
      expect(invalidTokenResponse.status).toBe(401);
      
      // Test expired token handling (simulate)
      const expiredTokenResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/family/gallery/expired-token-test`);
      expect(expiredTokenResponse.status).toBe(401);
      
      // Test admin endpoint without authentication
      const unauthenticatedResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/events`);
  ...
  describe('Phase 9: Mobile Responsiveness & Usability', () => {
    ...
    it('9.1 Mobile-optimized family gallery should work', async () => {
      ...
      if (mobileStudent) {
        ...
        const mobileResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/family/gallery/${token}`, {
          headers: {
  ...
  describe('Phase 10: Error Handling & Edge Cases', () => {
    ...
    it('10.1 Invalid event access should be handled gracefully', async () => {
      const invalidEventResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/family/event/invalid-event-id`);
    ...
    it('10.2 Malformed photo classification should be rejected', async () => {
      const malformedResponse = await fetch(`${TEST_CONFIG.baseUrl}/api/admin/photos/classify?action=to-student`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${adminSession.access_token}`
    ...
    it('10.3 Rate limiting should protect against abuse', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        rapidRequests.push(
          fetch(`${TEST_CONFIG.baseUrl}/api/family/gallery/test-rate-limit-${i}`)
        );
      }
      ...
      console.log(`✅ Rate limiting active (${rateLimitedRequests.length}/10 requests rate limited)`);
    });
  });
});
...
async function cleanupTestData() {
  if (testEventId) {
    try {
      ...
      await supabase.from('photo_courses').delete().in('photo_id', testPhotoIds);
      await supabase.from('student_tokens').delete().in('student_id', testStudentIds);
      await supabase.from('photos').delete().in('id', testPhotoIds);
      await supabase.from('students').delete().in('id', testStudentIds);
      await supabase.from('courses').delete().in('id', Object.values(testCourseIds));
      await supabase.from('levels').delete().in('id', Object.values(testLevelIds));
