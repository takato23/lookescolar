...
async function navigateToEventLibrary(page: Page, eventId: string) {
  ...
  await page.waitForSelector('[data-testid="event-library-main"]', { timeout: 10000 });
}

async function waitForGridToLoad(page: Page) {
  await page.waitForSelector('[data-testid="content-grid"]', { timeout: 10000 });
  // Wait for loading skeleton to disappear
  await page.waitForSelector('[data-testid="skeleton-grid"]', { state: 'hidden', timeout: 5000 });
}
...
test.describe('Event Photo Library - Core Workflows', () => {
  ...
  test.describe('Folder Management Workflow', () => {
    test('should create, rename, and delete folders', async ({ page }) => {
      ...
      await test.step('Create new folder', async () => {
        ...
        await expect(page.locator('[data-testid="folder-tree"]')).toContainText('Test Folder');
        await expect(page.locator('[data-testid="content-grid"]')).toContainText('Test Folder');
      });

      // Test folder rename
      await test.step('Rename folder', async () => {
        await page.click('[data-testid="folder-item-Test Folder"] [data-testid="folder-options"]');
        ...
        await expect(page.locator('[data-testid="folder-tree"]')).toContainText('Renamed Folder');
        await expect(page.locator('[data-testid="content-grid"]')).toContainText('Renamed Folder');
      });
      ...
      await test.step('Create subfolder', async () => {
        ...
        await expect(page.locator('[data-testid="folder-tree"] [data-testid="folder-item-Subfolder"]')).toBeVisible();
      });

      // Test folder deletion
      await test.step('Delete folder', async () => {
        await page.click('[data-testid="folder-item-Subfolder"] [data-testid="folder-options"]');
        await page.click('[data-testid="delete-folder"]');
        await page.click('[data-testid="confirm-delete"]');
        
        // Verify folder removed
        await expect(page.locator('[data-testid="folder-tree"] [data-testid="folder-item-Subfolder"]')).toHaveCount(0);
      });
    });

    test('should handle folder navigation and breadcrumbs', async ({ page }) => {
      await navigateToEventLibrary(page, testEventId);
      await waitForGridToLoad(page);
      ...
      await test.step('Navigate into folder', async () => {
        ...
        await expect(page.locator('[data-testid="breadcrumb"]')).toContainText('Test Folder');
      ...
      await test.step('Navigate back via breadcrumb', async () => {
        ...
        await expect(page.locator('[data-testid="breadcrumb"]')).not.toContainText('Test Folder');
      });
    });
  });
  ...
  test.describe('Photo Upload and Management Workflow', () => {
    ...
    test('should move photos between folders using drag and drop', async ({ page }) => {
      await navigateToEventLibrary(page, testEventId);
      await waitForGridToLoad(page);
      ...
      await test.step('Drag photo to folder', async () => {
        ...
        const folder = page.locator('[data-testid="folder-item-Test Folder"]').first();
  ...
  test.describe('Photo Viewing and Modal Workflow', () => {
    test('should open photos in lightbox with navigation', async ({ page }) => {
      ...
      await test.step('Open photo in lightbox', async () => {
        ...
        await expect(page.locator('[data-testid="photo-modal-image"]')).toBeVisible();
      });
      ...
      await test.step('Select photo in modal', async () => {
        ...
        await expect(page.locator('[data-testid="photo-selected-indicator"]')).toBeVisible();
      });
      ...
      await test.step('Close modal', async () => {
        ...
        // Verify modal closed
        await expect(page.locator('[data-testid="photo-modal"]')).toHaveCount(0);
      });
    });
  });

  test.describe('Keyboard Shortcuts Workflow', () => {
    test('should support keyboard shortcuts for efficiency', async ({ page }) => {
      await navigateToEventLibrary(page, testEventId);
      await waitForGridToLoad(page);
      ...
      await test.step('Select all with Ctrl+A', async () => {
        ...
        const itemCount = await page.locator('[data-testid="grid-item"]').count();
        await expect(page.locator('[data-testid="selection-count"]')).toContainText(`${itemCount} selected`);
      });
  ...
  test.describe('Share Generation Workflow', () => {
    test('should generate share links for photos and folders', async ({ page }) => {
      await navigateToEventLibrary(page, testEventId);
      await waitForGridToLoad(page);
      ...
      await test.step('Configure share settings', async () => {
        ...
        await expect(page.locator('[data-testid="share-link-text"]')).toContainText('https://');
      });
  ...
  test.describe('Performance and Virtualization', () => {
    test('should handle large numbers of photos efficiently', async ({ page }) => {
      await navigateToEventLibrary(page, testEventId);
      ...
      await test.step('Load large photo collection', async () => {
        ...
        await page.goto(`/admin/events/${testEventId}/library?folderId=large-folder`);
        await waitForGridToLoad(page);
        
        // Verify virtualization is working (only visible items rendered)
        const visiblePhotos = await page.locator('[data-testid="photo-item"]:visible').count();
        const totalPhotos = await page.locator('[data-testid="total-count"]').textContent();
  ...
  test.describe('Error Handling and Edge Cases', () => {
    test('should handle network errors gracefully', async ({ page }) => {
      ...
      await test.step('Handle network error during folder creation', async () => {
        ...
        await expect(page.locator('[data-testid="error-message"]')).toContainText('Failed to create folder');
      });
    });
  ...
  test.describe('Mobile Responsiveness', () => {
    test('should work on mobile devices', async ({ page, isMobile }) => {
      ...
      await test.step('Mobile layout', async () => {
        ...
        await expect(page.locator('[data-testid="folder-tree"]')).toHaveCSS('position', 'absolute');
