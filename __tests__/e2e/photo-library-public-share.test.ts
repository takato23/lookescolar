...
async function generateShareLink(page: Page, eventId: string): Promise<string> {
  ...
  await page.waitForSelector('[data-testid="content-grid"]', { timeout: 10000 });
  
  // Select some photos
  await page.click('[data-testid="photo-item"]');
  await page.click('[data-testid="photo-item"]:nth-child(2)', { modifiers: ['ControlOrMeta'] });
  ...
  const shareLink = await page.locator('[data-testid="share-link-text"]').textContent();
  return shareLink || '';
}
...
test.describe('Event Photo Library - Public Sharing Workflow', () => {
  ...
  test.describe('Share Link Generation (Admin)', () => {
    ...
    test('should create share with download permissions', async ({ page }) => {
      ...
      await test.step('Configure download permissions', async () => {
        ...
        await expect(page.locator('[data-testid="share-config-summary"]')).toContainText('Downloads: Enabled');
        await expect(page.locator('[data-testid="share-config-summary"]')).toContainText('Comments: Enabled');
        await expect(page.locator('[data-testid="share-config-summary"]')).toContainText('Expires: 7 days');
      });
    });
    ...
    test('should manage existing shares', async ({ page }) => {
      ...
      await test.step('View share management', async () => {
        // Verify shares list
        await expect(page.locator('[data-testid="shares-table"]')).toBeVisible();
        await expect(page.locator('[data-testid="share-row"]')).toHaveCountGreaterThan(0);
      });
      ...
      await test.step('Revoke share', async () => {
        ...
        await expect(page.locator('[data-testid="share-status"]')).toContainText('Revoked');
      });
      ...
      await test.step('View share analytics', async () => {
        ...
        await expect(page.locator('[data-testid="share-downloads"]')).toBeVisible();
  ...
  test.describe('Public Share Access', () => {
    test.beforeEach(async ({ page }) => {
      ...
      shareLink = '/share/1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef';
    });
    ...
    test('should access share with correct password', async ({ page }) => {
      await test.step('Access share page', async () => {
        ...
        await expect(page.locator('[data-testid="share-password-form"]')).toBeVisible();
      ...
      await test.step('Enter correct password', async () => {
        ...
        await expect(page.locator('[data-testid="share-gallery"]')).toBeVisible();
        await expect(page.locator('[data-testid="share-photo"]')).toHaveCountGreaterThan(0);
      });
    });
    ...
    test('should reject incorrect password', async ({ page }) => {
      ...
      await test.step('Enter wrong password', async () => {
        await page.fill('[data-testid="share-password-input"]', 'wrongpassword');
        await page.click('[data-testid="share-access-button"]');
    ...
    test('should handle expired shares', async ({ page }) => {
      const expiredShareLink = '/share/expired1234567890abcdef1234567890abcdef1234567890abcdef1234567890';
      
      await test.step('Access expired share', async () => {
        await page.goto(expiredShareLink);
        
        // Verify expiry message
        await expect(page.locator('[data-testid="share-expired"]')).toBeVisible();
        await expect(page.locator('[data-testid="share-expired"]')).toContainText('This share has expired');
      });
    });

    test('should handle revoked shares', async ({ page }) => {
      const revokedShareLink = '/share/revoked1234567890abcdef1234567890abcdef1234567890abcdef1234567890';
      
      await test.step('Access revoked share', async () => {
        await page.goto(revokedShareLink);
        
        // Verify revocation message
        await expect(page.locator('[data-testid="share-revoked"]')).toBeVisible();
        await expect(page.locator('[data-testid="share-revoked"]')).toContainText('This share is no longer available');
      });
    });
  });
  ...
  test.describe('Public Gallery Functionality', () => {
    ...
    test('should support photo downloads when enabled', async ({ page }) => {
      ...
      await test.step('Download multiple photos', async () => {
        ...
        await page.click('[data-testid="share-photo"]', { modifiers: ['ControlOrMeta'] });
        await page.click('[data-testid="share-photo"]:nth-child(2)', { modifiers: ['ControlOrMeta'] });
    ...
    test('should support comments when enabled', async ({ page }) => {
      await test.step('Add comment to photo', async () => {
        ...
        await expect(page.locator('[data-testid="comment"]')).toContainText('Great photo!');
        await expect(page.locator('[data-testid="comment"]')).toContainText('John Doe');
      });
      ...
      await test.step('View existing comments', async () => {
        ...
        await expect(page.locator('[data-testid="comment-count"]')).toContainText('1 comment');
      });
    });
    ...
    test('should handle watermarked previews', async ({ page }) => {
      await test.step('Verify watermarked images', async () => {
        ...
        const photoSrc = await page.locator('[data-testid="share-photo"] img').first().getAttribute('src');
  ...
  test.describe('Share Security and Rate Limiting', () => {
    ...
    test('should respect share view limits', async ({ page }) => {
      const limitedShareLink = '/share/limited1234567890abcdef1234567890abcdef1234567890abcdef1234567890';
      ...
      await test.step('Access share at view limit', async () => {
        ...
        await expect(page.locator('[data-testid="view-limit-warning"]')).toBeVisible();
        await expect(page.locator('[data-testid="remaining-views"]')).toContainText('1 view remaining');
      });
    });
    ...
    test('should prevent unauthorized access attempts', async ({ page }) => {
      ...
      await test.step('Access invalid share', async () => {
        ...
        await expect(page.locator('[data-testid="share-not-found"]')).toBeVisible();
        await expect(page.locator('[data-testid="share-not-found"]')).toContainText('Share not found');
      });
    });
  });
  ...
  test.describe('Mobile Share Experience', () => {
    test('should work on mobile devices', async ({ page, isMobile }) => {
      ...
      await test.step('Mobile download experience', async () => {
        ...
        await expect(page.locator('[data-testid="mobile-download-info"]')).toBeVisible();
      });
    });
  });
});