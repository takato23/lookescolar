...
describe('Wizard-Based Family Workflow', () => {
  ...
  describe('Family Gallery Access', () => {
    it('1.1 Direct token access should work (no manual entry)', async () => {
      const response = await fetch(
        `${TEST_CONFIG.baseUrl}/f/${testStudentToken}`
      );
      ...
      console.log(
        '✅ Direct token access works (family receives link, clicks, no manual token entry)'
      );
    });

    it('1.2 Gallery API should return student photos and group photos', async () => {
      const response = await fetch(
        `${TEST_CONFIG.baseUrl}/api/family/gallery/${testStudentToken}`
      );
      ...
      const individualPhotos = result.photos.filter(
        (p: any) => p.photo_type === 'individual'
      );
      ...
      console.log(
        `✅ Gallery API returns ${result.photos.length} photos (${individualPhotos.length} individual, ${groupPhotos.length} group)`
      );
    });

    it('1.3 Wizard page should load with proper options', async () => {
      const response = await fetch(
        `${TEST_CONFIG.baseUrl}/f/${testStudentToken}/wizard`
      );
  ...
  describe('Option 1 Workflow (1 Digital Photo)', () => {
    it('2.1 Option 1 selection should validate photo requirements', async () => {
      ...
      const response = await fetch(
        `${TEST_CONFIG.baseUrl}/api/family/wizard/validate`,
        {
          method: 'POST',
  ...
  describe('Option 2 Workflow (4 Digital Photos)', () => {
    ...
    it('3.2 Option 2 should allow photo repetition', async () => {
      ...
      console.log(
        `   Selected photos: [${selectedPhotos.map((id) => testPhotoIds.indexOf(id)).join(', ')}]`
      );
    });
    ...
    it('3.3 Option 2 order structure should handle repeated photos', async () => {
      ...
      console.log(
        `   Order items: ${order.order_items.length}, Total quantity: ${totalQuantity}`
      );
    });
  });
  ...
  describe('Upsell System', () => {
    it('4.1 Upsells should be available for selection', async () => {
      const response = await fetch(
        `${TEST_CONFIG.baseUrl}/api/family/upsells?token=${testStudentToken}`
      );
    ...
    it('4.2 Order with upsells should calculate total correctly', async () => {
      ...
      if (response.status === 201) {
        ...
        if (result.total_amount > 2000) {
          ...
          console.log(
            'ℹ️ Upsells not affecting price (may not be implemented)'
          );
        }
      }
    });
  ...
  describe('Pricing System Validation', () => {
    it('5.1 Price calculation should be accurate', async () => {
      ...
      console.log(
        `   Option 1: $${mockOption1.price} (${mockOption1.photos} photo)`
      );
      console.log(
        `   Option 2: $${mockOption2.price} (${mockOption2.photos} photos)`
      );
    });
  ...
  describe('Complete Purchase Flow', () => {
    ...
    it('6.3 Family can check order status', async () => {
      ...
      const response = await fetch(
        `${TEST_CONFIG.baseUrl}/api/family/order/status`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  describe('User Experience & Edge Cases', () => {
    ...
    it('7.3 Expired token should be handled gracefully', async () => {
      ...
      const response = await fetch(
        `${TEST_CONFIG.baseUrl}/api/family/gallery/expired-token-test-wizard`
      );
...
async function setupTestData() {
  ...
  const studentResponse = await fetch(
    `${TEST_CONFIG.baseUrl}/api/admin/students`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const photoResponse = await fetch(
      `${TEST_CONFIG.baseUrl}/api/admin/photos/upload`,
      {
        method: 'POST',
