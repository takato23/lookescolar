...
test.describe('Group Photo Complete Workflow', () => {
  ...
  test('Admin can assign photos to course as group photos', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator(`[data-testid="group-photo-${testPhotoId}"]`)
    ).toBeVisible();
  });
  ...
  test('Family can view group photos in gallery', async ({ page }) => {
    ...
    await expect(
      page.locator(
        `[data-testid="group-photo-${testPhotoId}"] [data-testid="photo-type"]`
      )
  ...
  test('Family can add group photos to cart', async ({ page }) => {
    ...
    // Add photo to cart
    await page.click(
      `[data-testid="group-photo-${testPhotoId}"] [data-testid="add-to-cart"]`
    );
  ...
  test('Family can view group photo in lightbox', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="photo-lightbox"] h2')
    ).toContainText('e2e-test-photo.jpg');
    await expect(
      page.locator(
        '[data-testid="photo-lightbox"] [data-testid="photo-description"]'
      )
    ).toContainText('Group Photos • E2E Test Course 1A');

    // Verify lightbox actions
    await expect(
      page.locator('[data-testid="lightbox-add-to-cart"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="lightbox-download"]')
    ).toBeVisible();
    await expect(page.locator('[data-testid="lightbox-share"]')).toBeVisible();
  });

  test('Admin can perform bulk operations on group photos', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator(
        `[data-testid="photo-${testPhotoId}"] [data-testid="approval-badge"]`
      )
    ).toContainText('Approved');
    await expect(
      page.locator(
        `[data-testid="photo-${photo2.id}"] [data-testid="approval-badge"]`
      )
    ).toContainText('Approved');
  ...
  test('Family can filter group photos by type', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="photo-count"]')).toContainText(
      '2 photo(s)'
    );
    ...
    await expect(
      page.locator(`[data-testid="group-photo-${activityPhoto.id}"]`)
    ).not.toBeVisible();
  ...
  test('System handles group photo permissions correctly', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="no-photos-message"]')
    ).toBeVisible();
