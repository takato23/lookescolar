...
import { setupE2EDatabase, cleanupE2EDatabase, createTestEvent, createTestPhotos, createTestCodes } from '../test-utils';
...
async function measureApiPerformance(page: Page, endpoint: string): Promise<number> {
...
test.describe('Admin Publish Workflow - Desktop', () => {
  ...
  test('complete admin workflow: login → publish → family access → unpublish', async () => {
    ...
    const unpublishedCount = await page.locator('[data-testid="stats-unpublished"]').textContent();
    ...
    const codeValue = await firstCard.locator('[data-testid="code-value"]').textContent();
    
    const publishStart = performance.now();
    await firstCard.locator('[data-testid="publish-button"]').click();
    
    // Wait for optimistic update
    await expect(firstCard.locator('[data-testid="status-badge"]')).toContainText('Publicado');
    
    // Measure API response time
    await waitForReactQuery(page);
    const publishTime = performance.now() - publishStart;
    expect(publishTime).toBeLessThan(PERFORMANCE_THRESHOLDS.API_RESPONSE_TIME * 2); // Allow for UI update

    // Verify toast notification
    await expect(page.locator('[data-testid="success-toast"]')).toBeVisible();
    await expect(page.locator('[data-testid="success-toast"]')).toContainText(codeValue || '');
    ...
    await expect(page.locator('[data-testid="rotate-success-toast"]')).toBeVisible();
    ...
    const publishedCards = page.locator('[data-testid="folder-card"][data-published="true"]');
    ...
    const finalUnpublishedCount = await page.locator('[data-testid="stats-unpublished"]').textContent();
  ...
  test('validates performance requirements (<200ms API)', async () => {
    ...
    const listApiTime = await measureApiPerformance(page, '/api/admin/publish/list');
    expect(listApiTime).toBeLessThan(PERFORMANCE_THRESHOLDS.API_RESPONSE_TIME);

    const publishApiTime = await measureApiPerformance(page, '/api/admin/publish');
    expect(publishApiTime).toBeLessThan(PERFORMANCE_THRESHOLDS.API_RESPONSE_TIME);
...
test.describe('Admin Publish Workflow - Mobile', () => {
  ...
  test('handles mobile viewport interactions correctly', async () => {
    ...
    await expect(page.locator('[data-testid="mobile-actions-drawer"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="mobile-qr-modal"]')).not.toBeVisible();
  });
  ...
  test('mobile performance Lighthouse score >95', async () => {
    ...
    expect(performanceMetrics.TTI).toBeLessThan(PERFORMANCE_THRESHOLDS.TIME_TO_INTERACTIVE);
  });
});
...
test.describe('Error Handling & Edge Cases', () => {
  ...
  test('handles empty states correctly', async () => {
    ...
    await expect(page.locator('[data-testid="empty-state"]')).toContainText('No hay códigos');
  });
