...
describe('Signed URL Cache System', () => {
  ...
  describe('SessionStorage Persistence', () => {
    ...
    it('debe cargar caché desde sessionStorage al inicializar', () => {
      ...
      expect(getCachedUrl('photo-stored')).toBe(
        'https://storage.supabase.co/stored-url'
      );
    });
    ...
    it('debe ignorar entradas expiradas al cargar desde sessionStorage', () => {
      ...
      expect(getCachedUrl('photo-valid')).toBe(
        'https://storage.supabase.co/valid-url'
      );
    });
  ...
  describe('Batch Preloading', () => {
    it('debe precargar múltiples URLs en batch', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        '/api/admin/storage/batch-signed-urls',
        {
          method: 'POST',
    ...
    it('debe reutilizar URLs ya cacheadas en preload', async () => {
      ...
      expect(results.get('new-photo')).toBe(
        'https://storage.supabase.co/new-url'
      );
    });
  ...
  describe('Security & Privacy', () => {
    it('no debe loggear URLs completas', () => {
      ...
      const sensitiveUrl =
        'https://storage.supabase.co/bucket/secret-token-abc123';
      ...
      expect(
        loggedContent.some((log) => log.includes('secret-token-abc123'))
      ).toBe(false);
