...
describe('Storage Service', () => {
  ...
  describe('uploadToStorage', () => {
    it('debe subir imagen procesada al bucket privado', async () => {
      ...
      expect(result).toMatchObject({
        path: expect.stringMatching(/^eventos\/test-event-123\/previews\/[a-f0-9]{16}\.webp$/),
    ...
    it('debe fallar cuando hay error de upload', async () => {
      ...
      await expect(
        uploadToStorage(processedImage, eventId)
    ...
    it('debe usar estructura de carpetas correcta', async () => {
      ...
      expect(result.path).toMatch(/^eventos\/evento-12345\/previews\/[a-f0-9]{16}\.webp$/)
    })
  })
  ...
  describe('getSignedUrl', () => {
    it('debe generar URL firmada con expiración por defecto', async () => {
      ...
      // Mock respuesta exitosa
      const mockCreateSignedUrl = mockSupabase.storage.from().createSignedUrl as any
    ...
    it('debe fallar cuando hay error generando URL', async () => {
      ...
      await expect(
        getSignedUrl(path)
      ).rejects.toThrow('Error al generar URL firmada: File not found')
    })
  })
  ...
  describe('trackEgressMetrics', () => {
    ...
    it('debe extraer event_id del path correctamente', async () => {
      const testCases = [
        ...
        { path: 'eventos/event-uuid-456/previews/image.webp', expected: 'event-uuid-456' },
  ...
  describe('CLAUDE.md Compliance', () => {
    ...
    it('debe usar estructura de carpetas eventos/{eventId}/previews/', async () => {
      ...
      expect(result.path).toMatch(/^eventos\/mi-evento-123\/previews\/[a-f0-9]{16}\.webp$/)
    })
