...
describe('Comprehensive Performance Test Suite', () => {
  ...
  afterAll(async () => {
    ...
    if (perfContext.metrics.length > 0) {
      const avgResponseTime =
        perfContext.metrics.reduce((sum, m) => sum + m.responseTime, 0) /
        perfContext.metrics.length;
      console.log(`\n📊 Performance Metrics Summary:`);
      console.log(`   Average Response Time: ${avgResponseTime.toFixed(2)}ms`);
      console.log(`   Total Tests: ${perfContext.metrics.length}`);
      console.log(
        `   Fastest: ${Math.min(...perfContext.metrics.map((m) => m.responseTime))}ms`
      );
      console.log(
        `   Slowest: ${Math.max(...perfContext.metrics.map((m) => m.responseTime))}ms`
      );
    }
  });
  ...
  describe('API Response Time Performance', () => {
    test('Admin events API should respond under 200ms', async () => {
      if (!perfContext.adminToken) {
        console.warn(
          'Skipping authenticated performance test - no admin token'
        );
        return;
      }
      ...
      if (result.ok) {
        ...
        console.warn(
          `   Admin events API returned ${result.status}, timing: ${metrics.responseTime.toFixed(2)}ms`
        );
      }
    });
    ...
    test('Family gallery API should respond under 200ms', async () => {
      const { result, metrics } = await measurePerformance(async () => {
        return fetch(
          `${API_BASE_URL}/api/family/gallery/${perfContext.subjectToken}`
        );
      });

      console.log(
        `   Family gallery API: ${metrics.responseTime.toFixed(2)}ms`
      );
      ...
      if (result.ok) {
        ...
        console.warn(
          `   Family gallery API returned ${result.status}, timing: ${metrics.responseTime.toFixed(2)}ms`
        );
      }
    });
    ...
    test('Signed URL generation should be under 100ms', async () => {
      ...
      console.log(
        `   Signed URL generation: ${metrics.responseTime.toFixed(2)}ms`
      );
      ...
      if (result.ok) {
        ...
        console.warn(
          `   Signed URL API returned ${result.status}, timing: ${metrics.responseTime.toFixed(2)}ms`
        );
      }
    });
    ...
    test('Public gallery should load under 2 seconds', async () => {
      ...
      console.log(
        `   Public gallery API: ${metrics.responseTime.toFixed(2)}ms`
      );
      ...
      if (result.ok) {
        ...
        console.warn(
          `   Public gallery API returned ${result.status}, timing: ${metrics.responseTime.toFixed(2)}ms`
        );
      }
    });
    ...
    test('Webhook processing should complete under 3 seconds', async () => {
      ...
      console.log(
        `   Webhook processing: ${metrics.responseTime.toFixed(2)}ms`
      );
  ...
  describe('Photo Processing Performance', () => {
    test('Photo upload and watermark processing should complete under 3 seconds', async () => {
      ...
      const mockImageBlob = new Blob(['x'.repeat(mockImageSize)], {
        type: 'image/jpeg',
      });
      ...
      if (result.ok) {
        ...
        console.warn(
          `   Photo upload returned ${result.status}, timing: ${metrics.responseTime.toFixed(2)}ms`
        );
      }
    });
    ...
    test('Batch photo processing should scale linearly', async () => {
      ...
      for (const photoCount of testSizes) {
        ...
        for (let i = 0; i < photoCount; i++) {
          const mockImageBlob = new Blob(['test image data'], {
            type: 'image/jpeg',
          });
        ...
        console.log(
          `   Batch ${photoCount} photos: ${metrics.responseTime.toFixed(2)}ms`
        );
  ...
  describe('Database Query Performance', () => {
    test('Photo lookup by subject should be under 50ms', async () => {
      ...
      console.log(
        `   Photo lookup query: ${metrics.responseTime.toFixed(2)}ms`
      );
    ...
    test('Event listing should be under 100ms', async () => {
      ...
      console.log(
        `   Event listing query: ${metrics.responseTime.toFixed(2)}ms`
      );
      expect(metrics.responseTime).toBeLessThan(100);
    });
    ...
    test('Subject token lookup should be under 25ms', async () => {
      ...
      console.log(
        `   Token lookup query: ${metrics.responseTime.toFixed(2)}ms`
      );
  ...
  describe('Concurrent Operations Performance', () => {
    test('Concurrent gallery requests should maintain performance', async () => {
      ...
      console.log(
        `     Successful requests: ${results.filter((r) => r.result.ok).length}/${concurrentRequests}`
      );
    ...
    test('Concurrent signed URL generation should handle load', async () => {
      ...
      console.log(`     Average time: ${averageTime.toFixed(2)}ms`);
      console.log(
        `     Successful: ${successfulRequests}/${concurrentRequests}`
      );
      console.log(
        `     Rate limited: ${concurrentRequests - successfulRequests}/${concurrentRequests}`
      );
    ...
    test('Mixed concurrent operations should maintain system stability', async () => {
      ...
      const successfulRequests = results.filter(
        (r) => r.result.ok || r.result.status === 429
      ).length;

      console.log(`   Mixed concurrent operations:`);
      console.log(`     Average time: ${averageTime.toFixed(2)}ms`);
      console.log(
        `     Stable responses: ${successfulRequests}/${operations.length}`
      );
  ...
  describe('Memory and Resource Usage', () => {
    ...
    test('Gallery requests should have consistent memory usage', async () => {
      ...
      console.log(
        `   Memory stability check: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB increase`
      );
  ...
  describe('Scalability Performance', () => {
    test('Response time should scale reasonably with data size', async () => {
      ...
      console.log(
        '   Note: Scalability tests require larger datasets to be meaningful'
      );
      ...
      console.log(
        `   Baseline gallery response: ${baselineTime.metrics.responseTime.toFixed(2)}ms`
      );
    ...
    test('System should handle burst traffic patterns', async () => {
      ...
      console.log(
        `     Recovery time: ${recoveryResult.metrics.responseTime.toFixed(2)}ms`
      );
      console.log(
        `     Rate limited responses: ${burstResults.filter((r) => r.result.status === 429).length}`
      );
