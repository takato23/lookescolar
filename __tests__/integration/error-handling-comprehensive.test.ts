...
describe('Comprehensive Error Handling Tests', () => {
  describe('Authentication & Authorization Errors', () => {
    it('1.1 Invalid login credentials should be rejected', async () => {
      const response = await fetch(
        `${ERROR_TEST_CONFIG.baseUrl}/api/admin/auth`,
        {
          method: 'POST',
    ...
    it('1.2 Missing authentication should block admin endpoints', async () => {
      ...
      console.log(
        `✅ ${protectedEndpoints.length} protected endpoints properly secured`
      );
    });
    ...
    it('1.3 Invalid JWT tokens should be rejected', async () => {
      ...
      for (const token of invalidTokens) {
        const response = await fetch(
          `${ERROR_TEST_CONFIG.baseUrl}/api/admin/events`,
          {
    ...
    it('1.5 Invalid family tokens should block gallery access', async () => {
      ...
      for (const token of invalidTokens) {
        const response = await fetch(
          `${ERROR_TEST_CONFIG.baseUrl}/api/family/gallery/${token}`
        );
        expect(response.status).toBe(401);
  ...
  describe('Input Validation & Sanitization', () => {
    it('2.1 SQL injection attempts should be prevented', async () => {
      ...
      for (const payload of sqlInjectionPayloads) {
        ...
        const response = await fetch(
          `${ERROR_TEST_CONFIG.baseUrl}/api/family/gallery/${payload}`
        );
    ...
    it('2.3 File upload validation should prevent malicious files', async () => {
      ...
      for (const file of maliciousFiles) {
        ...
        const response = await fetch(
          `${ERROR_TEST_CONFIG.baseUrl}/api/admin/photos/upload`,
          {
            method: 'POST',
  ...
  describe('Database Constraint Violations', () => {
    ...
    it('3.2 Foreign key constraint violations should be prevented', async () => {
      ...
      const { error: fkError } = await supabase.from('students').insert({
    ...
    it('3.3 NULL constraint violations should be caught', async () => {
      ...
      const { error: nullError } = await supabase.from('events').insert({
  ...
  describe('Rate Limiting & Abuse Prevention', () => {
    it('4.1 Rapid API requests should be rate limited', async () => {
      ...
      for (let i = 0; i < requestCount; i++) {
        rapidRequests.push(
          fetch(
            `${ERROR_TEST_CONFIG.baseUrl}/api/family/gallery/rate-limit-test-${i}`
          )
        );
      }
    ...
    it('4.2 File upload rate limiting should prevent abuse', async () => {
      ...
      const rateLimitedUploads = uploadResponses.filter(
        (r) => r.status === 429
      ).length;
      const successfulUploads = uploadResponses.filter(
        (r) => r.status === 201
      ).length;

      console.log(
        `✅ Upload rate limiting: ${successfulUploads} successful, ${rateLimitedUploads} rate limited`
      );
    });
    ...
    it('4.3 Login brute force should be prevented', async () => {
      ...
      const unauthorizedCount = loginResponses.filter(
        (r) => r.status === 401
      ).length;
      const rateLimitedCount = loginResponses.filter(
        (r) => r.status === 429
      ).length;
      ...
      console.log(
        `✅ Brute force protection: ${unauthorizedCount} rejected, ${rateLimitedCount} rate limited`
      );
    });
  });
  ...
  describe('Data Corruption & Recovery', () => {
    it('5.1 Malformed JSON should be rejected gracefully', async () => {
      ...
      for (const malformedJson of malformedJsonCases.slice(0, 4)) {
        // Skip valid JSON
        const response = await fetch(
          `${ERROR_TEST_CONFIG.baseUrl}/api/admin/events`,
          {
  ...
  describe('Network & System Failures', () => {
    it('6.1 Database connection errors should be handled', async () => {
      ...
      console.log(
        'ℹ️ Database connection error handling would be tested in staging environment'
      );
    });
    ...
    it('6.2 External service failures should not break core functionality', async () => {
      ...
      console.log(
        'ℹ️ External service failure handling validated through integration tests'
      );
    });
  ...
  describe('Edge Case Scenarios', () => {
    it('7.1 Concurrent operations should not cause conflicts', async () => {
      ...
      console.log(
        `✅ Concurrent operations handled: ${successCount}/5 successful`
      );
    });
