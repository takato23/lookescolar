...
describe('QR Code System Integration Tests', () => {
  describe('QR Code Generation & Management', () => {
    ...
    it('1.3 QR code regeneration should handle duplicates', async () => {
      ...
      const { data: regenerationResult } = await supabase.rpc(
        'batch_generate_student_qr_codes',
        {
      ...
      if (regenerationResult) {
        ...
        const duplicateCount = regenerationResult.filter(
    ...
    it('1.4 QR codes should be searchable and indexable', async () => {
      const randomStudent =
        testStudentsWithQR[
          Math.floor(Math.random() * testStudentsWithQR.length)
        ];
      ...
      console.log(
        `   Prefix "${qrPrefix}" matches: ${prefixMatches.length} students`
      );
    });
  });
  ...
  describe('Photo QR Detection Workflow', () => {
    ...
    it('2.2 QR detection API should validate input parameters', async () => {
      const response = await fetch(
        `${QR_TEST_CONFIG.baseUrl}/api/admin/photos/qr-detect`,
        {
          method: 'POST',
          headers: {
      ...
      expect(result.error).toContain(
        'Either photo_ids or event_id must be provided'
      );
    ...
    it('2.3 Event-wide QR detection should process all photos', async () => {
      ...
      console.log(
        `   Total: ${result.summary.total}, Successful: ${result.summary.successful}, Matched: ${result.summary.matched}`
      );
    });
    ...
    it('2.5 QR matching should associate photos with students', async () => {
      ...
      const matchResponse = await fetch(
        `${QR_TEST_CONFIG.baseUrl}/api/admin/photos/qr-match`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  describe('Secondary School Workflow Integration', () => {
    it('3.1 Secondary students should have QR-based workflow enabled', async () => {
      ...
      console.log(
        `   ${secondaryStudents.length} secondary students with unique QR codes`
      );
    });
    ...
    it('3.2 QR-based photo classification should work efficiently', async () => {
      ...
      const batchResponse = await fetch(
        `${QR_TEST_CONFIG.baseUrl}/api/admin/photos/classify-qr-batch`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (batchResponse.status === 200) {
        ...
        console.log(
          'ℹ️ QR-based batch classification endpoint not implemented yet'
        );
      }
    });
    ...
    it('3.3 Manual fallback should work when QR detection fails', async () => {
      ...
      const manualResponse = await fetch(
        `${QR_TEST_CONFIG.baseUrl}/api/admin/photos/classify?action=to-student`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      console.log(
        `   Photo manually classified to student: ${testStudent.name}`
      );
    });
    ...
    it('3.4 Mixed QR and manual workflow should maintain data integrity', async () => {
      ...
      if (allClassifications.length > 0) {
        ...
        const qrClassifications = allClassifications.filter(
          (c) => c.classification_method === 'qr_auto'
        );
        const manualClassifications = allClassifications.filter(
          (c) => c.classification_method === 'manual'
        );

        // Verify no duplicate photo-student associations
        const associations = allClassifications.map(
          (c) => `${c.photo_id}-${c.student_id}`
        );
        ...
        console.log(
          `   QR-based: ${qrClassifications.length}, Manual: ${manualClassifications.length}`
        );
      }
    });
  });
  ...
  describe('QR Code Performance & Scalability', () => {
    it('4.1 QR code lookup should be performant at scale', async () => {
      ...
      const avgLookupTime =
        lookupTimes.reduce((sum, time) => sum + time, 0) / lookupTimes.length;
      ...
      console.log(
        `   ${iterations} lookups - Avg: ${avgLookupTime.toFixed(1)}ms, Max: ${maxLookupTime}ms`
      );
    });
    ...
    it('4.2 Bulk QR operations should handle large datasets', async () => {
      ...
      if (regenerationResult) {
        ...
        console.log(
          `   Validation time: ${validationTime}ms for ${allQRCodes.length} QR codes`
        );
        console.log(`   Regeneration time: ${regenerationTime}ms`);
      } else {
        console.log(
          `✅ QR validation query efficient: ${validationTime}ms for ${allQRCodes.length} codes`
        );
      }
    });
    ...
    it('4.3 QR detection should scale with photo volume', async () => {
      ...
      if (performanceResults.length > 0) {
        ...
        performanceResults.forEach((r) => {
          console.log(
            `   Batch ${r.batchSize}: ${r.processingTime}ms (${r.avgTimePerPhoto.toFixed(0)}ms/photo)`
          );
        });
      } else {
        console.log(
          'ℹ️ QR detection performance test skipped (endpoint not available)'
        );
      }
    });
  });
  ...
  describe('QR Code Error Handling & Edge Cases', () => {
    ...
    it('5.2 Duplicate QR detection should prevent conflicts', async () => {
      ...
      const { error: duplicateError } = await supabase.from('students').insert({
        event_id: testEventId,
      ...
      console.log(
        '✅ Duplicate QR codes are prevented by database constraints'
      );
    });
    ...
    it('5.3 QR code format validation should work', async () => {
      ...
      for (const invalidFormat of invalidFormats) {
        ...
        const validationResponse = await fetch(
          `${QR_TEST_CONFIG.baseUrl}/api/admin/qr/validate`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
