...
describe('Hierarchical Structure Integration Tests', () => {
  ...
  describe('Basic Hierarchy Creation', () => {
    ...
    it('1.3 Courses should be distributed across levels', async () => {
      ...
      const coursesByLevel = coursesWithLevels.reduce((acc: any, course: any) => {
      ...
      console.log(`   Secundaria: ${coursesByLevel['Secundaria'].length} courses`);
    });
    ...
    it('1.4 Students should be properly distributed across courses', async () => {
      ...
      const studentsByLevel = studentsWithHierarchy.reduce((acc: any, student: any) => {
  ...
  describe('Hierarchy Navigation & Queries', () => {
    ...
    it('2.1 Top-down navigation should work efficiently', async () => {
      ...
      console.log(`   Levels: ${fullHierarchy.levels.length}, Courses: ${totalCourses}, Students: ${totalStudents}`);
    });
    ...
    it('2.2 Bottom-up navigation should work efficiently', async () => {
      ...
      const randomStudentId = testStudentIds[Math.floor(Math.random() * testStudentIds.length)];
      ...
      expect(studentHierarchy.courses.levels.events.name).toBe(HIERARCHY_TEST_DATA.event.name);
      ...
      console.log(`   Student: ${studentHierarchy.name} → Course: ${studentHierarchy.courses.name} → Level: ${studentHierarchy.courses.levels.name} → Event: ${studentHierarchy.courses.levels.events.name}`);
    });
  ...
  describe('Data Integrity & Constraints', () => {
    ...
    it('3.2 Event consistency should be maintained across hierarchy', async () => {
      ...
      const consistencyChecks = await Promise.all([
        ...
        supabase.from('students').select('event_id').eq('event_id', testEventId)
      ]);
      ...
      expect(students.data?.every(s => s.event_id === testEventId)).toBe(true);
      
      console.log('✅ Event consistency maintained across hierarchy');
      console.log(`   Levels: ${levels.data?.length}, Courses: ${courses.data?.length}, Students: ${students.data?.length}`);
    });
  ...
  describe('Performance & Scalability', () => {
    ...
    it('4.2 Pagination should work efficiently across hierarchy', async () => {
      ...
      console.log(`   Total students: ${count}, Page size: ${pageSize}, Total pages: ${totalPages}`);
    });
    ...
    it('4.3 Index usage should optimize common queries', async () => {
      ...
      console.log(`   Event query: ${time1}ms, Level query: ${time2}ms, Course query: ${time3}ms`);
    });
  });
  ...
  describe('Optional Level Handling', () => {
    ...
    it('5.1 Events without levels should work (courses directly under event)', async () => {
      ...
      console.log('✅ Optional levels work correctly (courses can be directly under events)');
    });
    ...
    it('5.2 Mixed hierarchy should handle both leveled and direct courses', async () => {
      ...
      console.log(`   Leveled courses: ${leveledCourses.length}, Direct courses: ${directCourses.length}`);
    });
  });
});
