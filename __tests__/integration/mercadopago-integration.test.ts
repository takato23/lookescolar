...
describe('Mercado Pago Integration Tests', () => {
  ...
  describe('Payment Preference Creation', () => {
    ...
    it('1.1 Basic preference creation should work', async () => {
      const response = await fetch(`${MP_TEST_CONFIG.baseUrl}/api/family/checkout`, {
        method: 'POST',
  ...
  describe('Webhook Processing', () => {
    ...
    it('2.1 Successful payment webhook should update order status', async () => {
      const orderId = testOrderIds[0];
      const scenario = MP_TEST_CONFIG.paymentScenarios.find(s => s.name === 'successful_payment')!;
      ...
      const webhookResponse = await fetch(`${MP_TEST_CONFIG.baseUrl}/api/payments/webhook`, {
        method: 'POST',
        headers: {
    ...
    it('2.2 Rejected payment webhook should update order accordingly', async () => {
      ...
      const scenario = MP_TEST_CONFIG.paymentScenarios.find(s => s.name === 'rejected_payment')!;
    ...
    it('2.4 Invalid webhook signatures should be rejected', async () => {
      ...
      const response = await fetch(`${MP_TEST_CONFIG.baseUrl}/api/payments/webhook`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  describe('Payment Status Synchronization', () => {
    ...
    it('3.1 Order status should reflect payment status accurately', async () => {
      ...
      for (const scenario of MP_TEST_CONFIG.paymentScenarios) {
        ...
        console.log(`✅ ${scenario.name}: ${order.status} (${order.mp_status_detail})`);
      }
    });
    ...
    it('3.2 Family should be able to check payment status', async () => {
      ...
      const response = await fetch(`${MP_TEST_CONFIG.baseUrl}/api/family/order/status`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          token: testStudentToken,
    ...
    it('3.3 Admin should see updated payment information', async () => {
      const response = await fetch(`${MP_TEST_CONFIG.baseUrl}/api/admin/orders`, {
  ...
  describe('Error Handling & Recovery', () => {
    ...
    it('4.2 Malformed webhook payloads should be rejected', async () => {
      ...
      for (const payload of malformedPayloads.slice(0, 3)) { // Skip string for JSON test
        const response = await fetch(`${MP_TEST_CONFIG.baseUrl}/api/payments/webhook`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
    ...
    it('4.3 Payment recovery should handle edge cases', async () => {
      ...
      const recoveryResponse = await fetch(`${MP_TEST_CONFIG.baseUrl}/api/admin/payments/recovery`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  describe('Test Mode & Production Validation', () => {
    ...
    it('5.1 Test mode should be properly configured', async () => {
      ...
      if (order.mp_init_point) {
        ...
        const isTestMode = order.mp_init_point.includes('sandbox') || 
        ...
        console.log(`   Init point: ${order.mp_init_point.substring(0, 50)}...`);
      }
    });
    ...
    it('5.3 Production readiness checks should pass', async () => {
      const readinessChecks = {
        ...
        mpCredentials: process.env.MP_ACCESS_TOKEN && process.env.NEXT_PUBLIC_MP_PUBLIC_KEY
      };
      ...
      console.log(`   MP credentials: ${readinessChecks.mpCredentials ? 'configured' : 'missing'}`);
    });
  });
});
