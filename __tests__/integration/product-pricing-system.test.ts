...
const PRODUCT_TEST_CONFIG = {
  ...
  upsells: [
    ...
    { id: 'size_20x30', name: 'Impresión 20x30', price: 1500, category: 'size' },
    { id: 'copy_extra', name: 'Copia adicional', price: 500, category: 'copy' },
    { id: 'frame_basic', name: 'Marco básico', price: 1200, category: 'frame' },
    { id: 'album_small', name: 'Álbum pequeño', price: 2500, category: 'album' }
  ],
...
describe('Product & Pricing System Tests', () => {
  ...
  describe('Wizard Options Validation', () => {
    ...
    it('1.1 Option 1 (1 photo) should validate correctly', async () => {
      ...
      const validationTests = [
        ...
        { photos: [testPhotoIds[0]], valid: true, message: '1 foto seleccionada' },
        { photos: [testPhotoIds[0], testPhotoIds[1]], valid: false, message: 'Solo puedes seleccionar 1 foto' }
      ];
    ...
    it('1.2 Option 2 (4 photos) should validate correctly', async () => {
      ...
      const validationTests = [
        ...
        { photos: [testPhotoIds[0]], valid: false, message: 'Selecciona 3 fotos más' },
        { photos: testPhotoIds.slice(0, 4), valid: true, message: '4 fotos seleccionadas' },
        { photos: testPhotoIds.slice(0, 5), valid: false, message: 'Solo puedes seleccionar 4 fotos' }
      ];
      ...
      const repeatedPhotos = [testPhotoIds[0], testPhotoIds[0], testPhotoIds[1], testPhotoIds[1]];
      ...
      console.log(`   Photo repetition allowed: ${repeatedPhotos.length} selections`);
    });
    ...
    it('1.3 Price comparison between options should be logical', async () => {
      ...
      const discount = ((option1PricePerPhoto - option2PricePerPhoto) / option1PricePerPhoto) * 100;
      ...
      console.log(`   Option 2 offers ${discount.toFixed(1)}% discount per photo`);
  ...
  describe('Upsell System', () => {
    ...
    it('2.1 Upsells should be properly categorized and priced', async () => {
      ...
      Object.entries(upsellsByCategory).forEach(([category, items]: [string, any]) => {
    ...
    it('2.3 Upsell API should return available options', async () => {
      const response = await fetch(`${PRODUCT_TEST_CONFIG.baseUrl}/api/family/upsells?token=${testStudentToken}`);
  ...
  describe('Product Order Integration', () => {
    ...
    it('4.1 Order with Option 1 should create correct line items', async () => {
      const response = await fetch(`${PRODUCT_TEST_CONFIG.baseUrl}/api/family/checkout`, {
        method: 'POST',
    ...
    it('4.2 Order with Option 2 and upsells should calculate correctly', async () => {
      const selectedPhotos = [testPhotoIds[0], testPhotoIds[1], testPhotoIds[2], testPhotoIds[3]];
      ...
      if (response.status === 201) {
        ...
        const baseResponse = await fetch(`${PRODUCT_TEST_CONFIG.baseUrl}/api/family/checkout`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            token: testStudentToken,
  ...
  describe('Product Availability & Inventory', () => {
    ...
    it('5.2 Physical products should have inventory tracking', async () => {
      ...
      const physicalUpsells = PRODUCT_TEST_CONFIG.upsells.filter(u => 
        u.category === 'frame' || u.category === 'album'
      );
      
      for (const upsell of physicalUpsells) {
        // Test inventory check endpoint
        const inventoryResponse = await fetch(`${PRODUCT_TEST_CONFIG.baseUrl}/api/products/inventory/${upsell.id}`);
        ...
        if (inventoryResponse.status === 200) {
          ...
          console.log(`✅ ${upsell.name} inventory: ${inventory.quantity} available`);
        } else {
          console.log(`ℹ️ Inventory tracking not implemented for ${upsell.name}`);
        }
      }
    });
    ...
    it('5.3 Product combinations should be validated', async () => {
      ...
      for (const combination of validCombinations) {
        ...
        combination.forEach(upsellId => {
          const upsell = PRODUCT_TEST_CONFIG.upsells.find(u => u.id === upsellId);
          if (upsell) totalPrice += upsell.price;
        });
        
        expect(totalPrice).toBeGreaterThan(PRODUCT_TEST_CONFIG.wizardOptions.option1.price);
        
        console.log(`✅ Valid combination [${combination.join(' + ')}]: $${totalPrice}`);
      }
    });
  });
  ...
  describe('Pricing Edge Cases & Discounts', () => {
    ...
    it('6.1 Bulk discount calculation should work', async () => {
      ...
      const totalItems = Object.values(bulkUpsells).reduce((sum, qty) => sum + qty, 0) + 1; // +1 for base
...
async function setupProductTestData() {
  ...
  const authResponse = await fetch(`${PRODUCT_TEST_CONFIG.baseUrl}/api/admin/auth`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
