...
vi.mock('@/lib/supabase/server', () => ({
  ...
  createServerSupabaseServiceClient: vi.fn(() => Promise.resolve(mockSupabaseService))
}))

vi.mock('@/lib/services/watermark', () => ({
  validateImage: vi.fn(() => Promise.resolve(true)),
  processImageBatch: vi.fn(() => Promise.resolve({
...
describe('/api/admin/photos/upload', () => {
  ...
  function createTestFormData(eventId: string, files: Array<{ name: string; content: string; type?: string }>) {
    const formData = new FormData()
    formData.append('eventId', eventId)
    
    files.forEach(file => {
      const blob = new Blob([file.content], { type: file.type || 'image/jpeg' })
      const fileObj = new File([blob], file.name, { type: file.type || 'image/jpeg' })
  ...
  describe('Logging', () => {
    ...
    it('no debe loggear información sensible', async () => {
      ...
      expect(loggedStrings.some(log => log.includes('sensitive-data'))).toBe(false)
