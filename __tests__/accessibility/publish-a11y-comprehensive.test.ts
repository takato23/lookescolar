...
async function getTouchTargetSize(page: Page, element: string): Promise<{ width: number; height: number }> {
  return page.evaluate((selector) => {
    const el = document.querySelector(selector);
...
async function checkFocusIndicator(page: Page, element: string): Promise<boolean> {
  ...
  return page.evaluate((selector) => {
    ...
    return outline !== 'none' && outlineWidth >= 1 || 
           boxShadow !== 'none' || 
           styles.border !== 'none';
  }, element);
}

async function getAriaAttributes(page: Page, element: string): Promise<Record<string, string>> {
  return page.evaluate((selector) => {
    const el = document.querySelector(selector);
    if (!el) return {};
...
test.describe('WCAG 2.1 AA Compliance Testing', () => {
  test.beforeEach(async () => {
    ...
    await createTestEvent({ id: TEST_EVENT_ID, name: 'Accessibility Test Event' });
  ...
  test('WCAG 2.1 AA compliance >98%', async ({ page }) => {
    ...
    const accessibilityResults = await page.evaluate(async () => {
      ...
      images.forEach((img, index) => {
        if (!img.hasAttribute('alt')) {
          ...
          passes.push({ id: 'image-alt', target: `img:nth-child(${index + 1})` });
        }
      });
      ...
      inputs.forEach((input, index) => {
        ...
        if (!hasLabel) {
          violations.push({
            ...
            nodes: [{ target: `${input.tagName.toLowerCase()}:nth-child(${index + 1})` }]
          });
        } else {
          passes.push({ id: 'label', target: `${input.tagName.toLowerCase()}:nth-child(${index + 1})` });
        }
      });
      ...
      headings.forEach((heading, index) => {
        ...
        if (level > previousLevel + 1) {
          violations.push({
            ...
            nodes: [{ target: `${heading.tagName.toLowerCase()}:nth-child(${index + 1})` }]
          });
        }
        previousLevel = level;
      });
      ...
      buttons.forEach((button, index) => {
        if (!button.hasAttribute('role') || button.getAttribute('role') !== 'button') {
          violations.push({
            ...
            nodes: [{ target: `${button.tagName.toLowerCase()}:nth-child(${index + 1})` }]
          });
        }
      });
    ...
    const totalChecks = accessibilityResults.violationCount + accessibilityResults.passCount;
    const complianceRate = totalChecks > 0 ? (accessibilityResults.passCount / totalChecks) * 100 : 100;
    ...
    expect(complianceRate).toBeGreaterThan(A11Y_CONFIG.WCAG_COMPLIANCE_THRESHOLD);
...
test.describe('Touch Target Accessibility', () => {
  test('all touch targets >44px minimum', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(interactiveElements.length, 20); i++) { // Test first 20 elements
      ...
      if (boundingBox) {
        ...
        if (isVisible) {
          expect(width).toBeGreaterThanOrEqual(A11Y_CONFIG.MIN_TOUCH_TARGET_SIZE);
          expect(height).toBeGreaterThanOrEqual(A11Y_CONFIG.MIN_TOUCH_TARGET_SIZE);
...
test.describe('Keyboard Navigation', () => {
  test('keyboard navigation works completely', async ({ page }) => {
    ...
    const focusableElements = await page.locator('button:visible, a:visible, input:visible, select:visible, [tabindex="0"]:visible').all();
    ...
    for (let i = 0; i < maxTabs; i++) {
      ...
      if (focusedElement) {
        expect(['BUTTON', 'A', 'INPUT', 'SELECT', 'TEXTAREA'].concat(
...
test.describe('Screen Reader Compatibility', () => {
  test('screen reader announcements are correct', async ({ page }) => {
    ...
    if (await publishButton.count() > 0) {
      ...
      const statusUpdate = await page.waitForSelector('[aria-live="polite"], [role="status"], [data-testid*="toast"]', { 
        timeout: 5000 
      }).catch(() => null);
...
test.describe('Color and Contrast Accessibility', () => {
  test('color contrast ratios meet requirements', async ({ page }) => {
    ...
    const textElements = [
      'h1', 'h2', 'h3', 'p', 'button', 'a', 'label', 'input', '.text-gray-600', '.text-gray-900'
    ];
    ...
    for (const selector of textElements) {
      ...
      for (let i = 0; i < Math.min(elements.length, 5); i++) {
        ...
        if (isVisible) {
          const contrast = await getColorContrast(page, `${selector}:nth-of-type(${i + 1})`);
          ...
          const isLargeText = fontSize >= 18 || (fontSize >= 14 && await page.evaluate((sel) => {
            const el = document.querySelector(sel);
            return el ? window.getComputedStyle(el).fontWeight === 'bold' || parseInt(window.getComputedStyle(el).fontWeight) >= 700 : false;
          }, `${selector}:nth-of-type(${i + 1})`));
          
          const minContrast = isLargeText ? A11Y_CONFIG.MIN_COLOR_CONTRAST_RATIO_LARGE : A11Y_CONFIG.MIN_COLOR_CONTRAST_RATIO;
          
          if (contrast > 0) { // Only test if we could calculate contrast
            console.log(`${selector}:nth-of-type(${i + 1}) contrast: ${contrast.toFixed(2)} (min: ${minContrast})`);
  ...
  test('color is not the only means of conveying information', async ({ page }) => {
    ...
    const statusElements = await page.evaluate(() => {
      const elements = document.querySelectorAll('[data-testid*="status"], .badge, .tag');
      return Array.from(elements).map(el => ({
        textContent: el.textContent?.trim(),
        hasIcon: !!el.querySelector('svg, .icon'),
        hasPattern: el.classList.contains('border') || el.classList.contains('striped'),
    ...
    for (const element of statusElements) {
      ...
      const hasNonColorIndicator = element.textContent || element.hasIcon || element.hasPattern;
    ...
    const links = await page.evaluate(() => {
      ...
      return Array.from(linkElements).map(link => ({
        hasUnderline: window.getComputedStyle(link).textDecoration.includes('underline'),
        hasIcon: !!link.querySelector('svg, .icon'),
        isBold: window.getComputedStyle(link).fontWeight === 'bold' || parseInt(window.getComputedStyle(link).fontWeight) >= 700
      }));
    });

    for (const link of links) {
      const hasNonColorIndicator = link.hasUnderline || link.hasIcon || link.isBold;
      expect(hasNonColorIndicator).toBeTruthy();
    }
  });
});
...
test.describe('Focus Management', () => {
      ...
      for (let i = 0; i < Math.min(elements.length, 3); i++) {
        ...
        if (isVisible) {
          ...
          const hasFocusIndicator = await page.evaluate((selector) => {
            ...
            return (outline !== 'none' && outlineWidth >= 1) ||
                   (boxShadow !== 'none' && !boxShadow.includes('rgba(0, 0, 0, 0)')) ||
                   border.includes('rgb');
  ...
  test('focus trapping in modals', async ({ page }) => {
    ...
    const modalTrigger = page.locator('[data-testid*="modal"], [data-testid*="preview"]').first();
    
    if (await modalTrigger.count() > 0) {
      await modalTrigger.click();
      
      // Wait for modal to appear
      const modal = await page.waitForSelector('[role="dialog"], [data-testid*="modal"]', { 
        timeout: 3000 
      }).catch(() => null);
      
      if (modal) {
        // Test focus is trapped within modal
        const focusableInModal = await page.locator('[role="dialog"] button, [role="dialog"] a, [role="dialog"] input').all();
  ...
  test('mobile accessibility standards', async ({ page }) => {
    ...
    const mobileFeatures = await page.evaluate(() => {
      return {
        ...
        hasMobileMenu: !!document.querySelector('[aria-label*="menu"], .mobile-menu'),
        touchTargetsCount: document.querySelectorAll('button, a, input, select').length
      };
    });
    ...
    if (await firstButton.count() > 0) {
      ...
      const hasVisualFeedback = await page.evaluate(() => {
        ...
        return button ? window.getComputedStyle(button).transform !== 'none' ||
                       button.classList.contains('active') ||
                       button.getAttribute('aria-pressed') === 'true' : false;
      });
