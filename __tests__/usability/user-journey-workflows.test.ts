...
test.describe('Photographer Workflow Testing', () => {
  ...
  test('Student list generation and QR workflow', async ({ page }) => {
    ...
    await page.setInputFiles(
      ...
      '__tests__/fixtures/students.csv'
    );
    await page.click('[data-testid="import-submit"]');

    // Verify students imported
    await expect(page.locator('[data-testid="student-count"]')).toContainText(
      '25 estudiantes'
    );
    ...
    await expect(
      page.locator('[data-testid="qr-generation-progress"]')
    ).toBeVisible();
    await page.waitForSelector('[data-testid="qr-download-ready"]', {
      timeout: 10000,
    });
  ...
  test('Bulk photo upload with progress tracking', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="upload-count"]')).toContainText(
      '5 fotos'
    );
    ...
    const processedPhotos = page.locator(
      '[data-testid="photo-card"][data-status="processed"]'
    );
    expect(await processedPhotos.count()).toBe(5);

    // Check success notification
    await expect(page.locator('[data-testid="upload-success"]')).toContainText(
      '5 fotos procesadas exitosamente'
    );
  });
  ...
  test('Photo tagging workflow with keyboard shortcuts', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="selected-student"]')
    ).toContainText('Ana García López');
    ...
    await expect(page.locator('[data-testid="tagged-count"]')).toContainText(
      '1 foto etiquetada'
    );
  ...
  test('Order management workflow', async ({ page }) => {
    ...
    const pendingOrders = page.locator(
      '[data-testid="order-row"][data-status="pending"]'
    );
...
test.describe('Client Workflow Testing', () => {
  test('Family portal access via token', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="event-info"]')).toContainText(
      'Graduación 2024'
    );

    // Check token validation
    await expect(page.locator('[data-testid="token-status"]')).toContainText(
      'Válido hasta'
    );
  ...
  test('Checkout and payment workflow', async ({ page }) => {
    ...
    await page.fill(
      '[data-testid="delivery-notes"]',
      'Por favor contactar antes de entregar'
    );
...
test.describe('Public Gallery Workflow Testing', () => {
  test('Public gallery access and navigation', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="watermark-notice"]')
    ).toBeVisible();

    // Navigate between photos
    await page.keyboard.press('ArrowRight');
    await expect(page.locator('[data-testid="photo-counter"]')).toContainText(
      '2 de'
    );
  });
  ...
  test('Contact form workflow', async ({ page }) => {
    ...
    await page.fill(
      ...
      'Estoy interesado en comprar fotos de mi hijo'
    );
