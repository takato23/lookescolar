...
async function getCacheStats(page: Page): Promise<any> {
  return page.evaluate(() => {
    ...
    return {
      ...
      cachedQueries: queries.filter((q: any) => q.state.data !== undefined)
        .length,
...
test.describe('API Performance Validation', () => {
  ...
  test('handles 100+ folders without N+1 queries', async ({ page }) => {
    ...
    const publishListRequests = requests.filter((url) =>
      url.includes('/api/admin/publish/list')
    );
    expect(publishListRequests.length).toBeLessThanOrEqual(2); // Initial + possible retry

    const individualCodeRequests = requests.filter((url) =>
      url.match(/\/api\/admin\/publish\/[^\/]+$/)
    );
  ...
  test('API response time <200ms consistently', async ({ page }) => {
    ...
    const endpoints = [
      { url: '/api/admin/publish/list', method: 'GET' as const },
      {
        ...
        data: { codeId: 'perf-code-0' },
      },
      {
        url: '/api/admin/publish/unpublish',
        method: 'POST' as const,
        data: { codeId: 'perf-code-0' },
      },
      {
        url: '/api/admin/publish/rotate',
        method: 'POST' as const,
        data: { codeId: 'perf-code-0' },
      },
    ];
    ...
    for (const endpoint of endpoints) {
      ...
      console.log(
        `${endpoint.url}: avg=${avgTime.toFixed(2)}ms, max=${maxTime.toFixed(2)}ms, p95=${p95Time.toFixed(2)}ms`
      );

      expect(avgTime).toBeLessThan(PERFORMANCE_CONFIG.API_RESPONSE_THRESHOLD);
      expect(p95Time).toBeLessThan(
        PERFORMANCE_CONFIG.API_RESPONSE_THRESHOLD * 1.5
      ); // Allow 50% variance for p95
    }
  });
  ...
  test('React Query cache hit rate >90%', async ({ page }) => {
    ...
    if (cacheStats) {
      ...
      expect(cacheHitRate).toBeGreaterThan(
        PERFORMANCE_CONFIG.CACHE_HIT_RATE_THRESHOLD
      );
    }
  ...
  test('memory usage stays within bounds', async ({ page }) => {
    ...
    if (initialMemory && finalMemory) {
      ...
      expect(memoryIncrease).toBeLessThan(
        PERFORMANCE_CONFIG.MEMORY_THRESHOLD * 0.5
      ); // Max 50MB increase
    }
  });
...
test.describe('Concurrent Load Testing', () => {
  test('handles multiple concurrent users', async ({ browser }) => {
    ...
    try {
      ...
      console.log(
        `Concurrent test results: ${successful}/${PERFORMANCE_CONFIG.CONCURRENT_USERS} successful`
      );
      console.log(`Average duration: ${avgDuration.toFixed(2)}ms`);

      expect(successful).toBe(PERFORMANCE_CONFIG.CONCURRENT_USERS);
      expect(avgDuration).toBeLessThan(
        PERFORMANCE_CONFIG.PAGE_LOAD_THRESHOLD * 1.5
      );
    } finally {
  ...
  test('bulk operations performance', async ({ page }) => {
    ...
    // Bulk publish
    const bulkPublishButton = page.locator(
  ...
  test('mobile performance optimization', async ({ browser }) => {
    ...
    try {
      ...
      if (mobileMemory) {
        ...
        expect(mobileMemory.usedJSHeapSize).toBeLessThan(
          PERFORMANCE_CONFIG.MEMORY_THRESHOLD * 0.7
...
test.describe('Stress Testing & Edge Cases', () => {
  test('sustained load testing', async ({ page }) => {
    ...
    console.log(
      `Stress test completed: ${operationsCount} operations, ${errorsCount} errors`
    );
