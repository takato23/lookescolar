  ...
  describe('API Response Times', () => {
    it('Admin events API should respond under 200ms', async () => {
      ...
      if (response.ok) {
        ...
        console.warn(`Admin events API returned ${response.status}, timing: ${duration}ms`);
      }
    });
    ...
    it('Public gallery should load under 2 seconds', async () => {
      ...
      const response = await fetch(`${BASE_URL}/api/gallery/${testData.eventId}`);
...
describe('Error Handling Tests', () => {
  ...
  describe('Database Error Scenarios', () => {
    it('Should handle database connection issues', async () => {
      ...
      const response = await fetch(`${BASE_URL}/api/gallery/non-existent-event-id`);
  ...
  describe('Edge Cases & Race Conditions', () => {
    ...
    it('Should handle token expiration edge cases', async () => {
      ...
      const response = await fetch(`${BASE_URL}/api/family/gallery/${expiringSubject.token}`);
