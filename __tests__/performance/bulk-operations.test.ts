...
describe('Bulk Operations Performance Tests', () => {
  describe('Bulk Photo Upload Operations', () => {
    it('1.1 Small batch photo upload should meet performance targets', async () => {
      ...
      for (let i = 0; i < photoCount; i++) {
        ...
        formData.append(
          'photo_type',
          i % 3 === 0 ? 'individual' : i % 3 === 1 ? 'group' : 'activity'
        );

        const response = await fetch(
          `${BULK_TEST_CONFIG.baseUrl}/api/admin/photos/upload`,
          {
            method: 'POST',
        ...
        expect(uploadTime).toBeLessThan(
          BULK_TEST_CONFIG.thresholds.photoUpload
        );
      }
      ...
      expect(avgUploadTime).toBeLessThan(
        BULK_TEST_CONFIG.thresholds.photoUpload
      );
    });
    ...
    it('1.2 Batch photo processing should handle concurrent uploads', async () => {
      ...
      expect(avgTimePerUpload).toBeLessThan(
        BULK_TEST_CONFIG.thresholds.photoUpload
      );
    });
  ...
  describe('Bulk Student Management Operations', () => {
    it('2.1 Batch student creation should scale efficiently', async () => {
      const { courses: courseCount, studentsPerCourse } =
        BULK_TEST_CONFIG.scales.medium;
      let totalStudentsCreated = 0;
      const courseTimes = [];

      console.log(
        `👥 Testing batch creation for ${courseCount} courses with ${studentsPerCourse} students each...`
      );
      ...
      for (let courseIndex = 0; courseIndex < courseCount; courseIndex++) {
        ...
        for (
          let studentIndex = 1;
          studentIndex <= studentsPerCourse;
          studentIndex++
        ) {
          students.push({
            event_id: testEventId,
        ...
        expect(avgTimePerStudent).toBeLessThan(
          BULK_TEST_CONFIG.thresholds.studentCreation
        );
      }
      ...
      console.log(
        `   ${totalStudentsCreated} students created in ${totalTime}ms`
      );
    ...
    it('2.2 Mass token generation should be performant', async () => {
      ...
      console.log(
        `🎫 Testing token generation for ${studentIds.length} students...`
      );

      const startTime = Date.now();

      // Test bulk token generation via database function
      const { data: tokenResults } = await supabase.rpc(
      ...
      console.log(
        `   ${tokenResults.length} tokens in ${tokenGenerationTime}ms`
      );
      console.log(`   ${avgTimePerToken.toFixed(1)}ms average per token`);

      expect(avgTimePerToken).toBeLessThan(
        BULK_TEST_CONFIG.thresholds.tokenGeneration
      );
    });
    ...
    it('2.3 Bulk student queries should handle large datasets', async () => {
      ...
      console.log(
        `   ${studentsWithDetails.length} students with details in ${queryTime}ms`
      );
    });
  });
  ...
  describe('Mass Photo Classification Operations', () => {
    it('3.1 Bulk photo classification should handle large batches', async () => {
      ...
      console.log(
        `🏷️ Testing bulk classification of ${photoIds.length} photos...`
      );
      ...
      const response = await fetch(
        `${BULK_TEST_CONFIG.baseUrl}/api/admin/photos/classify?action=to-course`,
        {
          method: 'POST',
          headers: {
      ...
      console.log(
        `   ${photoIds.length} photos classified in ${classificationTime}ms`
      );
      console.log(`   ${avgTimePerPhoto.toFixed(1)}ms average per photo`);

      expect(avgTimePerPhoto).toBeLessThan(
        BULK_TEST_CONFIG.thresholds.classification
      );
    ...
    it('3.2 Individual photo classification should scale', async () => {
      ...
      console.log(
        `👤 Testing individual classification of ${photoIds.length} photos...`
      );
      ...
      for (let i = 0; i < Math.min(photoIds.length, studentIds.length); i++) {
        ...
        const response = await fetch(
          `${BULK_TEST_CONFIG.baseUrl}/api/admin/photos/classify?action=to-student`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
      ...
      const avgClassificationTime =
        classificationTimes.reduce((sum, time) => sum + time, 0) /
      ...
      console.log(
        `   Average: ${avgClassificationTime.toFixed(1)}ms per photo`
      );
      console.log(`   Maximum: ${maxClassificationTime}ms per photo`);

      expect(avgClassificationTime).toBeLessThan(
        BULK_TEST_CONFIG.thresholds.classification
      );
    ...
    it('3.3 Classification statistics should compute efficiently', async () => {
      ...
      expect(statsComputeTime).toBeLessThan(
        BULK_TEST_CONFIG.thresholds.bulkQuery
      );
  ...
  describe('Large Scale Performance Tests', () => {
    it('4.1 Gallery access should scale with large photo sets', async () => {
      ...
      const response = await fetch(
        `${BULK_TEST_CONFIG.baseUrl}/api/family/gallery/${testToken}`
      );
      ...
      if (response.status === 200) {
        ...
        expect(galleryLoadTime).toBeLessThan(
          BULK_TEST_CONFIG.thresholds.bulkQuery
        );

        console.log('✅ Large gallery access performance:');
        console.log(
          `   ${result.photos.length} photos loaded in ${galleryLoadTime}ms`
        );
      } else {
    ...
    it('4.2 Admin dashboard should handle large datasets', async () => {
      ...
      const [eventData, coursesData, studentsData, photosData] =
        await Promise.all(dashboardDataPromises);
      ...
      expect(dashboardLoadTime).toBeLessThan(
        BULK_TEST_CONFIG.thresholds.bulkQuery
      );
      ...
      console.log(
        `   Courses: ${coursesData.data?.length}, Students: ${studentsData.count}, Photos: ${photosData.count}`
      );
    });
    ...
    it('4.3 Search operations should remain fast at scale', async () => {
      ...
      for (const search of searchQueries) {
        ...
        console.log(
          `✅ Search "${search.query}" (${search.type}): ${searchResults?.data?.length} results in ${searchTime}ms`
        );
      }
    });
  });
  ...
  describe('Concurrent Operations Stress Tests', () => {
    it('5.1 Concurrent classification should not cause conflicts', async () => {
      ...
      const availablePhotos = testData.photoIds.slice(
        0,
        concurrentOperations * photosPerOperation
      );
      const courseId = testData.courseIds[1];

      console.log(
        `⚡ Testing ${concurrentOperations} concurrent classification operations...`
      );
      ...
      for (let i = 0; i < concurrentOperations; i++) {
        const photosBatch = availablePhotos.slice(
          i * photosPerOperation,
          (i + 1) * photosPerOperation
        );
      ...
      console.log(
        `   ${concurrentOperations} operations (${totalClassified} photos) in ${concurrentTime}ms`
      );
...
async function setupBulkTestEnvironment() {
  ...
  const authResponse = await fetch(
    `${BULK_TEST_CONFIG.baseUrl}/api/admin/auth`,
    {
      method: 'POST',
  ...
  if (testEventId) {
    try {
      ...
      await supabase
        .from('student_tokens')
        .delete()
        .in('student_id', testData.studentIds);
