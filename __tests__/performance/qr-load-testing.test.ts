...
class PerformanceMonitor {
  ...
  private detectMemoryLeak(): boolean {
    ...
    const startAvg = start.reduce((sum, mem) => sum + mem.heapUsed, 0) / samples;
...
class LoadTester {
  ...
  async runConcurrentQRDecodes(config: LoadTestConfig, qrCodes: string[]): Promise<LoadTestResults> {
    ...
    for (let user = 0; user < config.concurrentUsers; user++) {
      ...
      promises.push(
        this.delayedExecution(delay, () => 
          this.simulateUser(config, qrCodes)
        )
      );
    }
  ...
  private async delayedExecution(delay: number, fn: () => Promise<void>): Promise<void> {
    await new Promise(resolve => setTimeout(resolve, delay));
    return fn();
  }

  private async simulateUser(config: LoadTestConfig, qrCodes: string[]): Promise<void> {
  ...
  private calculateResults(totalDuration: number, performanceData: any): LoadTestResults {
    ...
    return {
      ...
      averageResponseTime: this.responseTimes.reduce((sum, time) => sum + time, 0) / this.responseTimes.length || 0,
...
describe('QR Workflow Load Testing', () => {
  ...
  describe('Concurrent QR Decoding Load Tests', () => {
    ...
    it('should handle high concurrency stress test', async () => {
      ...
      const rateLimitErrors = results.errors.filter(error => 
  ...
  describe('Batch Tagging Load Tests', () => {
    it('should handle large batch assignments efficiently', async () => {
      ...
      const responseTimeGrowth = successful[3].responseTime / successful[0].responseTime;
      expect(responseTimeGrowth).toBeLessThan(5); // Should not be more than 5x slower for 4x the data

      console.log('📦 Batch Tagging Performance:', successful.map(r => ({
    ...
    it('should handle concurrent batch operations', async () => {
      ...
      const promises = Array.from({ length: concurrentBatches }, (_, i) => {
        ...
        return batchTag(new NextRequest('http://localhost', {
          method: 'POST',
          body: JSON.stringify({
            eventId,
      ...
      const photoAssignmentCounts = assignments.reduce((acc: any, assignment: any) => {
        acc[assignment.photo_id] = (acc[assignment.photo_id] || 0) + 1;
        return acc;
      }, {});

      const duplicateAssignments = Object.values(photoAssignmentCounts).filter((count: any) => count > 1);
  ...
  describe('Memory Usage and Resource Management', () => {
    ...
    it('should handle database connection pooling efficiently', async () => {
      ...
      const dbOperationPromises = Array.from({ length: concurrentDbOperations }, async (_, i) => {
        const startTime = performance.now();
    ...
    it('should gracefully handle invalid requests during high load', async () => {
      ...
      const allPromises = [
        ...
        ...Array.from({ length: validRequests }, () => {
          const qrCode = qrCodes[Math.floor(Math.random() * qrCodes.length)];
          return decodeQR(new NextRequest('http://localhost', {
            method: 'POST',
            body: JSON.stringify({ qrCode }),
          }));
        }),
  ...
  describe('Performance Benchmarks', () => {
    ...
    it('should maintain performance with database growth', async () => {
      ...
      for (const size of datasetSizes) {
        ...
        const avgTime = testResponses.reduce((sum, time) => sum + time, 0) / testResponses.length;
      ...
      const performanceDegradation = performanceResults[2].avgResponseTime / performanceResults[0].avgResponseTime;
      expect(performanceDegradation).toBeLessThan(2); // Should not be more than 2x slower

      console.log('📈 Database Growth Performance:', performanceResults.map(r => ({
