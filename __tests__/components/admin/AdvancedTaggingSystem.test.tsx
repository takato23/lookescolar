...
describe('AdvancedTaggingSystem', () => {
  ...
  describe('Modo Manual', () => {
    it('debe permitir seleccionar sujeto y asignar fotos', async () => {
      ...
      // Seleccionar sujeto
      const subjectCard = screen.getByText('María García').closest('.cursor-pointer')!
    ...
    it('debe mostrar error si no hay sujeto seleccionado', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Selecciona un alumno primero')).toBeInTheDocument()
      })
    ...
    it('debe mostrar error si no hay fotos seleccionadas', async () => {
      ...
      // Seleccionar sujeto
      const subjectCard = screen.getByText('Juan Pérez').closest('.cursor-pointer')!
      fireEvent.click(subjectCard)
      ...
      await waitFor(() => {
        expect(screen.getByText('Selecciona al menos una foto')).toBeInTheDocument()
      })
      ...
      vi.mocked(fetch).mockImplementation(async (url, _options) => {
        if (url === '/api/admin/subjects') {
          return {
            ok: true,
            json: () => Promise.resolve({
              subjects: mockSubjects.filter(s => s.token === 'test-token-123')
            })
          } as Response
        }
        return {
