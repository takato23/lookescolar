...
vi.mock('react-window', () => ({
  FixedSizeGrid: ({ children, itemData, columnCount, rowCount }: any) => (
    <div data-testid="virtualized-grid">
      ...
      {Array.from(
        { length: Math.min(columnCount * rowCount, 6) },
        (_, index) => {
...
describe('VirtualizedPhotoGrid', () => {
  ...
  beforeEach(() => {
    ...
    const {
      ...
      setCachedUrl,
      preloadPhotoUrls,
    } = require('@/lib/utils/signed-url-cache');
  ...
  describe('Rendering', () => {
    ...
    it('debe mostrar empty state cuando no hay fotos', () => {
      ...
      expect(
        screen.getByText('Las fotos aparecerán aquí cuando se suban')
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Photo Selection', () => {
    it('debe manejar selección de fotos', async () => {
      ...
      const checkboxes = screen
        ...
        .filter(
          (btn) =>
            btn.getAttribute('aria-label')?.includes('Seleccionar') ||
  ...
  describe('Photo Viewing', () => {
    ...
    it('debe mostrar botón de maximizar en hover (grid mode)', async () => {
      ...
      await waitFor(() => {
        ...
        const maximizeButtons = screen
          .getAllByRole('button')
          .filter(
            (btn) =>
              btn.getAttribute('aria-label')?.includes('Maximizar') ||
              btn
                .querySelector('svg')
                ?.getAttribute('class')
                ?.includes('Maximize')
  ...
  describe('URL Caching Integration', () => {
    it('debe usar URLs cacheadas cuando están disponibles', async () => {
      ...
      vi.mocked(getCachedUrl).mockReturnValue(
        'https://cached-url.com/photo1.jpg'
      );
  ...
  describe('Error Handling', () => {
    it('debe manejar errores al cargar URLs firmadas', async () => {
      ...
      await waitFor(() => {
        ...
        const errorPlaceholders = screen
          .getAllByRole('generic')
          .filter((el) =>
            el
              .querySelector('svg')
    ...
    it('debe manejar respuestas de error HTTP', async () => {
      ...
      await waitFor(() => {
        ...
        const errorElements = screen
          .getAllByRole('generic')
          .filter(
            (el) => el.className.includes('bg-muted') && el.querySelector('svg')
          );
    ...
    it('debe tener alt text apropiado para imágenes', async () => {
      ...
      await waitFor(() => {
        ...
        const images = screen
          ...
          .filter((img) => img.getAttribute('alt')?.includes('photo'));
