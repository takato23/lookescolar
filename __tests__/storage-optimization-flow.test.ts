...
describe('Storage Optimization Flow', () => {
  ...
  it('should process image through complete optimization flow', async () => {
    ...
    if (optimizedResult.actualSizeKB > 50) {
      throw new Error(
        `Actual size ${optimizedResult.actualSizeKB}KB exceeds limit of 50KB`
      );
    }

    if (optimizedResult.finalDimensions.w < 1) {
      throw new Error(
        `Width ${optimizedResult.finalDimensions.w} is less than 1`
      );
    }

    if (optimizedResult.finalDimensions.h < 1) {
      throw new Error(
        `Height ${optimizedResult.finalDimensions.h} is less than 1`
      );
    }

    if (optimizedResult.compressionLevel < 0) {
      throw new Error(
        `Compression level ${optimizedResult.compressionLevel} is negative`
      );
    }
    ...
    const storageAnalysis = FreeTierOptimizer.analyzeStorageRequirements(
      1000,
      20,
      35
    );
    if (storageAnalysis.totalPhotos !== 20000) {
      throw new Error(
        `Expected 20000 photos, got ${storageAnalysis.totalPhotos}`
      );
    }

    if (storageAnalysis.estimatedStorageGB >= 1) {
      throw new Error(
        `Expected storage under 1GB, got ${storageAnalysis.estimatedStorageGB}GB`
      );
    }

    if (storageAnalysis.fitsInFreeTier !== true) {
      throw new Error(
        `Expected to fit in free tier, got ${storageAnalysis.fitsInFreeTier}`
      );
    }
    ...
    if (metrics.targetSizeKB !== 35) {
      throw new Error(
        `Expected target size 35KB, got ${metrics.targetSizeKB}KB`
      );
    }

    if (metrics.maxDimension !== 500) {
      throw new Error(
        `Expected max dimension 500, got ${metrics.maxDimension}`
      );
    }

    if (metrics.estimatedPhotosForFreeTier <= 10000) {
      throw new Error(
        `Expected more than 10000 photos for free tier, got ${metrics.estimatedPhotosForFreeTier}`
      );
    }
  });
  ...
  it('should provide accurate storage monitoring', async () => {
    ...
    if (!usageMetrics.estimatedStorageUsed.includes('MB')) {
      throw new Error(
        `Expected storage usage in MB, got ${usageMetrics.estimatedStorageUsed}`
      );
    }

    if (usageMetrics.percentageOfFreeTier < 0) {
      throw new Error(
        `Expected non-negative percentage, got ${usageMetrics.percentageOfFreeTier}`
      );
    }
  });
  ...
  it('should maintain consistent optimization across different image sizes', async () => {
    ...
    for (const testCase of testCases) {
      ...
      if (result.actualSizeKB > testCase.targetSizeKB + 15) {
        throw new Error(
          `For ${testCase.description}: Expected size <= ${testCase.targetSizeKB + 15}KB, got ${result.actualSizeKB}KB`
        );
      }

      if (result.finalDimensions.w < 1) {
        throw new Error(
          `For ${testCase.description}: Width ${result.finalDimensions.w} is less than 1`
        );
      }

      if (result.finalDimensions.h < 1) {
        throw new Error(
          `For ${testCase.description}: Height ${result.finalDimensions.h} is less than 1`
        );
      }
    }
  });
  ...
  it('should enforce no original storage policy', async () => {
    ...
    if (result.actualSizeKB > 55) {
      throw new Error(
        `Actual size ${result.actualSizeKB}KB exceeds limit of 55KB`
      );
    }
  });
});
