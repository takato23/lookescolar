...
async function createTestImage(width: number = 800, height: number = 600): Promise<Buffer> {
  return sharp({
    create: {
      width,
      height,
...
describe('Watermark Service', () => {
  describe('processImageWithWatermark', () => {
    ...
    it('debe aplicar watermark en diferentes posiciones', async () => {
      ...
      const positions: Array<'center' | 'bottom-right' | 'bottom-left' | 'top-right' | 'top-left'> = [
    ...
    it('debe fallar con buffer inválido', async () => {
      ...
      await expect(
        processImageWithWatermark(invalidBuffer)
      ).rejects.toThrow()
    })
  })
  ...
  describe('processImageBatch', () => {
    ...
    it('debe procesar imágenes en lotes respetando concurrencia', async () => {
      ...
      const { results, errors } = await processImageBatch(resolvedImages, {}, 2)
  ...
  describe('validateImage', () => {
    it('debe validar imágenes válidas', async () => {
      const validFormats = [
        { buffer: await sharp({ create: { width: 100, height: 100, channels: 3, background: 'red' } }).jpeg().toBuffer(), format: 'JPEG' },
        { buffer: await sharp({ create: { width: 100, height: 100, channels: 3, background: 'green' } }).png().toBuffer(), format: 'PNG' },
        { buffer: await sharp({ create: { width: 100, height: 100, channels: 3, background: 'blue' } }).webp().toBuffer(), format: 'WebP' }
      ]
