...
beforeAll(async () => {
  ...
  process.env['APPROX_PREVIEW_BYTES'] =
    process.env['APPROX_PREVIEW_BYTES'] || '200000';
  process.env['UPSTASH_REDIS_REST_URL'] =
    process.env['UPSTASH_REDIS_REST_URL'] || 'http://localhost:6380';
  process.env['UPSTASH_REDIS_REST_TOKEN'] =
    process.env['UPSTASH_REDIS_REST_TOKEN'] || 'stub';
  process.env['MERCADOPAGO_ACCESS_TOKEN'] =
    process.env['MERCADOPAGO_ACCESS_TOKEN'] || 'stub';
  ...
  process.env['TEST_ADMIN_EMAIL'] =
    process.env['TEST_ADMIN_EMAIL'] || 'admin@lookescolar.test';
      ...
      (globalThis as any).fetch = ((input: any, init?: any) => {
        ...
        if (typeof urlStr === 'string' && !/^https?:\/\//i.test(urlStr)) {
          ...
          try {
            ...
            console.warn(
              `Failed to construct URL from ${sanitizedPath} and base ${base}:`,
              urlError
            );
          ...
          const from = (table: string) => {
            ...
            const chain: any = {
              ...
              insert: (payload: any) => {
                ...
                rows = rows.concat(
                  arr.map((x: any) => JSON.parse(JSON.stringify(x)))
                );
                dataset[table] = rows;
              ...
              not: (col: string, op: string, val: any) => {
                if (op === 'is' && val === null) {
                  rows = rows.filter(
                    (r: any) => r[col] !== null && typeof r[col] !== 'undefined'
                  );
                }
                return chain;
              },
            ...
            storage: {
              from: (_bucket: string) => ({
                createSignedUrl: async (key: string, expiresSec: number) => ({
                  data: {
                    signedUrl: `https://signed.local/${encodeURIComponent(key)}?exp=${expiresSec}`,
                    path: key,
