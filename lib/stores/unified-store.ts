...
import { ProductOption, AdditionalCopy, CartItem, ContactInfo, UnifiedOrder, PRODUCT_CATALOG } from '@/lib/types/unified-store';
  ...
  persist(
    (set, get) => ({
      ...
      selectPackage: (packageId) => {
        const packageOption = PRODUCT_CATALOG.productOptions.find(p => p.id === packageId);
        if (packageOption) {
          set({ 
