...
interface ProductCartStore {
  ...
  selected_products: Map<
    string,
    { product_id?: string; combo_id?: string; quantity: number }
  >;
  ...
  getPhotoProduct: (
    photo_id: string
  ) => { product_id?: string; combo_id?: string; quantity: number } | null;
  ...
  persist(
    (set, get) => ({
      ...
      selectPhoto: (photo_id, filename, watermark_url) => {
        set((state) => {
          ...
          return {
            ...
            enhanced_items: state.enhanced_items.some(
              (item) => item.photo_id === photo_id
            )
      ...
      deselectPhoto: (photo_id) => {
        set((state) => {
          ...
          return {
            selected_photos: new_photos,
            enhanced_items: state.enhanced_items.filter(
              (item) => item.photo_id !== photo_id
            ),
      ...
      syncWithUnifiedCart: () => {
        ...
        state.enhanced_items.forEach((item) => {
          if (item.unit_price > 0) {
            // Only add items with selected products
            unifiedCart.addItem({
      ...
      validateCart: () => {
        ...
        const photos_without_products = Array.from(
          state.selected_photos
        ).filter((photo_id) => !state.selected_products.has(photo_id));

        if (photos_without_products.length > 0) {
          errors.push(
            `${photos_without_products.length} foto(s) sin producto seleccionado`
          );
        }
      ...
      // Utility
      getTotalItems: () => {
        return get().enhanced_items.reduce(
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        customization_options: Array.from(
          state.customization_options.entries()
        ),
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          ...
          state.customization_options = new Map(
            state.customization_options as any
          );
        }
      },
    }
