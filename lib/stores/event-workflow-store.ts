...
export interface WorkflowAction {
  id: string;
  type: 'upload' | 'tag' | 'publish' | 'organize' | 'review' | 'export' | 'notify';
  title: string;
  description: string;
...
export interface QualityIssue {
  id: string;
  type: 'missing_photos' | 'untagged_content' | 'low_engagement' | 'incomplete_setup' | 'technical_issue';
...
export class WorkflowDetector {
  static detectEventPhase(metrics: EventMetrics, eventInfo: any): EventPhase {
    ...
    if (metrics.totalPhotos < 50 || metrics.untaggedPhotos > metrics.totalPhotos * 0.8) {
      return EventPhase.CONTENT_UPLOAD;
    }
    
    // Organization phase
    if (metrics.untaggedPhotos > metrics.totalPhotos * 0.3 || metrics.activeTokens === 0) {
    ...
    // Active sales phase
    if (metrics.totalOrders < metrics.totalSubjects * 0.3 && metrics.engagementRate > 0) {
  ...
  static generateNextActions(phase: EventPhase, metrics: EventMetrics, eventInfo: any): WorkflowAction[] {
    ...
    switch (phase) {
      case EventPhase.SETUP:
        actions.push({
          ...
          description: 'Importa la lista de estudiantes desde CSV o agrega manualmente',
      ...
      case EventPhase.CONTENT_UPLOAD:
        ...
        if (metrics.untaggedPhotos > 10) {
          actions.push({
            ...
            icon: 'Tag',
            color: 'orange',
            aiSuggestion: 'IA puede sugerir etiquetas basadas en reconocimiento facial'
          });
        }
        break;
  ...
  static detectQualityIssues(metrics: EventMetrics, eventInfo: any): QualityIssue[] {
  ...
  static generateSmartSuggestions(phase: EventPhase, metrics: EventMetrics): SmartSuggestion[] {
...
export const useEventManagement = create<EventManagementStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    initializeEvent: async (eventId: string) => {
      ...
      try {
        ...
        const metrics = {
          ...
          taggedPhotos: (eventInfo.stats?.totalPhotos || 0) - (eventInfo.stats?.untaggedPhotos || 0),
          ...
          conversionRate: eventInfo.stats?.totalSubjects > 0 ? (eventInfo.stats?.totalOrders || 0) / eventInfo.stats.totalSubjects : 0,
        ...
        const currentPhase = WorkflowDetector.detectEventPhase(metrics, eventInfo);
        const nextActions = WorkflowDetector.generateNextActions(currentPhase, metrics, eventInfo);
        const qualityIssues = WorkflowDetector.detectQualityIssues(metrics, eventInfo);
        const smartSuggestions = WorkflowDetector.generateSmartSuggestions(currentPhase, metrics);
    ...
    updateMetrics: (newMetrics: Partial<EventMetrics>) => {
      ...
      const currentPhase = WorkflowDetector.detectEventPhase(updatedMetrics, state.eventInfo);
      const nextActions = WorkflowDetector.generateNextActions(currentPhase, updatedMetrics, state.eventInfo);
      const qualityIssues = WorkflowDetector.detectQualityIssues(updatedMetrics, state.eventInfo);
      const smartSuggestions = WorkflowDetector.generateSmartSuggestions(currentPhase, updatedMetrics);
    ...
    completeAction: (actionId: string) => {
      ...
      set({
        ...
        nextActions: state.nextActions.filter(action => action.id !== actionId)
      });
    },
    ...
    dismissSuggestion: (suggestionId: string) => {
      const state = get();
      set({
        smartSuggestions: state.smartSuggestions.filter(suggestion => suggestion.id !== suggestionId)
      });
    },
    ...
    getCurrentPriorities: () => {
      ...
      return state.nextActions
        .filter(action => action.priority === WorkflowPriority.CRITICAL || action.priority === WorkflowPriority.HIGH)
        .sort((a, b) => {
