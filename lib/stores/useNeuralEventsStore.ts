...
export const useNeuralEventsStore = create<NeuralEventsState>()(
  devtools(
    (set, get) => ({
      // Initial state
      events: [],
      ...
      getTotalStats: () => {
        ...
        return events.reduce(
          (totals, event) => ({
            ...
            totalSubjects:
              totals.totalSubjects + (event.stats?.totalSubjects || 0),
            avgCompletionRate:
              totals.avgCompletionRate + (event.stats?.completionRate || 0),
          }),
          {
            totalEvents: 0,
            totalPhotos: 0,
