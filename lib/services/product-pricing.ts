...
class ProductPricingService {
  ...
  calculateCartTotal(
    ...
    const tax = context.tax_rate
      ? Math.round((subtotal * context.tax_rate) / 100)
      : 0;
  ...
  private getItemPrice(
    ...
    if (event_pricing) {
      const override = event_pricing.find(
        (ep) =>
  ...
  private calculateTieredPrice(
    ...
    const tiers =
      (combo.metadata?.pricing_tiers as Array<{
        min_photos: number;
        max_photos?: number;
        price: number;
      }>) || [];
  ...
  generateRecommendations(
    ...
    for (const combo of available_combos.filter((c) => c.is_active)) {
      if (
        photo_count >= combo.min_photos &&
        (!combo.max_photos || photo_count <= combo.max_photos)
      ) {
        const combo_price = this.calculateComboPrice(
          combo,
          photo_count,
          context
        );
    ...
    const featured_products = available_products.filter(
  ...
  validatePricing(calculation: PriceCalculation): {
    ...
    if (calculation.total > 100000000) {
      // $1M limit
      errors.push('El precio total excede el límite máximo');
    }

    // Validate breakdown sums
    const breakdown_total = calculation.breakdown.reduce(
      (sum, item) => sum + item.total_price,
      0
    );

    if (Math.abs(breakdown_total - calculation.total) > 1) {
  ...
  private initializeDefaultRules(): void {
    ...
    this.pricing_rules.push({
      ...
      calculate: (items, context) => {
        ...
        return Math.round(
          (subtotal * (context.bulk_discount_percentage || 10)) / 100
        );
      },
    ...
    this.pricing_rules.push({
      ...
      type: 'discount',
      condition: (items) => {
        const has_print = items.some(
          (item) => item.product_specs.type === 'print' && item.quantity > 0
        );
        const has_digital = items.some(
          (item) => item.product_specs.type === 'digital' && item.quantity > 0
        );
        return has_print && has_digital;
      },
      calculate: (items) => {
        const digital_items = items.filter(
          (item) => item.product_specs.type === 'digital'
        );
        return digital_items.reduce(
          (sum, item) => sum + item.unit_price * item.quantity,
          0
        );
      },
  ...
  removePricingRule(rule_id: string): void {
    this.pricing_rules = this.pricing_rules.filter(
...
export function calculateProductCartTotal(
  ...
  return productPricingService.calculateCartTotal(
    items,
    context,
    event_pricing
  );
