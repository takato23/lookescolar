...
import { 
  BarChart3, TrendingUp, Clock, Users, Camera, 
  ShoppingCart, DollarSign, Activity, Calendar,
  MapPin, RefreshCw, AlertCircle, CheckCircle2,
  Eye, Zap, Target, ArrowUp, ArrowDown, Minus
} from 'lucide-react';
...
export function ContextSidebar({ 
  ...
  const liveMetrics = [
    ...
    },
    {
      ...
      trend: metrics.engagementRate > 0.5 ? 'up' : metrics.engagementRate > 0.3 ? 'stable' : 'down',
      color: metrics.engagementRate > 0.5 ? 'green' : metrics.engagementRate > 0.3 ? 'yellow' : 'red'
    },
    {
      ...
      color: metrics.conversionRate > 0.4 ? 'green' : metrics.conversionRate > 0.2 ? 'yellow' : 'red'
    },
    {
      label: 'Ingresos/hora',
      ...
      >
        ...
        <div className="glass-stat-card-ios26 p-4">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h3 className="font-semibold text-sm mb-1">Información del Evento</h3>
            ...
            <Badge 
              ...
              className={`glass-label-ios26 ${
                healthScore > 80 ? 'border-green-300 text-green-700' :
                healthScore > 60 ? 'border-yellow-300 text-yellow-700' :
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="font-medium">
                {Math.ceil((Date.now() - new Date(eventInfo.created_at).getTime()) / (1000 * 60 * 60 * 24))} días
              </span>
            </div>
            ...
            {eventInfo.location && (
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground">{eventInfo.location}</span>
              </div>
            )}
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className={`font-medium ${eventInfo.active ? 'text-green-600' : 'text-red-600'}`}>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-lg font-bold gradient-text-ios26">{metric.value}</p>
                    </div>
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            <AnimatePresence>
              {recentActivities.map((activity, index) => (
                <motion.div
                  key={activity.id}
                  className="glass-stat-card-ios26 p-3 cursor-pointer hover:bg-white/10 transition-colors"
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg bg-${activity.color}-500/10 flex-shrink-0`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs font-bold text-blue-600">+{activity.count}</span>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="glass-stat-card-ios26 p-3 text-center">
              ...
              <p className="text-lg font-bold gradient-text-ios26">{metrics.totalSubjects}</p>
              <p className="text-xs text-muted-foreground">Familias</p>
            </div>
            
            <div className="glass-stat-card-ios26 p-3 text-center">
              <Camera className="h-4 w-4 text-blue-500 mx-auto mb-1" />
              <p className="text-lg font-bold gradient-text-ios26">{metrics.totalPhotos}</p>
              <p className="text-xs text-muted-foreground">Fotos</p>
            </div>
            
            <div className="glass-stat-card-ios26 p-3 text-center">
              <ShoppingCart className="h-4 w-4 text-green-500 mx-auto mb-1" />
              <p className="text-lg font-bold gradient-text-ios26">{metrics.totalOrders}</p>
              <p className="text-xs text-muted-foreground">Pedidos</p>
            </div>
            
            <div className="glass-stat-card-ios26 p-3 text-center">
              <DollarSign className="h-4 w-4 text-emerald-500 mx-auto mb-1" />
              <p className="text-lg font-bold gradient-text-ios26">
