...
export default function PublishClient(props?: {
  ...
  const normalizedFolders = useMemo(
    () =>
      effectiveFolders.map((folder) => ({
        ...
        parent_id:
          'parent_id' in folder
            ? ((folder as { parent_id?: string | null }).parent_id ?? null)
            : null,
        depth:
          'depth' in folder
            ? ((folder as { depth?: number }).depth ?? 0)
            : 0,
      })),
    [effectiveFolders]
  );
  ...
  useEffect(() => {
    setSelectedFolders([]);
    if (selectedEventId) {
      // Simple measurement: log number of fetches for this event scope
      const q = fetchCounter.getCount('folders/published.query', selectedEventId);
      const p = fetchCounter.getCount('folders/published.prefetch', selectedEventId);
      console.debug('[Publish] Event scope changed:', {
  ...
  // Prefetch folders when selecting an event (deduped via react-query)
  useEffect(() => {
    const prefetch = async () => {
      ...
      try {
        await queryClient.prefetchQuery({
          ...
          queryFn: async () => {
            fetchCounter.increment('folders/published.prefetch', selectedEventId);
            ...
            if (!res.ok) {
              ...
              const msg = `Fetch failed /api/admin/folders/published?event_id=${selectedEventId} (HTTP ${res.status}). ${txt}`;
              console.error('[Publish] Prefetch error:', msg);
              throw new Error(msg);
            }
  ...
  const unpublish = useCallback(
    ...
    [effectiveFolders, unpublishMutation, showUnpublishSuccess, publish, effectiveEvent, initializeEvent]
  );
  ...
  useEffect(() => {
    const loadPublicShare = async () => {
      const evId = (effectiveEvent?.id as string) || selectedEventId || '';
      if (!evId) return setPublicStoreUrl('');
      try {
        // find active event-level share token
        const params = new URLSearchParams({ event_id: evId, active: 'true' });
        const res = await fetch(`/api/admin/share/list?${params}`);
        if (!res.ok) return;
        const data = await res.json();
        const eventToken = (data.tokens || []).find((t: any) => t.share_type === 'event' && t.is_active);
        if (eventToken?.store_url) setPublicStoreUrl(eventToken.store_url);
        else setPublicStoreUrl('');
      } catch {}
    };
    loadPublicShare();
  }, [effectiveEvent?.id, selectedEventId]);
        ...
        <Card className="p-4">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              {getPublicUrl() ? (
                <div className="text-sm text-foreground break-all">{getPublicUrl()}</div>
              ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="mt-2 text-xs text-red-700">
                  Sugerencias: verifica migraciones (e.g. supabase/migrations/20250826_folder_sharing_system.sql),
                  permisos RLS en tablas y funciones RPC relacionadas.
                </div>
              </div>
              <Button
                variant="outline"
