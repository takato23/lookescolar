...
import { z } from 'zod';
import { enhancedOrderService, type UpdateOrderRequest } from '@/lib/services/enhanced-order.service';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[Admin Bulk Update] Starting bulk update of ${order_ids.length} orders`, {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!Array.isArray(order_ids) || order_ids.length === 0 || order_ids.length > 100) {
      return NextResponse.json(
    ...
    console.log(`[Admin Bulk Cancel] Starting bulk cancellation of ${order_ids.length} orders`, {
      reason,
      admin_id: adminId
    });
