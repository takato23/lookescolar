...
import { folderPublishService, FolderQuerySchema } from '@/lib/services/folder-publish.service';
...
const queryParamsSchema = z.object({
  ...
  order_by: z
    .enum(['name_asc', 'name_desc', 'photos_desc', 'photos_asc', 'published_asc', 'published_desc'])
    .optional()
...
async function handleGET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    let result;
    try {
      const { data: paginatedData, error: funcError } = await supabase
        .rpc('get_folders_paginated', {
          ...
          p_order_by: parsedParams.order_by.replace('published_desc', 'published_at_desc')
        });
      ...
      // Apply ordering
      const orderField = parsedParams.order_by.includes('name') ? 'name' : 
                        parsedParams.order_by.includes('photos') ? 'photo_count' : 'published_at';
