...
async function handlePOST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const { folder_ids, action, batch_size, settings } =
      bulkPublishSchema.parse(body);
    ...
    if (action === 'publish') {
      ...
      try {
        const { data: bulkResults, error: funcError } = await supabase.rpc(
        ...
        if (funcError) {
          console.warn(
            '[API] Bulk function error, using fallback:',
            funcError.message
          );
          throw funcError;
        }
        ...
        results = (bulkResults || []).map((result: any) => ({
          ...
          family_url: result.family_url
            ? `${request.headers.get('origin') || 'http://localhost:3000'}${result.family_url}`
            : null,
          qr_url: result.qr_url
            ? `${request.headers.get('origin') || 'http://localhost:3000'}${result.qr_url}`
            : null,
        ...
        results = await processSequentialBulk(
          supabase,
          folder_ids,
          'publish',
          batch_size,
    ...
    if (executionTime > 5000) {
      console.warn(
        `[PERF] Slow bulk operation: ${executionTime}ms for ${folder_ids.length} folders`
      );
      ...
      try {
        if (action === 'publish') {
          ...
          if (!photoCount || photoCount === 0) {
            console.log(
              `[DEBUG] Publishing empty folder ${folderId} for testing purposes`
            );
          }
