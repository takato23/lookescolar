  ...
  withAuth(async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        else if (result.error?.includes('already exists') || result.error?.includes('circular reference') || result.error?.includes('cannot move')) statusCode = 409;
...
export const DELETE = RateLimitMiddleware.withRateLimit(
  withAuth(async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    try {
      ...
      const normalizedMoveTarget = moveContentsTo === '' || moveContentsTo === 'null' ? null : moveContentsTo;
