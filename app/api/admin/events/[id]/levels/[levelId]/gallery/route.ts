...
export const GET = withAuth(async (req: NextRequest, { params }: { params: { id: string, levelId: string } }) => {
  try {
    ...
    photoQuery = photoQuery
      .or(`course_id.in.(${supabase.rpc('get_courses_for_level', { level_id: levelId })}),student_id.in.(${supabase.rpc('get_students_for_level', { level_id: levelId })})`);
    ...
    const { data: stats, error: statsError } = await supabase.rpc('get_level_gallery_stats', {
      level_id: levelId
    });
    ...
    console.error('Error in GET /api/admin/events/[id]/levels/[levelId]/gallery:', error);
    return NextResponse.json(
...
export const POST = withAuth(async (req: NextRequest, { params }: { params: { id: string, levelId: string } }) => {
  try {
    const eventId = params.id;
    const levelId = params.levelId;
    const { searchParams } = new URL(req.url);
    ...
    console.error('Error in POST /api/admin/events/[id]/levels/[levelId]/gallery/download:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
