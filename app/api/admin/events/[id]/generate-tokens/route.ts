...
export async function POST(
  ...
  try {
    ...
    console.log(`[${requestId}] Mass token generation for event:`, { eventId: `${eventId.substring(0, 8)}***` });
    ...
    if (!force_regenerate) {
      ...
      subjectsToProcess = subjects.filter(subject => !existingSubjectIds.has(subject.id));
    ...
    console.log(`[${requestId}] Generating tokens for ${subjectIds.length} subjects`);
...
export async function GET(
  ...
  try {
    ...
    console.log(`[${requestId}] Getting token generation summary for event:`, { eventId: `${eventId.substring(0, 8)}***` });
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        subjects_without_photos: (totalSubjects || 0) - (studentsWithPhotos || 0),
      ...
      recommendations: {
        ...
        ready_for_distribution: activeTokens.length > 0 && (studentsWithPhotos || 0) > 0,
        has_photos_assigned: (studentsWithPhotos || 0) > 0
      },
      actions: {
        generate_missing_tokens: needTokens > 0 ? `/api/admin/events/${eventId}/generate-tokens` : null,
        export_existing_tokens: activeTokens.length > 0 ? `/api/admin/events/${eventId}/tokens/export?format=csv` : null,
