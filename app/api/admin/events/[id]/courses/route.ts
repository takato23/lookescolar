  ...
  async (req: NextRequest, { params }: { params: { id: string } }) => {
    try {
      ...
      if (parentCourseId) {
        ...
        query = query.or('parent_course_id.is.null,parent_course_id.eq.', {
      ...
      if (search) {
        query = query.or(
          `name.ilike.%${search}%,grade.ilike.%${search}%,section.ilike.%${search}%`
        );
      }
      ...
      const processedCourses = (courses || []).map((course) => ({
        ...
        photo_count:
          course.students?.reduce((total: number, student: any) => {
  ...
  async (req: NextRequest, { params }: { params: { id: string } }) => {
    try {
      ...
      if (validatedData.parent_course_id) {
        nameCheckQuery = nameCheckQuery.eq(
          'parent_course_id',
          validatedData.parent_course_id
        );
      } else {
      ...
      if (existingCourse) {
        return NextResponse.json(
          {
            error:
              'A course or folder with this name already exists in this location',
          },
          { status: 400 }
        );
      }
      ...
      if (sortOrder === undefined) {
        ...
        if (validatedData.parent_course_id) {
          sortOrderQuery = sortOrderQuery.eq(
            'parent_course_id',
            validatedData.parent_course_id
          );
        } else {
