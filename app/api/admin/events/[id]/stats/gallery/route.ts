  ...
  try {
    ...
    if (student_id) {
      ...
      photoQuery = photoQuery
        .or(`course_id.eq.${course_id},student_id.in.(${supabase.rpc('get_students_for_course', { course_id })})`);
    } else if (level_id) {
      // Level-level photos (through courses and students)
      photoQuery = photoQuery
        .or(`course_id.in.(${supabase.rpc('get_courses_for_level', { level_id })}),student_id.in.(${supabase.rpc('get_students_for_level', { level_id })})`);
    }
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch photos for statistics', details: error.message },
        { status: 500 }
      );
    }
    ...
    const individual_photos = photos?.filter(p => p.photo_type === 'individual').length || 0;
    const group_photos = photos?.filter(p => p.photo_type === 'group').length || 0;
    const activity_photos = photos?.filter(p => p.photo_type === 'activity').length || 0;
    const event_photos = photos?.filter(p => p.photo_type === 'event').length || 0;
      ...
      photos.forEach(photo => {
        ...
        photo.photo_students?.forEach(ps => {
          if (ps.students?.grade) {
            by_grade[ps.students.grade] = (by_grade[ps.students.grade] || 0) + 1;
          }
        });
        ...
        photo.photo_courses?.forEach(pc => {
          if (pc.courses?.grade) {
            by_grade[pc.courses.grade] = (by_grade[pc.courses.grade] || 0) + 1;
          }
        });
      });
    }
    ...
    console.error('Error in GET /api/admin/events/[id]/stats/gallery:', error);
    return NextResponse.json(
