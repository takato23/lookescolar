...
export const PATCH = withAuth(async (req: NextRequest, { params }: { params: { id: string } }) => {
  try {
    ...
    const { enabled, expires_in_days, allow_download, allow_share, max_views } = PatchSchema.parse(body);
    ...
    if (updErr) {
      return NextResponse.json({ error: 'Failed to update event', details: updErr.message }, { status: 500 });
    }
    ...
    if (enabled) {
      ...
      const { data: shares, error: shareErr } = await supabase
        ...
        .limit(5);
      if (shareErr) {
        return NextResponse.json({ error: 'Failed to check existing shares' }, { status: 500 });
      }
      ...
      if (active) {
        ...
        if (error) {
          return NextResponse.json({ error: 'Failed to create public share', details: error.message }, { status: 500 });
        }
