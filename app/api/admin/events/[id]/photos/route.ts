  ...
  withAuth(
    async (
      ...
      try {
        ...
        if (folderId) {
          logger.info(
            'Folder ID provided but folders system not integrated with legacy photos',
            {
              requestId,
              folderId,
              eventId,
        ...
        if (folderId) {
          ...
          logger.info('Using all event folders', {
            requestId,
            folderCount: targetFolderIds.length,
          });
        }
        ...
        // Add sorting
        const validSortFields = ['created_at', 'filename', 'file_size'];
