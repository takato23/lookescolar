...
const ZipDownloadSchema = z.object({
  ...
  photo_types: z.array(z.enum(['individual', 'group', 'activity', 'event'])).optional(),
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch photos for download', details: error.message },
        { status: 500 }
      );
    }
    ...
    const filename = zip_filename || `photos-${new Date().toISOString().split('T')[0]}.zip`;
    ...
    console.error('Error in POST /api/admin/events/[id]/bulk-download/zip:', error);
...
export const GET = withAuth(async (req: NextRequest, { params }: { params: { id: string, jobId: string } }) => {
  try {
    ...
    console.error('Error in GET /api/admin/events/[id]/bulk-download/zip/[jobId]:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
