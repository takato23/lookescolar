  ...
  try {
    ...
    if (action === 'zip') {
      ...
      return NextResponse.json({
        ...
        zip_filename: zip_filename || `photos-${new Date().toISOString().split('T')[0]}.zip`,
        action: 'zip',
      });
    } else {
    ...
    console.error('Error in POST /api/admin/events/[id]/bulk-download:', error);
...
export const GET = withAuth(async (req: NextRequest, { params }: { params: { id: string } }) => {
  try {
    ...
    console.error('Error in GET /api/admin/events/[id]/bulk-download/status:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
