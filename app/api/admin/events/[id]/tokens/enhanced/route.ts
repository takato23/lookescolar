...
import { enhancedTokenService, TokenType } from '@/lib/services/enhanced-token.service';
...
const BulkTokenGenerationSchema = z.object({
  token_type: z.enum(['student_access', 'family_access', 'group_access']).default('family_access'),
  expiry_days: z.number().min(1).max(365).default(30),
  distribution_method: z.enum(['email', 'whatsapp', 'sms', 'print', 'direct']).default('email'),
...
const TokenRotationSchema = z.object({
  rotation_reason: z.enum(['expiry_warning', 'security_breach', 'admin_request', 'scheduled_rotation']).default('admin_request'),
  notify_families: z.boolean().default(true),
...
const TokenAnalyticsSchema = z.object({
  ...
  token_type: z.enum(['student_access', 'family_access', 'group_access', 'all']).default('all'),
...
export async function GET(
  ...
  try {
    ...
    if (tokensError) {
      console.error(`[${requestId}] Error fetching enhanced tokens:`, tokensError);
      return NextResponse.json(
    ...
    const processedTokens = (enhancedTokens || []).map(token => {
      ...
      return {
        ...
        expires_in_days: Math.ceil((expiresAt.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)),
    ...
    stats.avg_usage_per_token = stats.total > 0 ? Math.round(stats.total_usage / stats.total * 100) / 100 : 0;
    stats.students_with_access = [...new Set(
      (enhancedTokens || []).flatMap(t => t.student_ids || [])
    )].length;
    stats.families_with_access = [...new Set(
...
export async function POST(
  ...
  try {
    ...
    if (notify_families && bulkResult.successful.size > 0) {
      ...
      console.log(`[${requestId}] Family notifications requested for ${bulkResult.successful.size} tokens`);
    }
    ...
    return NextResponse.json({
      ...
      successful_tokens: Array.from(bulkResult.successful.values()).map(token => ({
        id: token.id,
        type: token.type,
...
export async function PUT(
  ...
  try {
    ...
    if (force_rotation) {
      ...
      tokensToRotate = { tokens: (data || []).map(t => ({
        id: t.id,
        token: t.token,
      ...
      // Get expiring tokens
      tokensToRotate = await enhancedTokenService.getExpiringTokens(days_before_expiry);
      
      // Filter by event
      tokensToRotate.tokens = tokensToRotate.tokens.filter(t => t.eventId === eventId);
    }
    ...
    // Perform rotation
    console.log(`[${requestId}] Rotating ${tokensToRotate.tokens.length} tokens`);
    ...
    for (const oldToken of tokensToRotate.tokens) {
      try {
        ...
        let newTokenData;
        if (oldToken.type === 'family_access' && oldToken.studentIds && oldToken.familyEmail) {
          newTokenData = await enhancedTokenService.generateFamilyToken(
            oldToken.studentIds,
            oldToken.familyEmail,
            { rotateExisting: true }
          );
        } else if (oldToken.type === 'student_access' && oldToken.studentIds?.[0]) {
          newTokenData = await enhancedTokenService.generateStudentToken(
            oldToken.studentIds[0],
            { rotateExisting: true }
          );
        } else {
          throw new Error(`Unsupported token type for rotation: ${oldToken.type}`);
        }
        ...
        if (notify_families && newTokenData.familyEmail) {
          console.log(`[${requestId}] Queuing notification for family: ${newTokenData.familyEmail}`);
        }
        ...
        console.error(`[${requestId}] Failed to rotate token ${oldToken.id}:`, error.message);
      }
    }
