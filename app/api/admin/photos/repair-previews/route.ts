...
async function handlePOST(request: NextRequest) {
  ...
  try {
    ...
    const { data: photos, error } = await supabase
      .from('photos')
      .select(
        'id, storage_path, preview_path, watermark_path, original_filename, event_id'
      )
      .eq('event_id', eventId)
      .limit(2000);
    ...
    for (const p of photos || []) {
      ...
      if (!needs) {
        ...
        const toCheck = [
          (p as any).preview_path,
          (p as any).watermark_path,
      ...
      try {
        ...
        if ((p as any).storage_path) {
          try {
            const { data: file, error: dlErr } = await supabase.storage
              ...
              .download((p as any).storage_path);
            if (!dlErr && file) {
              imageBuffer = Buffer.from(await file.arrayBuffer());
            }
          } catch (e) {
            console.log(
              'Could not download original, will regenerate from existing preview'
            );
          }
        }
        ...
        if (!imageBuffer && (p as any).preview_path) {
          try {
            const { data: file, error: dlErr } = await supabase.storage
              .from(PREVIEW_BUCKET)
              .download((p as any).preview_path);
            if (!dlErr && file) {
              imageBuffer = Buffer.from(await file.arrayBuffer());
            }
          } catch (e) {
          ...
          .update({
            ...
            metadata: {
              ...
              optimization_ratio: Math.round(
                (((p as any).file_size - optimizedResult.actualSizeKB * 1024) /
                  (p as any).file_size) *
                  100
              ),
            },
