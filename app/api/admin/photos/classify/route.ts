  ...
  classifications: z
    .array(
      z.object({
        ...
        photoType: z
          .enum(['individual', 'group', 'activity', 'event'])
          .optional(),
        confidenceScore: z.number().min(0).max(1).optional(),
      })
    )
    ...
    return NextResponse.json(
      {
        ...
        invalid_photos: invalidPhotos.map((p) => ({
          id: p.id,
          filename: p.filename,
        })),
      },
...
async function batchClassify(body: any, adminId: string) {
  ...
  await supabaseAdmin.from('admin_activity').insert({
    ...
    metadata: {
      ...
      successful,
      failed,
      actions: validatedData.classifications.reduce(
        (acc, c) => {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const classification_stats = {
      ...
      unclassified:
        stats?.filter((s) => !s.course_id && s.photo_type === 'individual')
          .length || 0,
      in_courses: stats?.filter((s) => s.course_id).length || 0,
      by_type: {
        individual:
          stats?.filter((s) => s.photo_type === 'individual').length || 0,
