 ...
 */
function parseStudentQR(
  qrText: string
): { studentId: string; studentName: string; eventId: string } | null {
...
async function assignPhotoToStudent(
  ...
  try {
    ...
    if (assignError) {
      console.error(
        `[${requestId}] Error assigning photo to student:`,
        assignError.message
      );
    ...
    console.error(
      `[${requestId}] Error in assignPhotoToStudent:`,
...
async function handlePOST(request: NextRequest) {
  try {
    ...
    console.log(
      '[simple-upload] Starting upload process, requestId:',
      requestId
    );
    ...
    const formEventId =
      typeof formEventIdRaw === 'string' ? formEventIdRaw : null;
    const queryEventIdRaw = request.nextUrl.searchParams.get('eventId');
    const queryEventId =
      typeof queryEventIdRaw === 'string' ? queryEventIdRaw : null;
    const photoTypeRaw = formData.get('photo_type');
    const photoType =
      typeof photoTypeRaw === 'string' &&
      ['private', 'public', 'classroom'].includes(photoTypeRaw)
        ? photoTypeRaw
        : 'private';
    ...
    for (const file of files) {
      if (!(file instanceof File) || file.size === 0) {
        invalidFiles.push({
          ...
          reason: 'Invalid file object or empty file',
        });
        continue;
      }
      ...
      try {
        ...
        const validatedFile = new File([arrayBuffer], file.name, {
          type: file.type,
        });
    ...
    if (validFiles.length === 0) {
      console.log(
        '[simple-upload] No valid files found after security validation'
      );
      return NextResponse.json(
        {
          success: false,
          error:
            'No se encontraron archivos válidos después de la validación de seguridad',
    ...
    if (invalidFiles.length > 0) {
      console.log(
        '[simple-upload] Some files failed validation:',
        invalidFiles
      );
    }
      ...
      try {
        ...
        try {
          ...
          if (qrResult?.text) {
            ...
            console.log(`[${requestId}] QR detected in ${file.name}:`, {
              qr: `${normalizedQR?.substring(0, 20)}...`,
            });
            ...
            if (normalizedQR) {
              ...
              if (
                parsedQR &&
                isValidUUID(parsedQR.studentId) &&
                isValidUUID(parsedQR.eventId)
              ) {
                qrDetectionResult = {
                  qrText: normalizedQR,
                ...
                console.log(
                  `[${requestId}] Valid student QR detected for ${parsedQR.studentName}`
                );
              } else {
                console.log(
                  `[${requestId}] QR detected but not valid student format`
                );
              }
            }
          } else {
        ...
        const finalEventId: string | null =
          formEventId && isValidUUID(formEventId)
            ? formEventId
        ...
        console.log(
          `[${requestId}] Final event ID resolution for ${file.name}:`,
          {
            finalEventId: finalEventId
              ? `${finalEventId.substring(0, 8)}***`
              : null,
            source:
              finalEventId === formEventId
                ? 'form'
                : finalEventId === queryEventId
                  ? 'query'
                  : finalEventId === qrDetectionResult?.eventId
                    ? 'qr'
                    : 'none',
          }
        );
        ...
        const containerPrefix = finalEventId
          ? `events/${finalEventId}`
          : 'shared';
        ...
        if (finalEventId) {
          try {
            ...
            if (e?.name || e?.school_name) {
              wmLabel =
                `${e.school_name || ''}${e.school_name && e.name ? ' · ' : ''}${e.name || ''}`.trim() ||
                wmLabel;
            }
          } catch {}
        }
        ...
        try {
          console.log(
            `[${requestId}] Starting FreeTierOptimizer for ${file.name}`
          );

          // Check for explicit optimization parameters from form data
          const forceOptimization =
            formData.get('force_optimization') === 'true';
          const targetSizeKB =
            parseInt(formData.get('target_size_kb') as string) || 35;
          const maxDimension =
            parseInt(formData.get('max_dimension') as string) || 500;

          // Use more aggressive settings for event library uploads
          const optimizationConfig = {
            targetSizeKB: targetSizeKB, // Use explicit or default to 35KB
            maxDimension: maxDimension, // Use explicit or default to 500px
            watermarkText: wmLabel,
            enableOriginalStorage: false,
          };

          console.log(
            `[${requestId}] Optimization config:`,
            optimizationConfig
          );
          ...
          console.log(
            `[${requestId}] FreeTierOptimizer completed for ${file.name}`
          );
        } catch (optimizationError) {
          console.error(
            `[${requestId}] FreeTierOptimizer failed for ${file.name}:`,
            optimizationError
          );
          ...
          try {
            ...
            console.log(
              `[${requestId}] Fallback processing completed for ${file.name}`
            );
          } catch (fallbackError) {
            console.error(
              `[${requestId}] Both optimization and fallback failed for ${file.name}:`,
              fallbackError
            );
        ...
        if (upPrev.error) {
          console.error(
            `[${requestId}] Storage upload failed for ${file.name}:`,
            upPrev.error
          );
        ...
        if (!dbEventId) {
          console.log(
            `[${requestId}] No event_id for ${file.name}, looking for general event`
          );
          ...
          if (generalEvent) {
            ...
            console.log(
              `[${requestId}] Using existing general event: ${dbEventId.substring(0, 8)}***`
            );
          } else {
            console.log(
              `[${requestId}] No general event found, photo cannot be saved - database requires event_id`
            );
            throw new Error(
              'Database requires event_id - no general event available'
            );
          }
        }
        ...
        const { data: photo, error: dbError } = await supabase
          .from('photos')
          .insert({
            filename: file.name, // Using correct field name
            storage_path: finalPreviewPath,
            file_size_bytes: optimizedResult.actualSizeKB * 1024,
            event_id: dbEventId,
        ...
        if (dbError) {
          console.error(
            `[${requestId}] Database error for ${file.name}:`,
            dbError
          );
        ...
        console.log(
          `[${requestId}] Database save completed for ${file.name}, photo ID: ${photo.id}`
        );
        ...
        if (!verifyPhoto) {
          console.error(
            `[${requestId}] Photo verification failed for ${file.name}`
          );
          throw new Error('Photo verification failed');
        }

        console.log(
          `[${requestId}] Photo verification successful for ${file.name}`
        );
        ...
        if (qrDetectionResult) {
          console.log(
            `[${requestId}] Attempting to assign photo ${photo.id} to student from QR`
          );
          ...
          if (assignmentResult?.success) {
            console.log(
              `[${requestId}] Photo assigned successfully, generating token for student`
            );
            try {
              tokenGenerationResult =
                await tokenService.generateTokenForSubject(
                  qrDetectionResult.studentId,
              ...
              console.error(
                `[${requestId}] Error generating token for student:`,
                {
                  studentId: `${qrDetectionResult.studentId.substring(0, 8)}***`,
                  error:
    ...
    const qrDetectedCount = results.filter(
      (r) => r.qr_detected !== null
    ).length;
    const autoAssignedCount = results.filter(
      (r) => r.qr_detected?.assignment_success === true
    ).length;
...
export const POST =
  process.env.NODE_ENV === 'development'
    ? handlePOST
    : withAuth(handlePOST, { requireCSRF: true });
export const GET =
