...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 1. Verificar evento válido y activo
    ...
    // 3. Validar que todas las fotos pertenecen al evento y están disponibles públicamente
    ...
    if (!photos || photos.length !== photoIds.length) {
      return NextResponse.json(
        { error: 'Some photos are not available' },
        { status: 403 }
      );
    }

    // Validar que todas las fotos son del evento correcto y son públicamente visibles
    const invalidPhotos = photos.filter((photo) => {
      const subject = photo.subjects as any;
      return (
        !subject ||
        subject.event_id !== eventId ||
        subject.public_visible !== true
      );
    });

    if (invalidPhotos.length > 0) {
      console.warn(
        `[${requestId}] Invalid or private photos in public checkout`,
        {
          invalidCount: invalidPhotos.length,
          totalCount: photos.length,
        }
      );

      return NextResponse.json(
        { error: 'Some photos are not publicly available for this event' },
        { status: 403 }
      );
    }
    ...
    // 6. Crear orden en base de datos (sin subject_id para público)
    const orderId = crypto.randomUUID();
    const { error: orderError } = await supabase.from('orders').insert({
      id: orderId,
      subject_id: null, // NULL para órdenes públicas
      event_id: eventId, // Referencia directa al evento
    ...
    if (orderError) {
      console.error(`[${requestId}] Error creating public order:`, orderError);
      return NextResponse.json(
    ...
    const mpItems = validatedItems.map((item) => {
      ...
      return {
        title: `${item.label} - ${photo?.filename || 'Photo'} (${event.name})`,
        quantity: item.quantity,
